...
PreparedStatement *DatabaseLayer::DoPrepareStatementWchar(const wxChar *format, ...)
{
	...
	return PrepareStatement(strQuery);
}
#endif
...
number_t DatabaseLayer::GetSingleResultNumber(const wxString& strSQL, int nField, bool bRequireUniqueResult /*= true*/)
{
	wxVariant variant((long)nField);
	return GetSingleResultNumber(strSQL, &variant, bRequireUniqueResult);
}

number_t DatabaseLayer::GetSingleResultNumber(const wxString& strSQL, const wxString& strField, bool bRequireUniqueResult /*= true*/)
{
	wxVariant variant(strField);
	return GetSingleResultNumber(strSQL, &variant, bRequireUniqueResult);
}

number_t DatabaseLayer::GetSingleResultNumber(const wxString& strSQL, const wxVariant* field, bool bRequireUniqueResult /*= true*/)
{
	bool valueRetrievedFlag = false;
	number_t value = -1;
