	...
	: wxDialog(parent, wxID_ANY, _("Procedures and functions")), m_docModule(moduleDoc), m_codeEditor(parent)
{
	...
	CMetaObjectModule* metaModule = dynamic_cast<CMetaObjectModule*>(moduleDoc->GetMetaObject());
	wxASSERT(metaModule);
...
void CFunctionList::OnButtonOk(wxCommandEvent& event)
{
	...
	if (lSelectedItem != wxNOT_FOUND) {
		...
		if (line.m_offset != wxNOT_FOUND) {
			...
			CMetaObjectModule* metaModule = wxDynamicCast(
				m_docModule->GetMetaObject(), CMetaObjectModule
			);
			wxASSERT(metaModule);
