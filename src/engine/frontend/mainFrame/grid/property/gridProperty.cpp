...
void CPropertyObjectGrid::ShowProperty()
{
	CPropertyObjectGrid::ClearSelectedCell(); bool hasBlocks = false;
...
CPropertyObjectGrid::CPropertyObjectGrid(CGrid* ownerGrid) : IPropertyObject(),
...
void CPropertyObjectGrid::OnPropertyChanged(Property* property, const wxVariant& oldValue, const wxVariant& newValue)
{
	...
	for (auto& coords : m_currentBlocks) {
		...
		CPropertyObjectGrid::OnPropertyChanged(property, coords);
	}
