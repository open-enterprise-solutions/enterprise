...
void KeyBinderDialog::Initialize()
{
	...
	for (unsigned int i = 0; i < m_commands.size(); ++i)
	{
		...
		if (iterator == groups.end())
		{
			...
			groups.emplace(std::make_pair(m_commands[i]->group.ToAscii(), groupNode));
		}
		else
		{
...
void KeyBinderDialog::OnCommandTreeSelectionChanged(wxTreeEvent& event)
{
	...
	m_keyMonitorCtrl->Enable(data != nullptr);
	m_shortcutsListCtrl->Enable(data != nullptr);
	m_removeAllButton->Enable(data != nullptr);
