...
public:
	...
	// WHAT THE CELL COMMITS is whatever the box holds — typed or written in the dialog.
...
private:
	void OnExpand(wxCommandEvent&) {
		...
		if (m_expand && m_expand(text) && box != nullptr)
			box->SetValue(text);
	}
...
public:
	ibStructureNode(int level, ibStructureNode* parent) : m_level(level), m_parent(parent) {}

	// wxNOT_FOUND = the Report node. Every other row is a level, by index into the ladder.
...
public:
	...
	explicit ibComposerStructureModel(std::function<ibValueGroupList*()> levels)
		: m_levels(std::move(levels)) {
	}

	size_t LevelCount() const {
		ibValueGroupList* levels = m_levels ? m_levels() : nullptr;
		return levels != nullptr ? levels->Count() : 0u;
	}

	// RE-READ THE LADDER. The rows survive it (they are pooled by position); what changes is how
	// many of them the walk hands out and which of them still has a child.
	void Rebuild() {
		const size_t count = LevelCount();
		NodeFor(wxNOT_FOUND);
		for (size_t i = 0; i < count; ++i)
			NodeFor((int)i);
		for (size_t i = 0; i < m_nodes.size(); ++i)
			m_nodes[i]->SetHasChild((int)i < (int)count);   // node i stands for level (i - 1)
		Cleared();
	}

	ibDataViewItem RootItem() const { return ibDataViewItem(NodeFor(wxNOT_FOUND)); }
	// The row for a level, or an invalid item when the ladder is shorter than that.
	ibDataViewItem ItemForLevel(int level) const {
		if (level < 0 || (size_t)level >= LevelCount())
			return ibDataViewItem();
		return ibDataViewItem(NodeFor(level));
	}
	// THE LEVEL A ROW STANDS FOR, or wxNOT_FOUND on the Report node and on nothing-selected — so
	// a command aimed at a level does nothing there instead of acting on level zero.
	...
	void GetValue(wxVariant& variant, const ibDataViewItem& item, unsigned int col) const override {
		...
		if (node == nullptr)
			return;
		if (node->GetLevel() == wxNOT_FOUND) {
			// THE ROOT SAYS WHAT IT IS. Everything below it is a level of this one report.
			if (col == kColNode)
				variant = _("Report");
			return;
		}
		if (col == kColNode) {
			// WHAT KIND OF NODE THIS IS, which today has exactly one answer below the root. It
			// stops having one when a table with rows and columns arrives, and this column is
			// where that will read.
			variant = _("Grouping");
			return;
		}

		ibValueGroupList* levels = m_levels ? m_levels() : nullptr;
		const size_t idx = (size_t)node->GetLevel();
		if (levels == nullptr || idx >= levels->Count())
			return;   // BOUNDS FIRST — a queued paint can outlive the level it was queued for

		if (col == kColField)
			variant = levels->GetFieldObject(idx) != nullptr ? levels->GetFieldObject(idx)->GetString()
			                                                 : levels->GetField(idx);
		else if (col == kColKind)
			// THE KIND AS ITS OWN CAPTION — a registered enumeration presenting itself, the same
			// way the list's grouping tab shows it.
			variant = ibValue::CreateEnumObject<ibValueEnumGroupKind>(levels->GetKind(idx)).GetString();
	}
	...
	bool HasValue(const ibDataViewItem& item, unsigned col) const override {
		const ibStructureNode* node = static_cast<const ibStructureNode*>(item.GetID());
		if (node != nullptr && node->GetLevel() == wxNOT_FOUND)
			return col == kColNode;
		return true;
	}

	// ...AND IT IS NOT EDITED. Its caption is what the report is, not a setting.
	...
	unsigned int GetFirstFetch(const ibDataViewItem& parent, const ibDataViewItem& /*anchor*/,
		...
		const ibStructureNode* node = static_cast<const ibStructureNode*>(parent.GetID());
		if (node == nullptr)
			return 0;
		const int child = node->GetLevel() + 1;   // the Report node is wxNOT_FOUND, so its child is 0
		if (child < 0 || (size_t)child >= LevelCount())
			return 0;
		out.Add(ibDataViewItem(NodeFor(child)));
		return 1;
	}

private:
	// The row for a position, created on the way down so every row has its parent.
	ibStructureNode* NodeFor(int level) const {
		const size_t idx = (size_t)(level + 1);   // wxNOT_FOUND (the Report node) is slot 0
		while (m_nodes.size() <= idx) {
			const int lv = (int)m_nodes.size() - 1;
			ibStructureNode* parent = m_nodes.empty() ? nullptr : m_nodes.back().get();
			m_nodes.push_back(wxObjectDataPtr<ibStructureNode>(new ibStructureNode(lv, parent)));
		}
		return m_nodes[idx].get();
	}

	std::function<ibValueGroupList*()> m_levels;   // asked every time — this window keeps no copy
	mutable std::vector<wxObjectDataPtr<ibStructureNode>> m_nodes;
};
...
wxWindow* ibComposerSettingsPanel::BuildStructurePane(wxWindow* parent)
{
	...
	// THE FIELD THE LEVEL GROUPS BY — a VALUE, edited through the shared cell, which opens the
	// panel's field picker because a composition field is what it holds.
	m_structureView->GetRootColumnGroup()->AppendColumn(new ibDataViewColumn(_("Field"),
		new ibRowValueCellRenderer(this, ibComposerFieldChooser(this),
			[this](const ibDataViewItem& row) -> ibValue {
				ibValueGroupList* levels = Levels();
				const int idx = m_structureModel != nullptr ? m_structureModel->LevelAt(row) : wxNOT_FOUND;
				return (levels != nullptr && idx != wxNOT_FOUND && (size_t)idx < levels->Count())
					? ibValue(levels->GetFieldObject((size_t)idx)) : ibValue();
			},
			[this](const ibDataViewItem& row, const ibValue& value) {
				ibValueGroupList* levels = Levels();
				const int idx = m_structureModel != nullptr ? m_structureModel->LevelAt(row) : wxNOT_FOUND;
				if (levels == nullptr || idx == wxNOT_FOUND || (size_t)idx >= levels->Count())
					return;
				// An empty value CLEARS the level's field. ⚠ SetLine, not Remove+Add: the second
				// moves the edited level to the end, and the order IS the nesting.
				ibValueCompositionField* field = nullptr;
				const bool chosen = value.ConvertToValue(field) && field != nullptr;
				levels->SetLine((size_t)idx, chosen ? field->GetPath() : wxString(), levels->GetKind((size_t)idx));
				ReloadStructure(idx);
			}),
		ibComposerStructureModel::kColField, FromDIP(220), wxAlignment::wxALIGN_LEFT));

	m_structureView->GetRootColumnGroup()->AppendColumn(new ibDataViewColumn(_("Kind"),
		new ibRowValueCellRenderer(this, ibComposerFieldChooser(this),
			[this](const ibDataViewItem& row) -> ibValue {
				ibValueGroupList* levels = Levels();
				const int idx = m_structureModel != nullptr ? m_structureModel->LevelAt(row) : wxNOT_FOUND;
				return (levels != nullptr && idx != wxNOT_FOUND && (size_t)idx < levels->Count())
					? ibValue::CreateEnumObject<ibValueEnumGroupKind>(levels->GetKind((size_t)idx)) : ibValue();
			},
			[this](const ibDataViewItem& row, const ibValue& value) {
				ibValueGroupList* levels = Levels();
				const int idx = m_structureModel != nullptr ? m_structureModel->LevelAt(row) : wxNOT_FOUND;
				if (levels == nullptr || idx == wxNOT_FOUND || (size_t)idx >= levels->Count())
					return;
				// ⚠ BY PATH, NOT BY OBJECT — the same line that crashed the designer on the list's
				// grouping tab: a facade list has no line object, and the object read back is null.
				levels->SetLine((size_t)idx, levels->GetField((size_t)idx),
					value.ConvertToEnumValue<ibQueryDimUnfold>());
				ReloadStructure(idx);
			}),
		ibComposerStructureModel::kColKind, FromDIP(150), wxAlignment::wxALIGN_LEFT));
	...
	m_structureModel = new ibComposerStructureModel([this] { return Levels(); });
...
wxWindow* ibComposerSettingsPanel::BuildSettingsPane(wxWindow* parent)
{
	...
	// worlds").
	wxNotebook* tabs = new wxNotebook(pane, wxID_ANY);
	m_filterEditor = new ibFilterEditor(tabs, m_settings, m_fieldTree.get());
	tabs->AddPage(m_filterEditor, _("Filter"), true);
	...
	const ibMetaData* metaData)
{
	...
	try {
		...
		const ibMetaData* against = metaData != nullptr ? metaData
		                                               : ibApplicationData::GetActiveMetaData();
		if (against != nullptr) {
			if (ibCompileValueCache* cache = against->GetCompileCache()) {
				ibRuntimeModuleDataObject* rootModule = nullptr;
				if (cache->FindCompileModule(against->GetCommonMetaObject(), rootModule) && rootModule != nullptr)
					if (ibCompileModule* context = rootModule->GetCompileModule())
						check.SetParent(context);
			}
			// RUNTIME HAS NO SUCH CACHE — there the root module IS live, and it is the parent.
			else if (ibSession* session = ibSession::Current()) {
				if (ibValueModuleManagerRuntimeConfiguration* root = session->GetManagerModule())
					if (ibCompileModule* context = root->GetCompileModule())
						check.SetParent(context);
			}
		}
...
wxString ibComposerSettingsPanel::CheckAllExpressions()
{
	...
	for (size_t i = 0; i < m_composer->ParameterCount(); ++i) {
		wxString complaint;
		if (CheckExpression(m_composer->GetParameterExpression(i), complaint, m_composer->GetSourceMetaData()))
			continue;
...
bool ibComposerSettingsPanel::EditParameterExpression(wxString& text)
{
	...
	const ibMetaData* metaData = m_composer != nullptr ? m_composer->GetSourceMetaData() : nullptr;
...
int ibComposerSettingsPanel::SelectedLevel() const
{
	if (m_structureView == nullptr || m_structureModel == nullptr)
		return wxNOT_FOUND;
	const ibDataViewItem row = m_structureView->GetSelection();
	if (!row.IsOk())
		return wxNOT_FOUND;
	const int level = m_structureModel->LevelAt(row);
	ibValueGroupList* levels = Levels();
	return (levels != nullptr && level != wxNOT_FOUND && (size_t)level < levels->Count())
		? level : wxNOT_FOUND;
}
...
void ibComposerSettingsPanel::ReloadStructure(int selectLevel)
{
	...
	CallAfter([this, selectLevel] {
		if (m_structureModel == nullptr || m_structureView == nullptr)
			return;
		// THE WHOLE LADDER IS OPEN. It is a chain — one child per level — so a collapsed node
		// hides everything below it, and there is nothing here big enough for that to be a mercy.
		m_structureView->Expand(m_structureModel->RootItem());
		for (size_t i = 0; i < m_structureModel->LevelCount(); ++i)
			m_structureView->Expand(m_structureModel->ItemForLevel((int)i));

		const ibDataViewItem row = m_structureModel->ItemForLevel(selectLevel);
...
void ibComposerSettingsPanel::UpdateSettingsHeader()
{
	...
	ibValueGroupList* levels = Levels();
	if (level != wxNOT_FOUND && levels != nullptr && (size_t)level < levels->Count()) {
		const wxString field = levels->GetFieldObject((size_t)level) != nullptr
			? levels->GetFieldObject((size_t)level)->GetString() : levels->GetField((size_t)level);
		node = field.IsEmpty() ? _("Grouping") : _("Grouping") + wxT(": ") + field;
	}

	// ⚠ ASCII ONLY IN UI LITERALS — this file has no BOM, so MSVC reads it as ANSI.
	m_settingsHeader->SetLabel(_("Settings of") + wxT(": ") + node
		+ wxT("  ") + _("(applied to the whole composition)"));
...
void ibComposerSettingsPanel::OnStructureAdd(wxCommandEvent&)
{
	...
	levels->Add(field, ibQueryDimUnfold::Elements);
	MarkSettingsTouched();
	ReloadStructure((int)levels->Count() - 1);
}
...
void ibComposerSettingsPanel::OnStructureRemove(wxCommandEvent&)
{
	const int level = SelectedLevel();
	ibValueGroupList* levels = Levels();
	if (level == wxNOT_FOUND || levels == nullptr)
		return;
	levels->Remove((size_t)level);
	MarkSettingsTouched();
	// The cursor stays where the deleted level was, or on the last one when it was the last.
	const int count = (int)levels->Count();
	ReloadStructure(count == 0 ? wxNOT_FOUND : std::min(level, count - 1));
}
...
void ibComposerSettingsPanel::MoveStructureLevel(int delta)
{
	...
	if (level == wxNOT_FOUND || levels == nullptr)
		return;
	if (!levels->Move((size_t)level, delta))
		return;   // already at the end it was moved towards
	MarkSettingsTouched();
	ReloadStructure(level + delta);
}
