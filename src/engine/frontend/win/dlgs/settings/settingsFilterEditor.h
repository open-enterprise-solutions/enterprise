...
//
// It edits a BUFFER (an ibValueListSettings the host owns and commits). Nothing in
// here knows about a model, a list or a report.
// ---------------------------------------------------------------------------
...
class ibFilterTreeModel;
class ibMetaData;

class ibFilterEditor : public wxPanel {
public:

	// `settings` — the buffer whose filter TREE is edited (never owned here).
	// `fields`   — which fields may be picked (never owned here either).
	ibFilterEditor(wxWindow* parent, ibValueListSettings* settings, ibSettingsFieldTree* fields);
	...
	// THE FILTER AS THE TREE IT IS — the root group everything hangs under. Never
	// null while a buffer is set: an empty filter is an empty ROOT GROUP, not the
	// absence of one, so the form always has a node to add to.
	ibValueFilterGroup* GetFilterRoot() const;
