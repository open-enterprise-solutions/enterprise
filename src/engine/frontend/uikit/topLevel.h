...
// Purpose:     Top level window, abstraction of ibFrame and ibDialog
...
// frame decorations type flags used in ibRenderer and ibColourScheme
enum
{
...
#define ibACTION_TOPLEVEL_ACTIVATE       wxT("activate")   // (de)activate the frame
#define ibACTION_TOPLEVEL_BUTTON_PRESS   wxT("pressbtn")   // press titlebar btn
#define ibACTION_TOPLEVEL_BUTTON_RELEASE wxT("releasebtn") // press titlebar btn
#define ibACTION_TOPLEVEL_BUTTON_CLICK   wxT("clickbtn")   // press titlebar btn
#define ibACTION_TOPLEVEL_MOVE           wxT("move")       // move the frame
#define ibACTION_TOPLEVEL_RESIZE         wxT("resize")     // resize the frame

//-----------------------------------------------------------------------------
// ibTopLevelWindow
//-----------------------------------------------------------------------------

#if 0 // UIKIT-REVIVE: top-level windows stay native for now; constants above are used
class FRONTEND_API ibTopLevelWindow : public wxTopLevelWindow,
                                     public ibInputConsumer
{
public:
    // construction
    ibTopLevelWindow() { Init(); }
    ibTopLevelWindow(wxWindow *parent,
                     wxWindowID id,
    ...
    virtual ibWindow *GetInputWindow() const override { return const_cast<ibTopLevelWindow*>(this); }
...
protected:
    ...
    wxDECLARE_DYNAMIC_CLASS(ibTopLevelWindow);
