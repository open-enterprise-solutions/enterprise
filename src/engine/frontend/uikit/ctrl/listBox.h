...
#define ibACTION_LISTBOX_SETFOCUS   wxT("setfocus")  // select the item
#define ibACTION_LISTBOX_MOVEDOWN   wxT("down")      // select item below
#define ibACTION_LISTBOX_MOVEUP     wxT("up")        // select item above
#define ibACTION_LISTBOX_PAGEDOWN   wxT("pagedown")  // go page down
#define ibACTION_LISTBOX_PAGEUP     wxT("pageup")    // go page up
#define ibACTION_LISTBOX_START      wxT("start")     // go to first item
#define ibACTION_LISTBOX_END        wxT("end")       // go to last item
#define ibACTION_LISTBOX_FIND       wxT("find")      // find item by 1st letter

// do something with the current item
#define ibACTION_LISTBOX_ACTIVATE   wxT("activate")  // activate (choose)
#define ibACTION_LISTBOX_TOGGLE     wxT("toggle")    // togglee selected state
#define ibACTION_LISTBOX_SELECT     wxT("select")    // sel this, unsel others
#define ibACTION_LISTBOX_SELECTADD  wxT("selectadd") // add to selection
#define ibACTION_LISTBOX_UNSELECT   wxT("unselect")  // unselect
#define ibACTION_LISTBOX_ANCHOR     wxT("selanchor") // anchor selection

// do something with the selection globally (not for single selection ones)
#define ibACTION_LISTBOX_SELECTALL   wxT("selectall")   // select all items
#define ibACTION_LISTBOX_UNSELECTALL wxT("unselectall") // unselect all items
#define ibACTION_LISTBOX_SELTOGGLE   wxT("togglesel")   // invert the selection
#define ibACTION_LISTBOX_EXTENDSEL   wxT("extend")      // extend to item

// ----------------------------------------------------------------------------
// ibListBox: a list of selectable items
// ----------------------------------------------------------------------------

#include <wx/listbox.h>       // wxItemContainer, wxLB_* styles
#include "frontend/uikit/ctrl/control.h"

// SEAM vs univ: wxListBoxBase sits on the native wxControl in our build;
// wxItemContainer carries the windowless items API
class FRONTEND_API ibListBox : public ibControl, public wxItemContainer, public wxScrollHelper
{
public:
    // wxListBoxBase shims (see the SEAM note above)
    bool HasMultipleSelection() const {
        return (GetWindowStyle() & (wxLB_MULTIPLE | wxLB_EXTENDED)) != 0;
    }
    ...
    // wxListBoxBase::HitTest lived in the lost base; without it the call
    // resolved to wxWindowBase::HitTest returning wxHT_WINDOW_INSIDE (=10!)
    using wxWindow::HitTest;
    int HitTest(const wxPoint& point) const { return DoListHitTest(point); }

    // ctors and such
    ibListBox() : wxScrollHelper(this) { Init(); }
    ibListBox(wxWindow *parent,
              wxWindowID id,
...
protected:
    ...
    // universal ibComboBox implementation internally uses ibListBox
    friend class ibComboBox;
...
public:
    ...
    // override the ibControl virtual methods
...
protected:
    ...
    // special hook for ibCheckListBox which allows it to update its internal
...
private:
    ...
    wxDECLARE_DYNAMIC_CLASS(ibListBox);
};
