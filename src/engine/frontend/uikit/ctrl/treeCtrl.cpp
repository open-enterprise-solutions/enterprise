...
using wxGenericTreeItems = std::vector<ibTreeItem*>;
...
// -----------------------------------------------------------------------------
// ibTreeCtrlBase — the re-declared base (see treeCtrl.h); implementations
// FORKED from src/common/treebase.cpp
// -----------------------------------------------------------------------------

ibTreeCtrlBase::ibTreeCtrlBase()
{
    // arbitrary default
    m_spacing = 18;
    ...
    Bind(wxEVT_CHAR_HOOK, &ibTreeCtrlBase::OnCharHook, this);
}
...
void ibTreeCtrlBase::SetItemState(const wxTreeItemId& item, int state)
{
...
bool ibTreeCtrlBase::HasAnyImages() const
{
    return HasImages() || m_imagesState.HasImages();
}

wxSize ibTreeCtrlBase::DoGetBestSize() const
{
    // SEAM vs wx: the canon walked every item through a TreeSizer here; a
    // tree is virtually always sizer-stretched, a sane default is enough
    return FromDIP(wxSize(120, 160));
}

void ibTreeCtrlBase::ExpandAll()
{
    if ( IsEmpty() )
        return;
...
void ibTreeCtrlBase::ExpandAllChildren(const wxTreeItemId& item)
{
    Freeze();
    // expand this item first, this might result in its children being added
    // on the fly
...
void ibTreeCtrlBase::CollapseAll()
{
    if ( IsEmpty() )
        return;
...
void ibTreeCtrlBase::CollapseAllChildren(const wxTreeItemId& item)
{
    Freeze();
...
bool ibTreeCtrlBase::IsEmpty() const
{
    return !GetRootItem().IsOk();
}

void ibTreeCtrlBase::OnCharHook(wxKeyEvent& event)
{
    if ( GetEditControl() )
    {
...
class ibTreeRenameTimer: public wxTimer
{
public:
    ...
    ibTreeRenameTimer( ibTreeCtrl *owner );
...
private:
    ...
    wxDECLARE_NO_COPY_CLASS(ibTreeRenameTimer);
};

// control used for in-place edit
// SEAM: the in-place editor is OUR text control, not the native wxTextCtrl
class ibTreeTextCtrl: public ibTextCtrl
{
public:
    ibTreeTextCtrl(ibTreeCtrl *owner, ibTreeItem *item);

    void EndEdit( bool discardChanges );

    const ibTreeItem* item() const { return m_itemEdited; }
...
private:
    ibTreeCtrl  *m_owner;
    ibTreeItem  *m_itemEdited;
    wxString            m_startValue;
    bool                m_aboutToFinish;

    wxDECLARE_EVENT_TABLE();
    wxDECLARE_NO_COPY_CLASS(ibTreeTextCtrl);
};

// timer used to clear ibTreeCtrl::m_findPrefix if no key was pressed
// for a sufficiently long time
class ibTreeFindTimer : public wxTimer
{
public:
    // reset the current prefix after half a second of inactivity
    enum { DELAY = 500 };

    ibTreeFindTimer( ibTreeCtrl *owner ) { m_owner = owner; }
...
private:
    ...
    wxDECLARE_NO_COPY_CLASS(ibTreeFindTimer);
};
...
public:
    ...
    void DeleteChildren(ibTreeCtrl *tree);

    // get count of all children (and grand children if 'recursively')
    size_t GetChildrenCount(bool recursively = true) const;

    void Insert(ibTreeItem *child, size_t index)
        { m_children.insert(m_children.begin() + index, child); }

    // calculate and cache the item size using either the provided DC (which is
    // supposed to already have ibTreeCtrl font selected into it!) or a
    // wxInfoDC associated with the control
    void CalculateSize(ibTreeCtrl *control, wxReadOnlyDC& dc)
        { DoCalculateSize(control, dc); }
    void CalculateSize(ibTreeCtrl *control);

    void GetSize( int &x, int &y, const ibTreeCtrl* );
    ...
    void RecursiveResetTextSize();
        ...
        // on the item's label
    ibTreeItem *HitTest( const wxPoint& point,
                                const ibTreeCtrl *,
                                int &flags,
                                int level );
...
private:
    ...
    void DoCalculateSize(ibTreeCtrl *control, wxReadOnlyDC& dc);
    ...
    ibTreeItem  *m_parent;       // parent of this item
    ...
    wxDECLARE_NO_COPY_CLASS(ibTreeItem);
};
...
// Get ibTreeItem from wxTreeItemId.
static inline ibTreeItem* GetItemPtr(const wxTreeItemId& item)
{
    return static_cast<ibTreeItem*>(item.GetID());
}
...
static bool
IsDescendantOf(const ibTreeItem *parent, const ibTreeItem *item)
{
    while ( item )
    {
        if ( item == parent )
...
static int
FindItemIndex(const wxGenericTreeItems& items, const ibTreeItem* item)
{
...
// -----------------------------------------------------------------------------
// ibTreeRenameTimer (internal)
// -----------------------------------------------------------------------------

ibTreeRenameTimer::ibTreeRenameTimer( ibTreeCtrl *owner )
{
    m_owner = owner;
}

void ibTreeRenameTimer::Notify()
{
    m_owner->OnRenameTimer();
}

//-----------------------------------------------------------------------------
// ibTreeTextCtrl (internal)
//-----------------------------------------------------------------------------

wxBEGIN_EVENT_TABLE(ibTreeTextCtrl,ibTextCtrl)
    EVT_CHAR           (ibTreeTextCtrl::OnChar)
    EVT_KILL_FOCUS     (ibTreeTextCtrl::OnKillFocus)
wxEND_EVENT_TABLE()

ibTreeTextCtrl::ibTreeTextCtrl(ibTreeCtrl *owner,
                               ibTreeItem *itm)
              : m_itemEdited(itm), m_startValue(itm->GetText())
{
    ...
    // SEAM: GetSizeFromTextSize lived in wxControlBase — take the theme
    // text paddings instead
    wxSize fullSize = ibThemeEngine::Get()->GetRenderer()
        ->GetTextTotalArea(this, wxRect(textSize)).GetSize();
...
void ibTreeTextCtrl::EndEdit(bool discardChanges)
{
    if ( m_aboutToFinish )
    {
...
bool ibTreeTextCtrl::AcceptChanges()
{
...
void ibTreeTextCtrl::Finish( bool setfocus )
{
...
void ibTreeTextCtrl::OnChar( wxKeyEvent &event )
{
...
void ibTreeTextCtrl::IncreaseSizeForText( const wxString& text )
{
    // auto-grow the textctrl:
    wxSize parentSize = m_owner->GetClientSize();
    wxPoint myPos = GetPosition();
    wxSize mySize = GetSize();
    // SEAM: GetSizeFromText lived in wxControlBase — measure through the
    // theme text paddings instead
    int sx = ibThemeEngine::Get()->GetRenderer()
        ->GetTextTotalArea(this, wxRect(GetTextExtent(text))).GetSize().x;
...
void ibTreeTextCtrl::OnKillFocus( wxFocusEvent &event )
{
    if ( !m_aboutToFinish )
    {
        m_aboutToFinish = true;
...
ibTreeItem::ibTreeItem(ibTreeItem *parent,
                                     const wxString& text,
...
void ibTreeItem::DeleteChildren(ibTreeCtrl *tree)
{
    size_t count = m_children.size();
    for ( size_t n = 0; n < count; n++ )
    {
        ibTreeItem *child = m_children[n];
...
size_t ibTreeItem::GetChildrenCount(bool recursively) const
{
    size_t count = m_children.size();
    if ( !recursively )
        return count;
...
void ibTreeItem::GetSize( int &x, int &y,
                                 const ibTreeCtrl *theButton )
{
...
ibTreeItem *ibTreeItem::HitTest(const wxPoint& point,
                                              const ibTreeCtrl *theCtrl,
                                              int &flags,
                                              int level)
{
    ...
    for ( size_t n = 0; n < count; n++ )
    {
        ibTreeItem *res = m_children[n]->HitTest( point,
                                                         theCtrl,
                                                         flags,
                                                         level + 1 );
        if ( res != nullptr )
...
int ibTreeItem::GetCurrentImage() const
{
    int image = NO_IMAGE;
    if ( IsExpanded() )
    {
...
void ibTreeItem::CalculateSize(ibTreeCtrl* control)
{
...
void
ibTreeItem::DoCalculateSize(ibTreeCtrl* control,
                                   wxReadOnlyDC& dc)
{
...
void ibTreeItem::RecursiveResetSize()
{
    m_width = 0;
...
void ibTreeItem::RecursiveResetTextSize()
{
    m_width = 0;
    m_widthText = -1;
...
// -----------------------------------------------------------------------------
// ibTreeCtrl implementation
// -----------------------------------------------------------------------------

wxIMPLEMENT_DYNAMIC_CLASS(ibTreeCtrl, ibControl);

wxBEGIN_EVENT_TABLE(ibTreeCtrl, ibTreeCtrlBase)
    EVT_PAINT          (ibTreeCtrl::OnPaint)
    EVT_SIZE           (ibTreeCtrl::OnSize)
    EVT_MOUSE_EVENTS   (ibTreeCtrl::OnMouse)
    EVT_KEY_DOWN       (ibTreeCtrl::OnKeyDown)
    EVT_CHAR           (ibTreeCtrl::OnChar)
    EVT_SET_FOCUS      (ibTreeCtrl::OnSetFocus)
    EVT_KILL_FOCUS     (ibTreeCtrl::OnKillFocus)
    EVT_TREE_ITEM_GETTOOLTIP(wxID_ANY, ibTreeCtrl::OnGetToolTip)
    EVT_SYS_COLOUR_CHANGED(ibTreeCtrl::OnSysColourChanged)
wxEND_EVENT_TABLE()
...
bool ibTreeCtrl::Create(wxWindow *parent,
                               wxWindowID id,
                               const wxPoint& pos,
                               const wxSize& size,
                               long style,
                               ...
                               const wxString& name )
{
    if ( !ibControl::Create( parent, id, pos, size,
...
void ibTreeCtrl::EnableBellOnNoMatch( bool on )
{
    m_findBell = on;
}

void ibTreeCtrl::InitVisualAttributes()
{
    ...
    m_hilightBrush = wxBrush(wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), HIGHLIGHT)); // SEAM: theme, not wxSYS
    m_hilightUnfocusedBrush = wxBrush(wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), HIGHLIGHT).ChangeLightness(130)); // SEAM: theme, not wxSYS
...
unsigned int ibTreeCtrl::GetCount() const
{
    if ( !m_anchor )
    {
        // the tree is empty
...
void ibTreeCtrl::SetIndent(unsigned int indent)
{
    m_indent = indent;
    m_dirty = true;
}
...
size_t
ibTreeCtrl::GetChildrenCount(const wxTreeItemId& item,
...
void ibTreeCtrl::SetWindowStyleFlag(long styles)
{
...
wxString ibTreeCtrl::GetItemText(const wxTreeItemId& item) const
{
...
int ibTreeCtrl::GetItemImage(const wxTreeItemId& item,
...
wxTreeItemData *ibTreeCtrl::GetItemData(const wxTreeItemId& item) const
{
...
int ibTreeCtrl::DoGetItemState(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTREE_ITEMSTATE_NONE, wxT("invalid tree item") );

    ibTreeItem *pItem = GetItemPtr(item);
    return pItem->GetState();
}

wxColour ibTreeCtrl::GetItemTextColour(const wxTreeItemId& item) const
{
...
wxColour
ibTreeCtrl::GetItemBackgroundColour(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxNullColour, wxT("invalid tree item") );
...
wxFont ibTreeCtrl::GetItemFont(const wxTreeItemId& item) const
{
...
void
ibTreeCtrl::SetItemText(const wxTreeItemId& item, const wxString& text)
{
...
void ibTreeCtrl::SetItemImage(const wxTreeItemId& item,
                              int image,
...
void
ibTreeCtrl::SetItemData(const wxTreeItemId& item, wxTreeItemData *data)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::DoSetItemState(const wxTreeItemId& item, int state)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::SetItemHasChildren(const wxTreeItemId& item, bool has)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::SetItemBold(const wxTreeItemId& item, bool bold)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::SetItemDropHighlight(const wxTreeItemId& item,
                                             bool highlight)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
    ...
    if (highlight)
    {
        bg = wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), HIGHLIGHT); // SEAM: theme
        fg = wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), HIGHLIGHT_TEXT); // SEAM: theme
    }
...
void ibTreeCtrl::SetItemTextColour(const wxTreeItemId& item,
                                   const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::SetItemBackgroundColour(const wxTreeItemId& item,
                                         const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void
ibTreeCtrl::SetItemFont(const wxTreeItemId& item, const wxFont& font)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
bool ibTreeCtrl::SetFont( const wxFont &font )
{
    if ( !ibTreeCtrlBase::SetFont(font) )
        return false;
...
bool ibTreeCtrl::IsVisible(const wxTreeItemId& item) const
{
    ...
    ibTreeItem *pItem = GetItemPtr(item);
    ibTreeItem* parent = pItem->GetParent();
    while (parent)
    {
...
bool ibTreeCtrl::ItemHasChildren(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
bool ibTreeCtrl::IsExpanded(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
bool ibTreeCtrl::IsSelected(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
bool ibTreeCtrl::IsBold(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetItemParent(const wxTreeItemId& item) const
{
...
wxTreeItemId ibTreeCtrl::GetFirstChild(const wxTreeItemId& item,
...
wxTreeItemId ibTreeCtrl::GetNextChild(const wxTreeItemId& item,
                                             wxTreeItemIdValue& cookie) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetLastChild(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetNextSibling(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );

    ibTreeItem *i = GetItemPtr(item);
    ibTreeItem *parent = i->GetParent();
    if ( parent == nullptr )
    {
...
wxTreeItemId ibTreeCtrl::GetPrevSibling(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetNext(const wxTreeItemId& item) const
{
    return DoGetNext(item, Next_Any);
}

wxTreeItemId
ibTreeCtrl::DoGetNext(const wxTreeItemId& item, int flags) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetFirstVisibleItem() const
{
...
wxTreeItemId ibTreeCtrl::GetNextVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId ibTreeCtrl::GetPrevVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
    wxASSERT_MSG( IsVisible(item), wxT("this item itself should be visible") );
...
void ibTreeCtrl::ResetTextControl()
{
    m_textCtrl = nullptr;
}

void ibTreeCtrl::ResetFindState()
{
    m_findPrefix.clear();
    if ( m_findBell == -1 )
        m_findBell = 1;
...
wxTreeItemId ibTreeCtrl::FindItem(const wxTreeItemId& idParent,
...
wxTreeItemId ibTreeCtrl::DoInsertItem(const wxTreeItemId& parentId,
                                             size_t previous,
                                             const wxString& text,
                                             int image,
                                             int selImage,
                                             wxTreeItemData *data)
{
    ibTreeItem *parent = GetItemPtr(parentId);
    if ( !parent )
    {
    ...
    ibTreeItem *item =
        new ibTreeItem( parent, text, image, selImage, data );
...
wxTreeItemId ibTreeCtrl::AddRoot(const wxString& text,
                                        int image,
                                        int selImage,
                                        wxTreeItemData *data)
{
    ...
    m_anchor = new ibTreeItem(nullptr, text,
                                   image, selImage, data);
    if ( data != nullptr )
    {
        data->SetId(m_anchor);
...
wxTreeItemId ibTreeCtrl::DoInsertAfter(const wxTreeItemId& parentId,
                                              ...
                                              wxTreeItemData *data)
{
    ...
    if (idPrevious.IsOk())
    {
        ...
        wxASSERT_MSG( index != wxNOT_FOUND,
                      "previous item in ibTreeCtrl::InsertItem() "
                      "is not a sibling" );
    }
...
void ibTreeCtrl::SendDeleteEvent(ibTreeItem *item)
{
    wxTreeEvent event(wxEVT_TREE_DELETE_ITEM, GetId()); event.SetEventObject(this); event.SetItem(item);
    GetEventHandler()->ProcessEvent( event );
}

// Don't leave edit or selection on a child which is about to disappear
void ibTreeCtrl::ChildrenClosing(ibTreeItem* item)
{
...
void ibTreeCtrl::DeleteChildren(const wxTreeItemId& itemId)
{
    m_dirty = true;     // do this first so stuff below doesn't cause flicker

    ibTreeItem *item = GetItemPtr(itemId);
    ChildrenClosing(item);
    item->DeleteChildren(this);
    InvalidateBestSize();
}

void ibTreeCtrl::Delete(const wxTreeItemId& itemId)
{
    m_dirty = true;     // do this first so stuff below doesn't cause flicker
    ...
    ibTreeItem *parent = item->GetParent();

    // if the selected item will be deleted, select the parent ...
    ibTreeItem *to_be_selected = parent;
    if (parent)
    {
...
void ibTreeCtrl::DeleteAllItems()
{
    if ( m_anchor )
    {
        Delete(m_anchor);
...
void ibTreeCtrl::Expand(const wxTreeItemId& itemId)
{
    ibTreeItem *item = GetItemPtr(itemId);

    wxCHECK_RET( item, wxT("invalid item in ibTreeCtrl::Expand") );
    ...
    wxTreeEvent event(wxEVT_TREE_ITEM_EXPANDING, GetId()); event.SetEventObject(this); event.SetItem(item);
...
void ibTreeCtrl::Collapse(const wxTreeItemId& itemId)
{
    wxCHECK_RET( !HasFlag(wxTR_HIDE_ROOT) || itemId != GetRootItem(),
    ...
    wxTreeEvent event(wxEVT_TREE_ITEM_COLLAPSING, GetId()); event.SetEventObject(this); event.SetItem(item);
    if ( GetEventHandler()->ProcessEvent( event ) && !event.IsAllowed() )
    {
        // cancelled by program
        return;
...
void ibTreeCtrl::CollapseAndReset(const wxTreeItemId& item)
{
    Collapse(item);
    DeleteChildren(item);
}

void ibTreeCtrl::Toggle(const wxTreeItemId& itemId)
{
    ibTreeItem *item = GetItemPtr(itemId);
...
void ibTreeCtrl::ClearFocusedItem()
{
...
void ibTreeCtrl::SetFocusedItem(const wxTreeItemId& item)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void ibTreeCtrl::UnselectAllChildren(ibTreeItem *item)
{
    if (item->IsSelected())
    {
...
void ibTreeCtrl::SelectChildren(const wxTreeItemId& parent)
{
    ...
    ibTreeItem * item = children[0];
    wxTreeEvent event(wxEVT_TREE_SEL_CHANGING, GetId()); event.SetEventObject(this); event.SetItem(item);
...
bool
ibTreeCtrl::TagNextChildren(ibTreeItem *crt_item,
                                   ibTreeItem *last_item,
                                   bool select)
{
    ibTreeItem *parent = crt_item->GetParent();
...
bool
ibTreeCtrl::TagAllChildrenUntilLast(ibTreeItem *crt_item,
                                           ibTreeItem *last_item,
                                           bool select)
{
...
void
ibTreeCtrl::SelectItemRange(ibTreeItem *item1,
                                   ibTreeItem *item2)
{
    m_select_me = nullptr;
    ...
    ibTreeItem *first= (item1->GetY()<item2->GetY()) ? item1 : item2;
    ibTreeItem *last = (item1->GetY()<item2->GetY()) ? item2 : item1;
...
void ibTreeCtrl::DoSelectItem(const wxTreeItemId& itemId,
                                     bool unselect_others,
                                     bool extended_select)
{
...
void ibTreeCtrl::SelectItem(const wxTreeItemId& itemId, bool select)
{
    ibTreeItem * const item = GetItemPtr(itemId);
...
void ibTreeCtrl::FillArray(ibTreeItem *item,
...
size_t ibTreeCtrl::GetSelections(wxArrayTreeItemIds &array) const
{
    array.Empty();
...
void ibTreeCtrl::EnsureVisible(const wxTreeItemId& item)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
    ...
    ibTreeItem *gitem = GetItemPtr(item);

    // first expand all parent branches
    ibTreeItem *parent = gitem->GetParent();
...
void ibTreeCtrl::ScrollTo(const wxTreeItemId &item)
{
    if (!item.IsOk())
        return;
...
void ibTreeCtrl::SortChildren(const wxTreeItemId& itemId)
{
    wxCHECK_RET( itemId.IsOk(), wxT("invalid tree item") );
    ...
    if ( children.size() > 1 )
    {
        ...
        std::sort(children.begin(), children.end(),
                  [this](ibTreeItem* a, ibTreeItem* b)
                  {
...
void ibTreeCtrl::CalculateLineHeight()
{
    wxInfoDC dc(this);
    ...
    if ( m_imagesState.HasImages() )
    {
        ...
        // May be toggle off. Then ibTreeCtrl will spread when
...
void ibTreeCtrl::OnImagesChanged()
{
...
void ibTreeCtrl::UpdateAfterImageListChange()
{
    m_dirty = true;
...
void ibTreeCtrl::SetImageList(wxImageList *imageList)
{
    wxWithImages::SetImageList(imageList);
    UpdateAfterImageListChange();
}

void ibTreeCtrl::SetStateImages(const wxVector<wxBitmapBundle>& images)
{
...
void ibTreeCtrl::SetStateImageList(wxImageList *imageList)
{
    m_imagesState.SetImageList(imageList);
    UpdateAfterImageListChange();
}

void ibTreeCtrl::SetButtonsImageList(wxImageList *imageList)
{
    m_imagesButtons.SetImageList(imageList);
    UpdateAfterImageListChange();
}

void ibTreeCtrl::AssignButtonsImageList(wxImageList *imageList)
{
...
void ibTreeCtrl::AdjustMyScrollbars()
{
    if (m_anchor)
    {
        int x = 0, y = 0;
...
int ibTreeCtrl::GetLineHeight(ibTreeItem *item) const
{
...
void ibTreeCtrl::PaintItem(ibTreeItem *item, wxDC& dc)
{
    ...
    if ( HasFlag(wxTR_FULL_ROW_HIGHLIGHT) )
    {
        ...
        else
        {
            ...
            // SEAM: the canon used the NATIVE wxRendererNative selection rect here
                // (system blue); the theme highlight brush is already selected into
                // the dc at the top of PaintItem — flat themed pad instead
                wxUnusedVar(flags);
...
void
ibTreeCtrl::PaintLevel(ibTreeItem *item,
                              wxDC &dc,
                              int level,
                              int &y)
{
    ...
    if (IsExposed(exposed_x, exposed_y, 10000, h))  // 10000 = very much
    {
        ...
        if ( item->IsSelected() )
        {
            ...
            if (m_hasFocus)
               colText = *wxWHITE;
#else
            colText = wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), HIGHLIGHT_TEXT); // SEAM: theme
#endif
        }
        else
        {
...
void ibTreeCtrl::DrawDropEffect(ibTreeItem *item)
{
    if ( item )
    {
        if ( item->HasPlus() )
...
void ibTreeCtrl::DrawBorder(const wxTreeItemId &item)
{
    wxCHECK_RET( item.IsOk(), "invalid item in ibTreeCtrl::DrawLine" );
...
void ibTreeCtrl::DrawLine(const wxTreeItemId &item, bool below)
{
    wxCHECK_RET( item.IsOk(), "invalid item in ibTreeCtrl::DrawLine" );
...
void ibTreeCtrl::OnSize( wxSizeEvent &event )
{
...
void ibTreeCtrl::OnPaint( wxPaintEvent &WXUNUSED(event) )
{
    // SEAM vs wx: ibWindow sets wxBG_STYLE_PAINT (no erase pass), so this
    // OnPaint owns the background too — buffered, otherwise the scrolled-in
    // areas keep stale pixels (duplicated rows) and the repaint flickers
...
void ibTreeCtrl::OnSetFocus( wxFocusEvent &event )
{
    m_hasFocus = true;
...
void ibTreeCtrl::OnKillFocus( wxFocusEvent &event )
{
    m_hasFocus = false;
...
void ibTreeCtrl::OnKeyDown( wxKeyEvent &event )
{
    // send a tree event
    wxTreeEvent te(wxEVT_TREE_KEY_DOWN, GetId()); te.SetEventObject(this);
...
void ibTreeCtrl::OnChar( wxKeyEvent &event )
{
    ...
    switch ( keyCode )
    {
        ...
        case WXK_MENU:
            {
                ...
                wxTreeEvent
                    eventMenu(wxEVT_TREE_ITEM_MENU, GetId());
                    eventMenu.SetEventObject(this);
                    eventMenu.SetItem(m_current);
                // Use the left edge, vertical middle
                eventMenu.SetPoint(wxPoint(ItemRect.GetX(),
                                                ItemRect.GetY() +
        ...
        case WXK_RETURN:
            ...
            {
                wxTreeEvent
                   eventAct(wxEVT_TREE_ITEM_ACTIVATED, GetId());
                   eventAct.SetEventObject(this);
                   eventAct.SetItem(m_current);
...
wxTreeItemId
ibTreeCtrl::DoTreeHitTest(const wxPoint& point, int& flags) const
{
    int w, h;
    GetSize(&w, &h);
    flags=0;
    ...
    ibTreeItem *hit =  m_anchor->HitTest(CalcUnscrolledPosition(point),
                                                this, flags, 0);
    if (hit == nullptr)
    {
        flags = wxTREE_HITTEST_NOWHERE;
...
bool ibTreeCtrl::GetBoundingRect(const wxTreeItemId& item,
                                        wxRect& rect,
                                        bool textOnly) const
{
    wxCHECK_MSG( item.IsOk(), false,
                 "invalid item in ibTreeCtrl::GetBoundingRect" );
...
ibTextCtrl *ibTreeCtrl::EditLabel(const wxTreeItemId& item,
                                  wxClassInfo * WXUNUSED(textCtrlClass))
{
    wxCHECK_MSG( item.IsOk(), nullptr, wxT("can't edit an invalid item") );

    ibTreeItem *itemEdit = GetItemPtr(item);

    wxTreeEvent te(wxEVT_TREE_BEGIN_LABEL_EDIT, GetId()); te.SetEventObject(this); te.SetItem(itemEdit);
    ...
    m_textCtrl = new ibTreeTextCtrl(this, itemEdit);
...
ibTextCtrl* ibTreeCtrl::GetEditControl() const
{
    return m_textCtrl;
}

void ibTreeCtrl::EndEditLabel(const wxTreeItemId& WXUNUSED(item),
                                     bool discardChanges)
{
    if (m_textCtrl)
    {
...
bool ibTreeCtrl::OnRenameAccept(ibTreeItem *item,
                                       const wxString& value)
{
    wxTreeEvent le(wxEVT_TREE_END_LABEL_EDIT, GetId()); le.SetEventObject(this); le.SetItem(item);
    le.SetLabel(value);
    le.SetEditCanceled(false);

    return !GetEventHandler()->ProcessEvent( le ) || le.IsAllowed();
}

void ibTreeCtrl::OnRenameCancelled(ibTreeItem *item)
{
...
void ibTreeCtrl::OnRenameTimer()
{
    EditLabel( m_current );
}

void ibTreeCtrl::OnMouse( wxMouseEvent &event )
{
    if ( !m_anchor )
        return;
    ...
    int flags = 0;
    ibTreeItem *thisItem = m_anchor->HitTest(pt, this, flags, 0);
    ibTreeItem *underMouse = thisItem;
#if wxUSE_TOOLTIPS
                ...
                (!m_renameTimer || !m_renameTimer->IsRunning()) )
    {
        ...
        wxTreeEvent
            hevent(wxEVT_TREE_ITEM_GETTOOLTIP, GetId());
    ...
    flags = 0;
    ibTreeItem *item = m_anchor->HitTest(pt, this, flags, 0);
    ...
    if ( event.Dragging() && !m_isDragging )
    {
        ...
        wxTreeEvent nevent(command, GetId()); nevent.SetEventObject(this); nevent.SetItem(m_current);
    ...
    else if ( (event.LeftUp() || event.RightUp()) && m_isDragging )
    {
        ...
        wxTreeEvent eventEndDrag(wxEVT_TREE_END_DRAG, GetId()); eventEndDrag.SetEventObject(this); eventEndDrag.SetItem(item);

        eventEndDrag.SetPoint(CalcScrolledPosition(pt));
    ...
    else
    {
        ...
        if ( event.RightDown() )
        {
            ...
            wxTreeEvent
                nevent(wxEVT_TREE_ITEM_RIGHT_CLICK, GetId());
            ...
            wxTreeEvent nevent2(wxEVT_TREE_ITEM_MENU, GetId()); nevent2.SetEventObject(this); nevent2.SetItem(item);
            nevent2.SetPoint(CalcScrolledPosition(pt));
            GetEventHandler()->ProcessEvent(nevent2);
        }
        else if ( event.MiddleDown() )
        {
            wxTreeEvent
                nevent(wxEVT_TREE_ITEM_MIDDLE_CLICK, GetId());
                nevent.SetEventObject(this);
                nevent.SetItem(item);
            nevent.SetPoint(CalcScrolledPosition(pt));
            event.Skip(!GetEventHandler()->ProcessEvent(nevent));
        ...
        else if ( event.LeftUp() )
        {
            ...
            {
                wxTreeEvent
                    nevent(wxEVT_TREE_STATE_IMAGE_CLICK, GetId());
                    nevent.SetEventObject(this);
                    nevent.SetItem(item);
            ...
            if ( m_lastOnSame )
            {
                     ...
                     HasFlag(wxTR_EDIT_LABELS) )
                {
                    ...
                    {
                        m_renameTimer = new ibTreeRenameTimer( this );
                    }

                    if ( canStartRenameTimer )
                    {
                        m_renameTimer->Start( ibTreeRenameTimer::DELAY, true );
                    }
                }
        ...
        else // !RightDown() && !MiddleDown() && !LeftUp()
        {
            ...
            if ( event.LeftDClick() )
            {
                ...
                wxTreeEvent
                    nevent(wxEVT_TREE_ITEM_ACTIVATED, GetId());
                    nevent.SetEventObject(this);
                    nevent.SetItem(item);
                nevent.SetPoint(CalcScrolledPosition(pt));
...
void ibTreeCtrl::OnInternalIdle()
{
...
void
ibTreeCtrl::CalculateLevel(ibTreeItem *item,
                                  wxReadOnlyDC &dc,
                                  int level,
                                  int &y )
{
...
void ibTreeCtrl::CalculatePositions()
{
    if ( !m_anchor )
        return;
...
void ibTreeCtrl::Refresh(bool eraseBackground, const wxRect *rect)
{
    if ( !IsFrozen() )
        ibTreeCtrlBase::Refresh(eraseBackground, rect);
}

void ibTreeCtrl::RefreshSubtree(ibTreeItem *item)
{
    if (m_dirty || IsFrozen() )
        return;
...
void ibTreeCtrl::RefreshLine( ibTreeItem *item )
{
    if (m_dirty || IsFrozen() )
        return;
...
void ibTreeCtrl::RefreshSelected()
{
    if (IsFrozen())
        return;
...
void ibTreeCtrl::RefreshSelectedUnder(ibTreeItem *item)
{
    if (IsFrozen())
        return;
...
void ibTreeCtrl::DoThaw()
{
    ibTreeCtrlBase::DoThaw();
...
bool ibTreeCtrl::SetBackgroundColour(const wxColour& colour)
{
...
bool ibTreeCtrl::SetForegroundColour(const wxColour& colour)
{
...
void ibTreeCtrl::OnGetToolTip( wxTreeEvent &event )
{
#if wxUSE_TOOLTIPS
    wxTreeItemId itemId = event.GetItem();
    const ibTreeItem* const pItem = GetItemPtr(itemId);
...
wxVisualAttributes
#if _USE_VISATTR
ibTreeCtrl::GetClassDefaultAttributes(wxWindowVariant variant)
#else
ibTreeCtrl::GetClassDefaultAttributes(wxWindowVariant WXUNUSED(variant))
#endif
{
#if _USE_VISATTR
    ...
    attr.colFg = wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), CONTROL_TEXT); // SEAM: theme
    attr.colBg = wxSCHEME_COLOUR(ibThemeEngine::Get()->GetColourScheme(), WINDOW); // SEAM: theme
...
void ibTreeCtrl::DoDirtyProcessing()
{
    if (IsFrozen())
        return;
...
wxSize ibTreeCtrl::DoGetBestSize() const
{
    ...
    // correct result
    wxConstCast(this, ibTreeCtrl)->CalculatePositions();

    wxSize size = ibTreeCtrlBase::DoGetBestSize();
