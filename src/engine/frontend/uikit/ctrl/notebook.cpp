...
// Purpose:     ibNotebook implementation
...
// ----------------------------------------------------------------------------
// ibStdNotebookInputHandler: translates SPACE and ENTER keys and the left mouse
// click into button press/release actions
// ----------------------------------------------------------------------------

class FRONTEND_API ibStdNotebookInputHandler : public ibStdInputHandler
{
public:
    ibStdNotebookInputHandler(ibInputHandler *inphand);
...
protected:
    void HandleFocusChange(ibInputConsumer *consumer);
};
...
class ibNotebookSpinBtn : public ibSpinButton
{
public:
    ibNotebookSpinBtn(ibNotebook *nb)
        : ibSpinButton(nb, wxID_ANY,
...
protected:
    ...
    {
        m_nb->PerformAction(ibACTION_NOTEBOOK_GOTO, event.GetPosition());
    }
...
wxBEGIN_EVENT_TABLE(ibNotebookSpinBtn, ibSpinButton)
    EVT_SPIN(wxID_ANY, ibNotebookSpinBtn::OnSpin)
wxEND_EVENT_TABLE()
...
bool ibNotebook::Create(wxWindow *parent,
                        wxWindowID id,
                        const wxPoint& pos,
                        const wxSize& size,
                        long style,
                        const wxString& name)
{
    ...
    CreateInputHandler(ibINP_HANDLER_NOTEBOOK);

    return true;
}

// ----------------------------------------------------------------------------
// ibNotebook page titles and images
// ----------------------------------------------------------------------------

wxString ibNotebook::GetPageText(size_t nPage) const
{
...
bool ibNotebook::SetPageText(size_t nPage, const wxString& strText)
{
    wxCHECK_MSG( IS_VALID_PAGE(nPage), false, wxT("invalid notebook page") );

    if ( strText != m_titles[nPage] )
    {
        m_accels[nPage] = wxControlBase::FindAccelIndex(strText, &m_titles[nPage]);
...
int ibNotebook::GetPageImage(size_t nPage) const
{
...
bool ibNotebook::SetPageImage(size_t nPage, int nImage)
{
    wxCHECK_MSG( IS_VALID_PAGE(nPage), false, wxT("invalid notebook page") );
...
ibNotebook::~ibNotebook()
{
}

// ----------------------------------------------------------------------------
// ibNotebook page switching
// ----------------------------------------------------------------------------

int ibNotebook::DoSetSelection(size_t nPage, int flags)
{
    wxCHECK_MSG( IS_VALID_PAGE(nPage), wxNOT_FOUND, wxT("invalid notebook page") );
...
// ----------------------------------------------------------------------------
// ibNotebook pages adding/deleting
// ----------------------------------------------------------------------------

bool ibNotebook::InsertPage(size_t nPage,
                            ...
                            int imageId)
{
    ...
    wxString label;
    m_accels.Insert(wxControlBase::FindAccelIndex(strText, &label), nPage);
...
bool ibNotebook::DeleteAllPages()
{
    // SEAM: the base page destruction now lives here
    while ( !m_pages.empty() )
    {
        wxWindow* page = m_pages.back();
        m_pages.pop_back();
        page->Destroy();
    }
...
wxNotebookPage *ibNotebook::DoRemovePage(size_t nPage)
{
...
void ibNotebook::RefreshCurrent()
{
...
void ibNotebook::RefreshTab(int page, bool forceSelected)
{
...
void ibNotebook::RefreshAllTabs()
{
...
void ibNotebook::DoDrawTab(wxDC& dc, const wxRect& rect, size_t n)
{
    wxBitmap bmp;
    if ( HasImage(n) )
    {
...
void ibNotebook::DoDraw(ibControlRenderer *renderer)
{
...
int ibNotebook::HitTest(const wxPoint& pt, long *flags) const
{
    if ( flags )
...
bool ibNotebook::IsVertical() const
{
...
wxRect ibNotebook::GetTabRect(size_t page) const
{
    wxRect rect;
...
wxRect ibNotebook::GetAllTabsRect() const
{
    wxRect rect;
...
wxRect ibNotebook::GetTabsPart() const
{
    wxRect rect = GetAllTabsRect();
...
void ibNotebook::GetTabSize(int page, wxCoord *w, wxCoord *h) const
{
...
void ibNotebook::SetTabSize(const wxSize& sz)
{
...
wxSize ibNotebook::CalcTabSize(int page) const
{
...
void ibNotebook::ResizeTab(int page)
{
...
void ibNotebook::SetPadding(const wxSize& padding)
{
...
wxRect ibNotebook::GetPagePart() const
{
...
wxRect ibNotebook::GetPageRect() const
{
...
wxSize ibNotebook::GetSizeForPage(const wxSize& size) const
{
    wxSize sizeNb = size;
    wxRect rect = GetAllTabsRect();
    if ( IsVertical() )
...
void ibNotebook::SetPageSize(const wxSize& size)
{
    SetClientSize(GetSizeForPage(size));
}

wxSize ibNotebook::CalcSizeFromPage(const wxSize& sizePage) const
{
    return AdjustSize(GetSizeForPage(sizePage));
}

// ----------------------------------------------------------------------------
// ibNotebook spin button
// ----------------------------------------------------------------------------

bool ibNotebook::HasSpinBtn() const
{
    return m_spinbtn && m_spinbtn->IsShown();
}

void ibNotebook::CalcLastVisibleTab()
{
    bool isVertical = IsVertical();
...
void ibNotebook::UpdateSpinBtn()
{
    ...
    if ( !allTabsShown )
    {
        ...
        {
            // create it once only
            m_spinbtn = new ibNotebookSpinBtn(this);
...
void ibNotebook::PositionSpinBtn()
{
    if ( !m_spinbtn )
        return;
...
// ----------------------------------------------------------------------------
// ibNotebook scrolling
// ----------------------------------------------------------------------------

void ibNotebook::ScrollTo(size_t page)
{
    wxCHECK_RET( IS_VALID_PAGE(page), wxT("invalid notebook page") );
...
void ibNotebook::ScrollLastTo(size_t page)
{
    wxCHECK_RET( IS_VALID_PAGE(page), wxT("invalid notebook page") );
...
// ----------------------------------------------------------------------------
// ibNotebook sizing/moving
// ----------------------------------------------------------------------------

void ibNotebook::DoMoveWindow(int x, int y, int width, int height)
{
    ibControl::DoMoveWindow(x, y, width, height);

    // move the spin ctrl too (NOP if it doesn't exist)
    PositionSpinBtn();
}

void ibNotebook::DoSetSize(int x, int y,
...
// ----------------------------------------------------------------------------
// ibNotebook input processing
// ----------------------------------------------------------------------------

bool ibNotebook::PerformAction(const ibControlAction& action,
                               long numArg,
                               const wxString& strArg)
{
    if ( action == ibACTION_NOTEBOOK_NEXT )
        SetSelection(GetNextPage(true));
    else if ( action == ibACTION_NOTEBOOK_PREV )
        SetSelection(GetNextPage(false));
    else if ( action == ibACTION_NOTEBOOK_GOTO )
...
/* static */
ibInputHandler *ibNotebook::GetStdInputHandler(ibInputHandler *handlerDef)
{
    static ibStdNotebookInputHandler s_handler(handlerDef);

    return &s_handler;
}

// ----------------------------------------------------------------------------
// ibStdNotebookInputHandler
// ----------------------------------------------------------------------------

ibStdNotebookInputHandler::ibStdNotebookInputHandler(ibInputHandler *inphand)
                         : ibStdInputHandler(inphand)
{
}

bool ibStdNotebookInputHandler::HandleKey(ibInputConsumer *consumer,
                                          const wxKeyEvent& event,
                                          bool pressed)
{
    ...
    if ( pressed )
    {
        ibNotebook *notebook = wxStaticCast(consumer->GetInputWindow(), ibNotebook);
...
bool ibStdNotebookInputHandler::HandleMouse(ibInputConsumer *consumer,
                                            const wxMouseEvent& event)
{
    if ( event.ButtonDown(1) )
    {
        ...
        if ( page != -1 )
        {
            consumer->PerformAction(ibACTION_NOTEBOOK_GOTO, page);
...
bool ibStdNotebookInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                                const wxMouseEvent& event)
{
...
bool
ibStdNotebookInputHandler::HandleFocus(ibInputConsumer *consumer,
...
bool ibStdNotebookInputHandler::HandleActivation(ibInputConsumer *consumer,
...
void ibStdNotebookInputHandler::HandleFocusChange(ibInputConsumer *consumer)
{
    ibNotebook *notebook = wxStaticCast(consumer->GetInputWindow(), ibNotebook);
    notebook->RefreshCurrent();
}

wxIMPLEMENT_DYNAMIC_CLASS(ibNotebook, ibControl);
