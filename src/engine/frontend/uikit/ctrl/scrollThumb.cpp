...
// Purpose:     ibScrollThumb and related classes
...
#if wxUSE_SCROLLBAR
// ----------------------------------------------------------------------------
// ibScrollThumbCaptureData: the struct used while dragging the scroll thumb
// ----------------------------------------------------------------------------

struct FRONTEND_API ibScrollThumbCaptureData
{
    // start mouse capture after the user clicked the mouse button btn on this
    // part of the control
    ibScrollThumbCaptureData(ibScrollThumb::Shaft part,
                             int btn,
                             ibControlWithThumb *control)
    {
        m_shaftPart = part;
        m_btnCapture = btn;
    ...
    ibScrollThumb::Shaft m_shaftPart;
...
// ----------------------------------------------------------------------------
// ibScrollTimer: the timer used when the arrow is kept pressed
// ----------------------------------------------------------------------------

#if wxUSE_TIMER

class ibScrollThumbTimer : public ibScrollTimer
{
public:
    ibScrollThumbTimer(ibControlWithThumb *control,
                       ibScrollThumb::Shaft shaftPart)
    {
        m_control = control;
        switch ( shaftPart )
        {
            case ibScrollThumb::Shaft_Above:
                m_inc = -1;
                break;

            default:
                wxFAIL_MSG(wxT("unexpected shaft part in ibScrollThumbTimer"));
                // fall through

            case ibScrollThumb::Shaft_Below:
                m_inc = 1;
                break;
        }
...
// ----------------------------------------------------------------------------
// ibScrollThumb constructor and dtor
// ----------------------------------------------------------------------------

ibScrollThumb::ibScrollThumb(ibControlWithThumb *control)
{
...
ibScrollThumb::~ibScrollThumb()
{
    // make sure the mouse capture data will be released
    // when destroy the thumb.
    delete m_captureData;
    wxConstCast(this, ibScrollThumb)->m_captureData = nullptr;
}

// ----------------------------------------------------------------------------
// ibScrollThumb mouse handling
// ----------------------------------------------------------------------------

bool ibScrollThumb::HandleMouse(const wxMouseEvent& event) const
{
    ...
    if ( event.ButtonDown() || event.ButtonDClick() )
    {
        ...
        // capture the mouse
        wxConstCast(this, ibScrollThumb)->m_captureData =
            new ibScrollThumbCaptureData(shaftPart, btn, m_control);
        ...
        else // not the thumb
        {
            ...
            m_captureData->m_timerScroll =
                new ibScrollThumbTimer(m_control, shaftPart);
        }
#endif // wxUSE_TIMER
    }
...
bool ibScrollThumb::HandleMouseMove(const wxMouseEvent& event) const
{
    if ( HasCapture() )
    {
    ...
    else // no capture
    {
        ...
        if ( shaftPart != m_shaftPart )
        {
            ...
            wxConstCast(this, ibScrollThumb)->m_shaftPart = shaftPart;
...
wxCoord ibScrollThumb::GetMouseCoord(const wxMouseEvent& event) const
{
    wxPoint pt = event.GetPosition();
    return m_control->IsVertical() ? pt.y : pt.x;
}

int ibScrollThumb::GetThumbPos(const wxMouseEvent& event) const
{
