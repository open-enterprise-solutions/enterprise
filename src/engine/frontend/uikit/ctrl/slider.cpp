...
// Purpose:     implementation of the universal version of ibSlider
...
// ----------------------------------------------------------------------------
// ibStdSliderInputHandler: default slider input handling
// ----------------------------------------------------------------------------

class FRONTEND_API ibStdSliderInputHandler : public ibStdInputHandler
{
public:
    // default ctor
    ibStdSliderInputHandler(ibInputHandler *inphand)
...
// ============================================================================
// implementation of ibSlider
// ============================================================================

wxBEGIN_EVENT_TABLE(ibSlider, ibControl)
    EVT_SIZE(ibSlider::OnSize)
wxEND_EVENT_TABLE()
...
ibSlider::ibSlider(wxWindow *parent,
                   wxWindowID id,
...
bool ibSlider::Create(wxWindow *parent,
                      wxWindowID id,
                      int value, int minValue, int maxValue,
                      const wxPoint& pos,
                      const wxSize& size,
                      ...
                      const wxString& name)
{
    ...
    CreateInputHandler(ibINP_HANDLER_SLIDER);

    return true;
}

// ----------------------------------------------------------------------------
// ibSlider range and value
// ----------------------------------------------------------------------------

int ibSlider::GetValue() const
{
    return m_value;
}

int ibSlider::NormalizeValue(int value) const
{
    if ( value < m_min )
        return m_min;
    else if ( value > m_max )
...
bool ibSlider::ChangeValueBy(int inc)
{
    return ChangeValueTo(NormalizeValue(m_value + inc));
}

bool ibSlider::ChangeValueTo(int value)
{
...
void ibSlider::SetValue(int value)
{
...
void ibSlider::SetRange(int minValue, int maxValue)
{
...
int ibSlider::GetMin() const
{
    return m_min;
}

int ibSlider::GetMax() const
{
    return m_max;
}

// ----------------------------------------------------------------------------
// ibSlider line/page/thumb size
// ----------------------------------------------------------------------------

void ibSlider::SetLineSize(int lineSize)
{
...
void ibSlider::SetPageSize(int pageSize)
{
...
int ibSlider::GetLineSize() const
{
    if ( !m_lineSize )
    {
        // the default line increment is 1
        wxConstCast(this, ibSlider)->m_lineSize = 1;
    }

    return m_lineSize;
}

int ibSlider::GetPageSize() const
{
    if ( !m_pageSize )
    {
        // the default page increment is m_tickFreq
        wxConstCast(this, ibSlider)->m_pageSize = m_tickFreq;
    }

    return m_pageSize;
}

void ibSlider::SetThumbLength(int lenPixels)
{
...
int ibSlider::GetThumbLength() const
{
...
void ibSlider::DoSetTickFreq(int n)
{
...
wxSize ibSlider::CalcLabelSize() const
{
    wxSize size;
...
wxSize ibSlider::DoGetBestClientSize() const
{
...
void ibSlider::OnSize(wxSizeEvent& event)
{
    CalcGeometry();
...
const wxRect& ibSlider::GetSliderRect() const
{
    if ( m_rectSlider.width < 0 )
    {
        wxConstCast(this, ibSlider)->CalcGeometry();
    }

    return m_rectSlider;
}

void ibSlider::CalcGeometry()
{
    /*
...
wxString ibSlider::FormatValue(int value) const
{
    return wxString::Format(wxT("%d"), value);
}

void ibSlider::DoDraw(ibControlRenderer *renderer)
{
    ibRenderer *rend = GetRenderer();
...
// ----------------------------------------------------------------------------
// ibSlider input processing
// ----------------------------------------------------------------------------

bool ibSlider::PerformAction(const ibControlAction& action,
                             long numArg,
                             const wxString& strArg)
{
    ...
    if ( action == ibACTION_SLIDER_START )
    {
        scrollEvent = wxEVT_SCROLL_TOP;
        value = m_min;
    }
    else if ( action == ibACTION_SLIDER_END )
    {
        scrollEvent = wxEVT_SCROLL_BOTTOM;
        value = m_max;
    }
    else if ( action == ibACTION_SLIDER_PAGE_CHANGE )
    {
        scrollEvent = wxEVT_SCROLL_CHANGED;
        value = NormalizeValue(m_value + numArg * GetPageSize());
    }
    else if ( action == ibACTION_SLIDER_LINE_UP )
    {
        scrollEvent = wxEVT_SCROLL_LINEUP;
        value = NormalizeValue(m_value + -GetLineSize());
    }
    else if ( action == ibACTION_SLIDER_LINE_DOWN )
    {
        scrollEvent = wxEVT_SCROLL_LINEDOWN;
        value = NormalizeValue(m_value + +GetLineSize());
    }
    else if ( action == ibACTION_SLIDER_PAGE_UP )
    {
        scrollEvent = wxEVT_SCROLL_PAGEUP;
        value = NormalizeValue(m_value + -GetPageSize());
    }
    else if ( action == ibACTION_SLIDER_PAGE_DOWN )
    {
        scrollEvent = wxEVT_SCROLL_PAGEDOWN;
        value = NormalizeValue(m_value + +GetPageSize());
    }
    else if ( action == ibACTION_SLIDER_THUMB_DRAG ||
                action == ibACTION_SLIDER_THUMB_MOVE )
    {
        scrollEvent = wxEVT_SCROLL_THUMBTRACK;
        value = (int)numArg;
        Refresh();
    }
    else if ( action == ibACTION_SLIDER_THUMB_RELEASE )
    {
...
/* static */
ibInputHandler *ibSlider::GetStdInputHandler(ibInputHandler *handlerDef)
{
    static ibStdSliderInputHandler s_handler(handlerDef);

    return &s_handler;
}

// ----------------------------------------------------------------------------
// ibSlider implementation of ibControlWithThumb interface
// ----------------------------------------------------------------------------

ibScrollThumb::Shaft ibSlider::HitTest(const wxPoint& pt) const
{
    ...
    {
        return ibScrollThumb::Shaft_None;
    }
    ...
    {
        // or to the left
        return ibScrollThumb::Shaft_Above;
    }

    if ((x3 < x) && (x <= x4)) {
        // or to the right
        return ibScrollThumb::Shaft_Below;
    }

    // where else can it be?
    return ibScrollThumb::Shaft_Thumb;
}

wxCoord ibSlider::ThumbPosToPixel() const
{
    wxRect rectThumb;
...
int ibSlider::PixelToThumbPos(wxCoord x) const
{
    wxRect rectShaft = GetShaftRect();
    wxSize sizeThumb = GetThumbSize();
...
void ibSlider::SetShaftPartState(ibScrollThumb::Shaft shaftPart,
                                 int flag,
                                 bool set)
{
    // for now we ignore the flags for the shaft as no renderer uses them
    // anyhow
    if ( shaftPart == ibScrollThumb::Shaft_Thumb )
    {
        if ( set )
            m_thumbFlags |= flag;
        else
...
void ibSlider::OnThumbDragStart(int pos)
{
    if (IsVert())
    {
        PerformAction(ibACTION_SLIDER_THUMB_DRAG, m_max - pos);
    }
    else
    {
        PerformAction(ibACTION_SLIDER_THUMB_DRAG, pos);
    }
}

void ibSlider::OnThumbDrag(int pos)
{
    if (IsVert())
    {
        PerformAction(ibACTION_SLIDER_THUMB_MOVE, m_max - pos);
    }
    else
    {
        PerformAction(ibACTION_SLIDER_THUMB_MOVE, pos);
    }
}

void ibSlider::OnThumbDragEnd(int pos)
{
    if (IsVert())
    {
        PerformAction(ibACTION_SLIDER_THUMB_RELEASE, m_max - pos);
    }
    else
    {
        PerformAction(ibACTION_SLIDER_THUMB_RELEASE, pos);
    }
}

void ibSlider::OnPageScrollStart()
{
    // we do nothing here
}

bool ibSlider::OnPageScroll(int pageInc)
{
    int value = GetValue();
    PerformAction(ibACTION_SLIDER_PAGE_CHANGE, pageInc);

    return GetValue() != value;
}

// ----------------------------------------------------------------------------
// ibStdSliderInputHandler
// ----------------------------------------------------------------------------

bool ibStdSliderInputHandler::HandleKey(ibInputConsumer *consumer,
                                              const wxKeyEvent& event,
                                              bool pressed)
{
    if ( pressed )
    {
        ...
        {
            case WXK_HOME:
                action = ibACTION_SLIDER_END;
                break;

            case WXK_END:
                action = ibACTION_SLIDER_START;
                break;

            case WXK_LEFT:
            case WXK_UP:
                action = ibACTION_SLIDER_LINE_UP;
                break;

            case WXK_RIGHT:
            case WXK_DOWN:
                action = ibACTION_SLIDER_LINE_DOWN;
                break;

            case WXK_PAGEUP:
                action = ibACTION_SLIDER_PAGE_UP;
                break;

            case WXK_PAGEDOWN:
                action = ibACTION_SLIDER_PAGE_DOWN;
                break;
        }
...
bool ibStdSliderInputHandler::HandleMouse(ibInputConsumer *consumer,
                                                const wxMouseEvent& event)
{
    ibSlider *slider = wxStaticCast(consumer->GetInputWindow(), ibSlider);
...
bool ibStdSliderInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                                    const wxMouseEvent& event)
{
    ibSlider *slider = wxStaticCast(consumer->GetInputWindow(), ibSlider);
...
bool
ibStdSliderInputHandler::HandleFocus(ibInputConsumer * WXUNUSED(consumer),
...
wxIMPLEMENT_DYNAMIC_CLASS(ibSlider, ibControl);
