...
// Purpose:     implementation of ibToolBar for wxUniversal
...
// ----------------------------------------------------------------------------
// ibStdToolbarInputHandler: translates SPACE and ENTER keys and the left mouse
// click into button press/release actions
// ----------------------------------------------------------------------------

class FRONTEND_API ibStdToolbarInputHandler : public ibStdInputHandler
{
public:
    ibStdToolbarInputHandler(ibInputHandler *inphand);
...
// ----------------------------------------------------------------------------
// ibToolBarTool: our implementation of wxToolBarToolBase
// ----------------------------------------------------------------------------

class FRONTEND_API ibToolBarTool : public wxToolBarToolBase
{
public:
    ibToolBarTool(ibToolBar *tbar,
                  int id,
        ...
        : wxToolBarToolBase(nullptr /* SEAM: not a wxToolBarBase */, id, label,
                            bmpNormal, bmpDisabled, kind,
    ...
    ibToolBarTool(ibToolBar *tbar, ibControl *control, const wxString& label)
        : wxToolBarToolBase(nullptr /* SEAM */, nullptr, label)
    {
        m_ibControl = control;
        wxUnusedVar(tbar);
        // no position yet
    ...
    // SEAM: the uikit control of an AddControl() tool (hides the base
    // wxControl accessor which is always null here)
    ibControl *GetControl() const { return m_ibControl; }
...
private:
    ...
    // SEAM: see GetControl() above
    ibControl *m_ibControl;
};

// ============================================================================
// ibToolBar implementation
// ============================================================================


// ----------------------------------------------------------------------------
// wxToolBarBase shims: the public tool-adding API (lived in the lost base)
// ----------------------------------------------------------------------------

wxToolBarToolBase *ibToolBar::InsertTool(size_t pos, int id, const wxString& label,
        const wxBitmapBundle& bitmap, const wxBitmapBundle& bmpDisabled,
        wxItemKind kind, const wxString& shortHelp)
{
    wxToolBarToolBase *tool = CreateTool(id, label, bitmap, bmpDisabled,
        kind, nullptr, shortHelp, wxEmptyString);
    if ( !DoInsertTool(pos, tool) )
    {
        delete tool;
        return nullptr;
    }
...
wxToolBarToolBase *ibToolBar::AddTool(int id, const wxString& label,
        const wxBitmapBundle& bitmap, const wxString& shortHelp, wxItemKind kind)
{
    return InsertTool(GetToolsCount(), id, label, bitmap,
        wxBitmapBundle(), kind, shortHelp);
}

wxToolBarToolBase *ibToolBar::AddCheckTool(int id, const wxString& label,
        const wxBitmapBundle& bitmap, const wxBitmapBundle& bmpDisabled,
        const wxString& shortHelp)
{
    return InsertTool(GetToolsCount(), id, label, bitmap,
        bmpDisabled, wxITEM_CHECK, shortHelp);
}

wxToolBarToolBase *ibToolBar::AddSeparator()
{
    return InsertTool(GetToolsCount(), wxID_SEPARATOR, wxEmptyString,
        wxBitmapBundle(), wxBitmapBundle(), wxITEM_SEPARATOR);
}
wxIMPLEMENT_DYNAMIC_CLASS(ibToolBar, ibControl);
...
bool ibToolBar::Create(wxWindow *parent,
                       wxWindowID id,
                       const wxPoint& pos,
                       const wxSize& size,
                       long style,
                       const wxString& name)
{
    ...
    // FixupStyle(); — SEAM: wxToolBarBase style normalisation, not needed

    CreateInputHandler(ibINP_HANDLER_TOOLBAR);
...
void ibToolBar::SetMargins(int x, int y)
{
    // This required for similar visual effects under
    // native platforms and wxUniv.
    // SEAM: the univ original delegated to wxToolBarBase::SetMargins —
    // the storage lives here now (a bare self-call would recurse forever)
    m_xMargin = x + 3;
    m_yMargin = y + 3;
}

// ----------------------------------------------------------------------------
// ibToolBar tool-related methods
// ----------------------------------------------------------------------------

wxToolBarToolBase *ibToolBar::FindToolForPosition(wxCoord x, wxCoord y) const
{
...
void ibToolBar::SetToolShortHelp(int id, const wxString& help)
{
...
bool ibToolBar::DoInsertTool(size_t WXUNUSED(pos),
...
bool ibToolBar::DoDeleteTool(size_t WXUNUSED(pos),
                             wxToolBarToolBase * WXUNUSED(tool))
{
    // as above
    m_needsLayout = true;
...
void ibToolBar::DoEnableTool(wxToolBarToolBase *tool, bool enable)
{
#if wxUSE_IMAGE
...
void ibToolBar::DoToggleTool(wxToolBarToolBase *tool, bool WXUNUSED(toggle))
{
...
void ibToolBar::DoSetToggle(wxToolBarToolBase *tool, bool WXUNUSED(toggle))
{
    RefreshTool(tool);
}

wxToolBarToolBase *ibToolBar::CreateTool(int id,
                                         const wxString& label,
                                         const wxBitmapBundle& bmpNormal,
                                         const wxBitmapBundle& bmpDisabled,
                                         wxItemKind kind,
                                         ...
                                         const wxString& longHelp)
{
    return new ibToolBarTool(this, id, label, bmpNormal, bmpDisabled, kind,
                             clientData, shortHelp, longHelp);
}

wxToolBarToolBase *
ibToolBar::CreateTool(ibControl *control, const wxString& label)
{
    return new ibToolBarTool(this, control, label);
}
...
wxRect ibToolBar::GetToolRect(wxToolBarToolBase *toolBase) const
{
    const ibToolBarTool *tool = (ibToolBarTool *)toolBase;
    ...
    if ( m_needsLayout )
    {
        wxConstCast(this, ibToolBar)->DoLayout();
    }
...
bool ibToolBar::Realize()
{
    // SEAM: no wxToolBarBase::Realize — nothing to pre-realize
    ...
    for ( wxToolBarToolsList::compatibility_iterator node = m_tools.GetFirst();
          ...
          node = node->GetNext() )
    {
        ibToolBarTool *tool = (ibToolBarTool*) node->GetData();
...
void ibToolBar::SetWindowStyleFlag( long style )
{
    ibControl::SetWindowStyleFlag(style);
...
void ibToolBar::DoLayout()
{
          ...
          node = node->GetNext() )
    {
        ...
        {
            ibControl *control = tool->GetControl();
...
wxSize ibToolBar::DoGetBestClientSize() const
{
...
void ibToolBar::RefreshTool(wxToolBarToolBase *tool)
{
    RefreshRect(GetToolRect(tool));
}

void ibToolBar::GetRectLimits(const wxRect& rect,
                              wxCoord *start,
                              wxCoord *end) const
{
...
void ibToolBar::DoDraw(ibControlRenderer *renderer)
{
...
bool ibToolBar::PerformAction(const ibControlAction& action,
                              long numArg,
                              const wxString& strArg)
{
    ibToolBarTool *tool = (ibToolBarTool*) FindById(numArg);
    if (!tool)
        return false;

    if ( action == ibACTION_TOOLBAR_TOGGLE )
    {
        PerformAction( ibACTION_BUTTON_RELEASE, numArg );

        PerformAction( ibACTION_BUTTON_CLICK, numArg );

        // Set mouse leave toolbar button range (If still in the range,
        // toolbar button would get focus again
        PerformAction( ibACTION_TOOLBAR_LEAVE, numArg );
    }
    else if ( action == ibACTION_TOOLBAR_PRESS )
    {
    ...
    else if ( action == ibACTION_TOOLBAR_RELEASE )
    {
    ...
    else if ( action == ibACTION_TOOLBAR_CLICK )
    {
        bool isToggled;
    ...
    else if ( action == ibACTION_TOOLBAR_ENTER )
    {
    ...
    else if ( action == ibACTION_TOOLBAR_LEAVE )
    {
...
/* static */
ibInputHandler *ibToolBar::GetStdInputHandler(ibInputHandler *handlerDef)
{
    static ibStdToolbarInputHandler s_handler(handlerDef);

    return &s_handler;
}

// ============================================================================
// ibStdToolbarInputHandler implementation
// ============================================================================

ibStdToolbarInputHandler::ibStdToolbarInputHandler(ibInputHandler *handler)
...
bool ibStdToolbarInputHandler::HandleKey(ibInputConsumer *consumer,
                                         const wxKeyEvent& event,
                                         bool pressed)
{
...
bool ibStdToolbarInputHandler::HandleMouse(ibInputConsumer *consumer,
                                           const wxMouseEvent& event)
{
    ibToolBar *tbar = wxStaticCast(consumer->GetInputWindow(), ibToolBar);
    ...
    if ( event.Button(1) )
    {
        ...
        if ( event.LeftDown() || event.LeftDClick() )
        {
            ...
            consumer->PerformAction( ibACTION_BUTTON_PRESS, tool->GetId() );
        ...
        else if ( event.LeftUp() )
        {
            ...
            if (m_toolCapture)
            {
                if ( tool == m_toolCapture )
                    consumer->PerformAction( ibACTION_BUTTON_TOGGLE, m_toolCapture->GetId() );
                else
                    consumer->PerformAction( ibACTION_TOOLBAR_LEAVE, m_toolCapture->GetId() );
            }
...
bool ibStdToolbarInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                               const wxMouseEvent& event)
{
    if ( !ibStdInputHandler::HandleMouseMove(consumer, event) )
    {
        ibToolBar *tbar = wxStaticCast(consumer->GetInputWindow(), ibToolBar);
        ...
        {
            tool = (ibToolBarTool*) tbar->FindToolForPosition( event.GetX(), event.GetY() );
        }
        ...
        if (m_toolCapture)
        {
            ...
            if (tool)
                consumer->PerformAction( ibACTION_BUTTON_PRESS, m_toolCapture->GetId() );
            else
                consumer->PerformAction( ibACTION_BUTTON_RELEASE, m_toolCapture->GetId() );
        ...
        else
        {
            ...
            if (m_toolLast)
            {
                ...
                consumer->PerformAction( ibACTION_TOOLBAR_LEAVE, m_toolLast->GetId() );
            }

            if (tool)
            {
                // Enter new tool if any
                consumer->PerformAction( ibACTION_TOOLBAR_ENTER, tool->GetId() );
            }
...
bool ibStdToolbarInputHandler::HandleFocus(ibInputConsumer *consumer,
...
bool ibStdToolbarInputHandler::HandleActivation(ibInputConsumer *consumer,
                                                bool activated)
{
