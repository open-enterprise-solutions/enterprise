...
// Purpose:     ibScrollBar implementation
...
// ----------------------------------------------------------------------------
// ibScrollBarTimer: this class is used to repeatedly scroll the scrollbar
// when the mouse is help pressed on the arrow or on the bar. It generates the
// given scroll action command periodically.
// ----------------------------------------------------------------------------

class ibScrollBarTimer : public ibScrollTimer
{
public:
    ibScrollBarTimer(ibStdScrollBarInputHandler *handler,
...
private:
    ibStdScrollBarInputHandler *m_handler;
    ibControlAction m_action;
    ibScrollBar    *m_control;
};
...
wxBEGIN_EVENT_TABLE(ibScrollBar, ibControl)
wxEND_EVENT_TABLE()
...
ibScrollBar::ibScrollBar()
           : m_arrows(this)
{
    Init();
}

ibScrollBar::ibScrollBar(wxWindow *parent,
                         wxWindowID id,
                         const wxPoint& pos,
                         const wxSize& size,
                         long style,
...
bool ibScrollBar::Create(wxWindow *parent,
                         wxWindowID id,
                         const wxPoint &pos,
                         const wxSize &size,
                         long style,
                         ...
                         const wxString &name)
{
    ...
    CreateInputHandler(ibINP_HANDLER_SCROLLBAR);
...
bool ibScrollBar::IsStandalone() const
{
    ...
    // SEAM vs univ: a native parent has no univ scrollbars at all
    ibWindow *parentUniv = dynamic_cast<ibWindow*>(parent);
    if ( !parentUniv )
        return true;

    return (parentUniv->GetScrollbar(wxHORIZONTAL) != this) &&
           (parentUniv->GetScrollbar(wxVERTICAL) != this);
}

bool ibScrollBar::AcceptsFocus() const
{
    // the window scrollbars never accept focus
    return ibControl::AcceptsFocus() && IsStandalone();
}
...
void ibScrollBar::DoSetThumb(int pos)
{
...
int ibScrollBar::GetThumbPosition() const
{
    return m_thumbPos;
}

int ibScrollBar::GetThumbSize() const
{
    return m_thumbSize;
}

int ibScrollBar::GetPageSize() const
{
    return m_pageSize;
}

int ibScrollBar::GetRange() const
{
    return m_range;
}

void ibScrollBar::SetThumbPosition(int pos)
{
...
void ibScrollBar::SetScrollbar(int position, int thumbSize,
...
wxSize ibScrollBar::DoGetBestClientSize() const
{
...
ibScrollArrows::Arrow ibScrollBar::HitTestArrow(const wxPoint& pt) const
{
...
wxHitTest ibScrollBar::HitTestBar(const wxPoint& pt) const
{
...
/* static */
void ibScrollBar::GetScrollBarThumbSize(wxCoord length,
                                        int thumbPos,
                                        int thumbSize,
                                        int range,
                                        wxCoord *thumbStart,
...
wxRect ibScrollBar::GetScrollbarRect(ibScrollBar::Element elem,
                                     int thumbPos) const
{
    if ( thumbPos == -1 )
    {
    ...
    switch ( elem )
    {
        case ibScrollBar::Element_Arrow_Line_1:
            *start = 0;
            *width = arrow;
            break;

        case ibScrollBar::Element_Arrow_Line_2:
            *start = length - arrow;
            *width = arrow;
            break;

        case ibScrollBar::Element_Arrow_Page_1:
        case ibScrollBar::Element_Arrow_Page_2:
            // we don't have them at all
            break;

        case ibScrollBar::Element_Thumb:
        case ibScrollBar::Element_Bar_1:
        case ibScrollBar::Element_Bar_2:
            // we need to calculate the thumb position - do it
            {
                ...
                if ( elem == ibScrollBar::Element_Thumb )
                {
                    *start = thumbStart;
                    *width = thumbEnd - thumbStart;
                }
                else if ( elem == ibScrollBar::Element_Bar_1 )
                {
                    *start = 0;
                    *width = thumbStart;
                }
                else // elem == ibScrollBar::Element_Bar_2
                {
                    *start = thumbEnd;
...
wxCoord ibScrollBar::GetScrollbarSize() const
{
    const wxSize sizeArrowSB = m_renderer->GetScrollbarArrowSize();
...
wxCoord ibScrollBar::ScrollbarToPixel(int thumbPos)
{
    int range = GetRange();
    if ( !range )
    {
...
int ibScrollBar::PixelToScrollbar(wxCoord coord)
{
    const wxSize sizeArrow = m_renderer->GetScrollbarArrowSize();
...
void ibScrollBar::OnInternalIdle()
{
    UpdateThumb();
    ibControl::OnInternalIdle();
}

void ibScrollBar::UpdateThumb()
{
    if ( m_dirty )
    {
        for ( size_t n = 0; n < WXSIZEOF(m_elementsState); n++ )
...
void ibScrollBar::DoDraw(ibControlRenderer *renderer)
{
...
static inline ibScrollBar::Element ElementForArrow(ibScrollArrows::Arrow arrow)
{
    return arrow == ibScrollArrows::Arrow_First
            ? ibScrollBar::Element_Arrow_Line_1
            : ibScrollBar::Element_Arrow_Line_2;
}

int ibScrollBar::GetArrowState(ibScrollArrows::Arrow arrow) const
{
    return GetState(ElementForArrow(arrow));
}

void ibScrollBar::SetArrowFlag(ibScrollArrows::Arrow arrow, int flag, bool set)
{
...
int ibScrollBar::GetState(Element which) const
{
...
void ibScrollBar::SetState(Element which, int flags)
{
...
bool ibScrollBar::OnArrow(ibScrollArrows::Arrow arrow)
{
    int oldThumbPos = GetThumbPosition();
    PerformAction(arrow == ibScrollArrows::Arrow_First
                    ? ibACTION_SCROLL_LINE_UP
                    : ibACTION_SCROLL_LINE_DOWN);

    // did we scroll till the end?
    return GetThumbPosition() != oldThumbPos;
}

bool ibScrollBar::PerformAction(const ibControlAction& action,
                                long numArg,
                                const wxString& strArg)
{
    ...
    if ( action == ibACTION_SCROLL_THUMB_MOVE )
    {
        DoSetThumb(numArg);
    ...
    else if ( action == ibACTION_SCROLL_LINE_UP )
    {
        scrollType = wxEVT_SCROLLWIN_LINEUP;
        ScrollLines(-1);
    }
    else if ( action == ibACTION_SCROLL_LINE_DOWN )
    {
    ...
    else if ( action == ibACTION_SCROLL_PAGE_DOWN )
    {
        scrollType = wxEVT_SCROLLWIN_PAGEDOWN;
        ScrollPages(1);
    }
    else if ( action == ibACTION_SCROLL_START )
    {
        scrollType = wxEVT_SCROLLWIN_THUMBRELEASE; // anything better?
        ScrollToStart();
    }
    else if ( action == ibACTION_SCROLL_END )
    {
        scrollType = wxEVT_SCROLLWIN_THUMBRELEASE; // anything better?
        ScrollToEnd();
    }
    else if ( action == ibACTION_SCROLL_THUMB_DRAG )
    {
    ...
    else if ( action == ibACTION_SCROLL_THUMB_RELEASE )
    {
...
void ibScrollBar::ScrollToStart()
{
    DoSetThumb(0);
}

void ibScrollBar::ScrollToEnd()
{
    DoSetThumb(m_range - m_thumbSize);
}

bool ibScrollBar::ScrollLines(int nLines)
{
    DoSetThumb(m_thumbPos + nLines);
    return true;
}

bool ibScrollBar::ScrollPages(int nPages)
{
...
/* static */
ibInputHandler *ibScrollBar::GetStdInputHandler(ibInputHandler *handlerDef)
{
    static ibStdScrollBarInputHandler
        s_handler(ibThemeEngine::Get()->GetRenderer(), handlerDef);
...
ibScrollBarTimer::ibScrollBarTimer(ibStdScrollBarInputHandler *handler,
                                   const ibControlAction& action,
                                   ibScrollBar *control)
{
    m_handler = handler;
...
bool ibScrollBarTimer::DoNotify()
{
    return m_handler->OnScrollTimer(m_control, m_action);
}

// ----------------------------------------------------------------------------
// ibStdScrollBarInputHandler
// ----------------------------------------------------------------------------

ibStdScrollBarInputHandler::ibStdScrollBarInputHandler(ibRenderer *renderer,
...
ibStdScrollBarInputHandler::~ibStdScrollBarInputHandler()
{
...
void ibStdScrollBarInputHandler::SetElementState(ibScrollBar *control,
                                                 int flag,
                                                 bool doIt)
{
    ...
    {
        ibScrollBar::Element
            elem = (ibScrollBar::Element)(m_htLast - wxHT_SCROLLBAR_FIRST - 1);
...
bool ibStdScrollBarInputHandler::OnScrollTimer(ibScrollBar *scrollbar,
                                               const ibControlAction& action)
{
...
void ibStdScrollBarInputHandler::StopScrolling(ibScrollBar *control)
{
...
wxCoord
ibStdScrollBarInputHandler::GetMouseCoord(const ibScrollBar *scrollbar,
...
void ibStdScrollBarInputHandler::HandleThumbMove(ibScrollBar *scrollbar,
                                                 const wxMouseEvent& event)
{
    int thumbPos = GetMouseCoord(scrollbar, event) - m_ofsMouse;
    thumbPos = scrollbar->PixelToScrollbar(thumbPos);
    scrollbar->PerformAction(ibACTION_SCROLL_THUMB_MOVE, thumbPos);
}

bool ibStdScrollBarInputHandler::HandleKey(ibInputConsumer *consumer,
                                           ...
                                           bool pressed)
{
    ...
    if ( pressed )
    {
        ...
        {
            case WXK_DOWN:
            case WXK_RIGHT:     action = ibACTION_SCROLL_LINE_DOWN; break;
            case WXK_UP:
            case WXK_LEFT:      action = ibACTION_SCROLL_LINE_UP;   break;
            case WXK_HOME:      action = ibACTION_SCROLL_START;     break;
            case WXK_END:       action = ibACTION_SCROLL_END;       break;
            case WXK_PAGEUP:    action = ibACTION_SCROLL_PAGE_UP;   break;
            case WXK_PAGEDOWN:  action = ibACTION_SCROLL_PAGE_DOWN; break;
        }
...
bool ibStdScrollBarInputHandler::HandleMouse(ibInputConsumer *consumer,
                                             const wxMouseEvent& event)
{
    ...
    if ( btn == wxMOUSE_BTN_LEFT )
    {
        ...
        ibScrollBar *scrollbar = wxStaticCast(consumer->GetInputWindow(), ibScrollBar);
        ...
        if ( event.ButtonDown() || event.ButtonDClick() )
        {
            if ( !m_winCapture )
            {
                ...
                {
                    case wxHT_SCROLLBAR_ARROW_LINE_1:
                        action = ibACTION_SCROLL_LINE_UP;
                        break;

                    case wxHT_SCROLLBAR_ARROW_LINE_2:
                        action = ibACTION_SCROLL_LINE_DOWN;
                        break;

                    case wxHT_SCROLLBAR_BAR_1:
                        action = ibACTION_SCROLL_PAGE_UP;
                        m_ptStartScrolling = event.GetPosition();
                        break;

                    case wxHT_SCROLLBAR_BAR_2:
                        action = ibACTION_SCROLL_PAGE_DOWN;
                        m_ptStartScrolling = event.GetPosition();
                        break;

                    case wxHT_SCROLLBAR_THUMB:
                        consumer->PerformAction(ibACTION_SCROLL_THUMB_DRAG);
                ...
                if ( hasAction )
                {
                    m_timerScroll = new ibScrollBarTimer(this, action,
                                                         scrollbar);
        ...
        else if ( btn == m_btnCapture )
        {
            if ( m_winCapture )
            {
                ...
                if ( m_htLast == wxHT_SCROLLBAR_THUMB )
                {
                    scrollbar->PerformAction(ibACTION_SCROLL_THUMB_RELEASE);
                }
...
bool ibStdScrollBarInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                                 const wxMouseEvent& event)
{
    ibScrollBar *scrollbar = wxStaticCast(consumer->GetInputWindow(), ibScrollBar);
    ...
    if ( event.Dragging() )
    {
        ...
        if ( !isArrow )
            Highlight(scrollbar, true);
        //else: already done by ibScrollArrows::HandleMouseMove
    }
...
ibScrollTimer::ibScrollTimer()
{
    m_skipNext = false;
}

void ibScrollTimer::StartAutoScroll()
{
...
wxIMPLEMENT_DYNAMIC_CLASS(ibScrollBar, ibControl);
