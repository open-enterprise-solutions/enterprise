...
#if wxDEBUG_LEVEL >= 2
    // turn extra ibTextCtrl-specific debugging on/off
    #define WXDEBUG_TEXT

    // turn ibTextCtrl::Replace() debugging on (slows down code a *lot*!)
...
// ----------------------------------------------------------------------------
// ibStdTextCtrlInputHandler: this control handles only the mouse/kbd actions
// common to Win32 and GTK, platform-specific things are implemented elsewhere
// ----------------------------------------------------------------------------

class FRONTEND_API ibStdTextCtrlInputHandler : public ibStdInputHandler
{
public:
    ibStdTextCtrlInputHandler(ibInputHandler *inphand);
...
protected:
    ...
    static wxTextPos HitTest(const ibTextCtrl *text, const wxPoint& pos);

    // capture data
    ibTextCtrl *m_winCapture;
};
...
friend void ibTextCtrl::LayoutLine(wxTextCoord line,
                                    wxWrappedLineData& lineData) const;
friend void ibTextCtrl::LayoutLines(wxTextCoord) const;
...
/*
   ...
   as we want to avoid storing the backpointer to ibTextCtrl in wxCommand
   itself: this is a waste of memory as all commands in the given command
   processor always have the same associated ibTextCtrl and so it makes sense
...
// a command corresponding to a ibTextCtrl action
class ibTextCtrlCommand : public wxCommand
{
public:
    ibTextCtrlCommand(const wxString& name) : wxCommand(true, name) { }

    // we don't use these methods as they don't make sense for us as we need a
    // ibTextCtrl to be applied
    ...
    virtual bool Do(ibTextCtrl *text) = 0;
    virtual bool Undo(ibTextCtrl *text) = 0;
};

// insert text command
class ibTextCtrlInsertCommand : public ibTextCtrlCommand
{
public:
    ibTextCtrlInsertCommand(const wxString& textToInsert)
        : ibTextCtrlCommand(wxTEXT_COMMAND_INSERT), m_text(textToInsert)
    {
        m_from = -1;
    }

    // combine the 2 commands together
    void Append(ibTextCtrlInsertCommand *other);

    virtual bool CanUndo() const;
    virtual bool Do(ibTextCtrl *text);
    virtual bool Do() { return ibTextCtrlCommand::Do(); }
    virtual bool Undo() { return ibTextCtrlCommand::Undo(); }
    virtual bool Undo(ibTextCtrl *text);
...
// remove text command
class ibTextCtrlRemoveCommand : public ibTextCtrlCommand
{
public:
    ibTextCtrlRemoveCommand(wxTextPos from, wxTextPos to)
        : ibTextCtrlCommand(wxTEXT_COMMAND_REMOVE)
    {
        m_from = from;
        m_to = to;
    }
...
// a command processor for a ibTextCtrl
class ibTextCtrlCommandProcessor : public wxCommandProcessor
{
public:
    ibTextCtrlCommandProcessor(ibTextCtrl *text)
    {
    ...
    // override Store() to compress multiple ibTextCtrlInsertCommand into one
    ...
    // accessors
    ibTextCtrl *GetTextCtrl() const { return m_text; }
    bool IsCompressing() const { return m_compressInserts; }

protected:
    virtual bool DoCommand(wxCommand& cmd)
        { return ((ibTextCtrlCommand &)cmd).Do(m_text); }
    virtual bool UndoCommand(wxCommand& cmd)
        { return ((ibTextCtrlCommand &)cmd).Undo(m_text); }

    // check if this command is a ibTextCtrlInsertCommand and return it casted
    // to the right type if it is or nullptr otherwise
    ibTextCtrlInsertCommand *IsInsertCommand(wxCommand *cmd);
...
wxBEGIN_EVENT_TABLE(ibTextCtrl, ibControl)
    EVT_CONTEXT_MENU(ibTextCtrl::OnContextMenu)
    EVT_MENU(wxID_UNDO, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_REDO, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_CUT, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_COPY, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_PASTE, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_DELETE, ibTextCtrl::OnContextMenuCommand)
    EVT_MENU(wxID_SELECTALL, ibTextCtrl::OnContextMenuCommand)
    EVT_CHAR(ibTextCtrl::OnChar)

    EVT_SIZE(ibTextCtrl::OnSize)
wxEND_EVENT_TABLE()
...
void ibTextCtrl::Init()
{
    ...
    m_cmdProcessor = new ibTextCtrlCommandProcessor(this);

    m_passwordMaskLive = false;
    m_passwordMaskTimer = nullptr;

    // no data yet
    m_data.data = nullptr;
}

bool ibTextCtrl::Create(wxWindow *parent,
                        wxWindowID id,
                        ...
                        const wxString &name)
{
    ...
    {
        // SEAM vs univ: the canon forced wxALWAYS_SHOW_SB here ("for
        // compatibility with wxMSW") — a permanently visible scrollbar with
        // an empty range reads as broken; ours appears on demand
        ...
        else // we must wrap lines if we don't have horizontal scrollbar
        {
            ...
            //     later and even ibWindow::Create() itself temporarily resets
    ...
    // SEAM vs univ: on the native host the dialog navigation
    // (IsDialogMessage) eats Enter/Tab/arrows before the control sees them;
    // ask for the chars — Tab navigation and the default button are then
    // re-implemented in OnChar/ClickDefaultButtonIfPossible
    style |= wxWANTS_CHARS;
    ...
    if ( IsPassword() )
    {
        // collapses the live typing mask back to the fixed length
        m_passwordMaskTimer = new wxTimer(this);
        Bind(wxEVT_TIMER, [this](wxTimerEvent&) {
            m_passwordMaskLive = false;
            Refresh();
        }, m_passwordMaskTimer->GetId());
    }
    ...
    CreateInputHandler(ibINP_HANDLER_TEXTCTRL);
...
ibTextCtrl::~ibTextCtrl()
{
    delete m_passwordMaskTimer;
...
void ibTextCtrl::DoSetValue(const wxString& value, int flags)
{
    if ( value != DoGetValue() )
    {
...
const wxArrayString& ibTextCtrl::GetLines() const
{
    return MData().m_lines;
}

size_t ibTextCtrl::GetLineCount() const
{
    return MData().m_lines.GetCount();
}

wxString ibTextCtrl::DoGetValue() const
{
    ...
    if ( !IsSingleLine() && m_value.empty() )
    {
        ...
        ibTextCtrl *self = wxConstCast(this, ibTextCtrl);
...
void ibTextCtrl::Clear()
{
    SetValue(wxEmptyString);
}

bool ibTextCtrl::ReplaceLine(wxTextCoord line,
                             const wxString& text)
{
    if ( WrapLines() )
    {
...
void ibTextCtrl::RemoveLine(wxTextCoord line)
{
...
void ibTextCtrl::InsertLine(wxTextCoord line, const wxString& text)
{
...
void ibTextCtrl::Replace(wxTextPos from, wxTextPos to, const wxString& text)
{
    ...
    {
        wxFAIL_MSG(wxT("invalid range in ibTextCtrl::Replace"));
    ...
    if ( IsSingleLine() )
    {
        ...
        if ( IsPassword() )
        {
            // show the live mask while typing, collapse it back to the
            // fixed length shortly after the input stops
            m_passwordMaskLive = true;
            if ( m_passwordMaskTimer )
                m_passwordMaskTimer->StartOnce(800);

            // the incremental pixel ranges are meaningless through the
            // mask — repaint the short line fully
            Refresh();
        }
        else
        {
...
void ibTextCtrl::Remove(wxTextPos from, wxTextPos to)
{
...
void ibTextCtrl::WriteText(const wxString& text)
{
...
void ibTextCtrl::AppendText(const wxString& text)
{
...
void ibTextCtrl::SetInsertionPoint(wxTextPos pos)
{
    if ( pos == -1 )
        pos = GetLastPosition();
...
void ibTextCtrl::InitInsertionPoint()
{
...
void ibTextCtrl::MoveInsertionPoint(wxTextPos pos)
{
...
void ibTextCtrl::DoSetInsertionPoint(wxTextPos pos)
{
...
void ibTextCtrl::SetInsertionPointEnd()
{
    SetInsertionPoint(GetLastPosition());
}

wxTextPos ibTextCtrl::GetInsertionPoint() const
{
...
wxTextPos ibTextCtrl::GetLastPosition() const
{
    wxTextPos pos;
    if ( IsSingleLine() )
    {
...
void ibTextCtrl::GetSelection(wxTextPos* from, wxTextPos* to) const
{
    if ( from )
        *from = m_selStart;
    if ( to )
...
wxString ibTextCtrl::GetSelectionText() const
{
    wxString sel;
...
void ibTextCtrl::SetSelection(wxTextPos from, wxTextPos to)
{
    ...
    else // valid sel range
    {
        ...
        wxCHECK_RET( to <= GetLastPosition(),
                     wxT("invalid range in ibTextCtrl::SetSelection") );
...
void ibTextCtrl::ClearSelection()
{
    if ( HasSelection() )
    {
        // we need to use temp vars as RefreshTextRange() may call DoDraw()
...
void ibTextCtrl::RemoveSelection()
{
    if ( !HasSelection() )
        return;
...
bool ibTextCtrl::GetSelectedPartOfLine(wxTextCoord line,
...
bool ibTextCtrl::IsModified() const
{
    return m_isModified;
}

bool ibTextCtrl::IsEditable() const
{
    // disabled control can never be edited
    return m_isEditable && IsEnabled();
}

void ibTextCtrl::MarkDirty()
{
    m_isModified = true;
}

void ibTextCtrl::DiscardEdits()
{
    m_isModified = false;
}

void ibTextCtrl::SetEditable(bool editable)
{
...
int ibTextCtrl::GetLineLength(wxTextCoord line) const
{
    if ( IsSingleLine() )
    {
...
wxString ibTextCtrl::GetLineText(wxTextCoord line) const
{
    if ( IsSingleLine() )
    {
        wxASSERT_MSG( line == 0, wxT("invalid GetLineLength() parameter") );
...
int ibTextCtrl::GetNumberOfLines() const
{
    // there is always 1 line, even if the text is empty
    return IsSingleLine() ? 1 : GetLineCount();
}

wxTextPos ibTextCtrl::XYToPosition(wxTextCoord x, wxTextCoord y) const
{
...
bool ibTextCtrl::PositionToXY(wxTextPos pos,
...
wxTextCoord ibTextCtrl::GetRowsPerLine(wxTextCoord line) const
{
...
wxTextCoord ibTextCtrl::GetRowCount() const
{
...
wxTextCoord ibTextCtrl::GetRowAfterLine(wxTextCoord line) const
{
    if ( !WrapLines() )
        return line + 1;
...
wxTextCoord ibTextCtrl::GetFirstRowOfLine(wxTextCoord line) const
{
    if ( !WrapLines() )
        return line;
...
bool ibTextCtrl::PositionToLogicalXY(wxTextPos pos,
                                     wxCoord *xOut,
                                     wxCoord *yOut) const
{
...
bool ibTextCtrl::PositionToDeviceXY(wxTextPos pos,
                                    wxCoord *xOut,
                                    wxCoord *yOut) const
{
    wxCoord x, y;
...
wxPoint ibTextCtrl::GetCaretPosition() const
{
...
void ibTextCtrl::ShowPosition(wxTextPos pos)
{
...
wxTextPos ibTextCtrl::GetWordStart() const
{
...
wxTextPos ibTextCtrl::GetWordEnd() const
{
    if ( m_curPos == -1 )
        return 0;
...
void ibTextCtrl::Copy()
{
#if wxUSE_CLIPBOARD
    // nothing ever leaves a password field through the clipboard — that is
...
bool ibTextCtrl::DoCut()
{
    // see Copy(): the clipboard never sees a password field
    if ( IsPassword() )
        return false;
...
void ibTextCtrl::Paste()
{
    (void)DoPaste();
}

bool ibTextCtrl::DoPaste()
{
#if wxUSE_CLIPBOARD
    wxClipboardLocker clipLock;
...
ibTextCtrlInsertCommand *
ibTextCtrlCommandProcessor::IsInsertCommand(wxCommand *command)
{
    return (ibTextCtrlInsertCommand *)
            (command && (command->GetName() == wxTEXT_COMMAND_INSERT)
                ? command : nullptr);
}

void ibTextCtrlCommandProcessor::Store(wxCommand *command)
{
    ibTextCtrlInsertCommand *cmdIns = IsInsertCommand(command);
    if ( cmdIns )
    {
        if ( IsCompressing() )
        {
...
void ibTextCtrlInsertCommand::Append(ibTextCtrlInsertCommand *other)
{
    m_text += other->m_text;
}

bool ibTextCtrlInsertCommand::CanUndo() const
{
    return m_from != -1;
}

bool ibTextCtrlInsertCommand::Do(ibTextCtrl *text)
{
...
bool ibTextCtrlInsertCommand::Undo(ibTextCtrl *text)
{
...
bool ibTextCtrlRemoveCommand::CanUndo() const
{
    // if we were executed, we should have the text we removed
    return !m_textDeleted.empty();
}

bool ibTextCtrlRemoveCommand::Do(ibTextCtrl *text)
{
...
bool ibTextCtrlRemoveCommand::Undo(ibTextCtrl *text)
{
...
bool ibTextCtrl::CanUndo() const
{
    return IsEditable() && m_cmdProcessor->CanUndo();
}

bool ibTextCtrl::CanRedo() const
{
...
wxSize ibTextCtrl::DoGetBestClientSize() const
{
    ...
    if ( m_heightLine == -1 )
    {
        wxConstCast(this, ibTextCtrl)->RecalcFontMetrics();
    }

    // SEAM vs univ: the canon sized the field by its CURRENT text, so every
    // field came out a different size. The default is uniform — the width
    // comes from the font only, the height from the font (the line height)
    int wChar = GetAverageWidth(),
        hChar = GetLineHeight();

    wxCoord w = wxMax(8*wChar, 50);
    wxCoord h = hChar;
    ...
    // the default field is compact: the text line + the theme paddings,
    // nothing more (per the reference look — no button-height stretching)
    return wxSize(rectTotal.width, rectTotal.height);
}

void ibTextCtrl::UpdateTextRect()
{
    wxRect rectTotal(GetClientSize());
    wxCoord *extraSpace = WrapLines() ? &WData().m_widthMark : nullptr;
    ...
    // SEAM vs univ: a sizer-stretched single-line field drew its text glued
    // to the top; centre the text band vertically the way native edits do
    // (the caret and hit-test share m_rectText, so everything stays aligned)
    if ( IsSingleLine() && m_heightLine > 0 && m_rectText.height > m_heightLine )
    {
        m_rectText.y += (m_rectText.height - m_heightLine) / 2;
        m_rectText.height = m_heightLine;
    }
...
void ibTextCtrl::UpdateLastVisible()
{
...
void ibTextCtrl::OnSize(wxSizeEvent& event)
{
    UpdateTextRect();
...
void ibTextCtrl::SetMaxLength(unsigned long len)
{
...
wxCoord ibTextCtrl::GetTotalWidth() const
{
    wxCoord w;
    CalcUnscrolledPosition(m_rectText.width, 0, &w, nullptr);
    return w;
}

wxCoord ibTextCtrl::GetTextWidth(const wxString& text) const
{
    wxCoord w;
    GetTextExtent(GetTextToShow(text), &w, nullptr);
    return w;
}

wxRect ibTextCtrl::GetRealTextArea() const
{
...
wxTextCoord ibTextCtrl::GetRowInLine(wxTextCoord line,
                                     wxTextCoord col,
...
void ibTextCtrl::LayoutLine(wxTextCoord line, wxWrappedLineData& lineData) const
{
...
void ibTextCtrl::LayoutLines(wxTextCoord lineLast) const
{
...
size_t ibTextCtrl::GetPartOfWrappedLine(const wxChar* text,
...
wxTextCtrlHitTestResult ibTextCtrl::HitTestLine(const wxString& line,
                                                wxCoord x,
                                                wxTextCoord *colOut) const
{
    ...
    else // we're inside the line
    {
        ...
        dc.GetTextExtent(line[0], &width, nullptr);
        ...
        for ( ;; )
        {
            ...
            dc.GetTextExtent(strBefore, &width, nullptr);
            if ( width > x )
            {
            ...
            wxASSERT_MSG( matchDir, wxT("logic error in ibTextCtrl::HitTest") );
    ...
    if ( res == wxTE_HT_ON_TEXT )
    {
        ...
        dc.GetTextExtent(text, &width1, nullptr);
        if ( (size_t)col < line.length() )
        {
            wxCoord width2;

            text += line[col];
            dc.GetTextExtent(text, &width2, nullptr);
...
wxTextCtrlHitTestResult ibTextCtrl::HitTest(const wxPoint& pt, long *pos) const
{
    wxTextCoord x, y;
...
wxTextCtrlHitTestResult ibTextCtrl::HitTest(const wxPoint& pos,
                                            wxTextCoord *colOut,
                                            wxTextCoord *rowOut) const
{
    return HitTest2(pos.y, pos.x, 0, rowOut, colOut, nullptr, nullptr);
}

wxTextCtrlHitTestResult ibTextCtrl::HitTestLogical(const wxPoint& pos,
                                                   wxTextCoord *colOut,
                                                   wxTextCoord *rowOut) const
{
    return HitTest2(pos.y, pos.x, 0, rowOut, colOut, nullptr, nullptr, false);
}

wxTextCtrlHitTestResult ibTextCtrl::HitTest2(wxCoord y0,
                                             wxCoord x10,
                                             wxCoord x20,
                                             wxTextCoord *rowOut,
...
bool ibTextCtrl::GetLineAndRow(wxTextCoord row,
                               wxTextCoord *lineOut,
...
/*
   ibTextCtrl has not one but two scrolling mechanisms: one is semi-automatic
...
void ibTextCtrl::ShowHorzPosition(wxCoord pos)
{
...
void ibTextCtrl::ScrollText(wxTextCoord col)
{
...
void ibTextCtrl::CalcUnscrolledPosition(int x, int y, int *xx, int *yy) const
{
    if ( IsSingleLine() )
    {
...
void ibTextCtrl::CalcScrolledPosition(int x, int y, int *xx, int *yy) const
{
    if ( IsSingleLine() )
    {
        // we don't use wxScrollHelper
...
void ibTextCtrl::DoPrepareReadOnlyDC(wxReadOnlyDC& dc)
{
...
void ibTextCtrl::UpdateMaxWidth(wxTextCoord line)
{
    // OPT!
    ...
    wxCoord widthMaxOld = MData().m_widthMax,
            width;
    GetTextExtent(GetLineText(line), &width, nullptr);
...
void ibTextCtrl::RecalcFontMetrics()
{
    m_heightLine = GetCharHeight();
    m_widthAvg = GetCharWidth();
}

void ibTextCtrl::RecalcMaxWidth()
{
...
wxCoord ibTextCtrl::GetMaxWidth() const
{
    if ( MData().m_widthMax == -1 )
    {
        ...
        {
            wxCoord width;
            dc.GetTextExtent(GetLines()[n], &width, nullptr);
            if ( width > MData().m_widthMax )
            {
...
// SEAM vs univ: the univ partial-blit scroll (ScrollNoRefresh on the target
// rect) leaves stale rows with the buffered paint pipeline — scroll by full
// repaint instead; the caret must be re-seated too: its DEVICE position
// changed with the scroll offset (otherwise it visibly rides the scroll)
void ibTextCtrl::ScrollWindow(int dx, int dy, const wxRect* rect)
{
    wxUnusedVar(dx);
    wxUnusedVar(dy);
    wxUnusedVar(rect);
...
void ibTextCtrl::UpdateScrollbars()
{
    wxASSERT_MSG( !IsSingleLine(), wxT("only used for multiline") );
    ...
    if ( (scrollRangeY != scrollRangeYOld) || (scrollRangeX != scrollRangeXOld) )
    {
        ...
        // SEAM vs univ: wxScrollHelperBase::SetScrollbars calls
        // AdjustScrollbars() for target==win ONLY under __WXUNIVERSAL__;
        // in the native build it relies on a size event which never comes
        // when only the CONTENT grew — push the ranges to the bars ourselves
        AdjustScrollbars();
...
void ibTextCtrl::OnInternalIdle()
{
...
bool ibTextCtrl::SendAutoScrollEvents(wxScrollWinEvent& event) const
{
...
void ibTextCtrl::RefreshSelection()
{
    if ( HasSelection() )
    {
        RefreshTextRange(m_selStart, m_selEnd);
...
void ibTextCtrl::RefreshLineRange(wxTextCoord lineFirst, wxTextCoord lineLast)
{
...
void ibTextCtrl::RefreshTextRange(wxTextPos start, wxTextPos end)
{
...
void ibTextCtrl::RefreshColRange(wxTextCoord line,
                                 wxTextPos start,
                                 size_t count)
{
...
void ibTextCtrl::RefreshPixelRange(wxTextCoord line,
                                   wxCoord start,
                                   wxCoord width)
{
...
void ibTextCtrl::RefreshTextRect(const wxRect& rectClient, bool textOnly)
{
    wxRect rect;
...
void ibTextCtrl::RefreshLineWrapMarks(wxTextCoord rowFirst,
                                      wxTextCoord rowLast)
{
...
void ibTextCtrl::DoDrawBorder(wxDC& dc, const wxRect& rect)
{
...
/*
   Several remarks about ibTextCtrl redraw logic:
...
// the password mask has a FIXED length: the shown asterisks must not leak
// the real password length
static const size_t kPasswordMaskLength = 8;

wxString ibTextCtrl::GetTextToShow(const wxString& text) const
{
    wxString textShown;
    if ( IsPassword() )
        textShown = wxString(wxT('*'),
            m_passwordMaskLive ? text.length() : kPasswordMaskLength);
    else
        textShown = text;
...
void ibTextCtrl::DoDrawTextInRect(wxDC& dc, const wxRect& rectUpdate)
{
    ...
    // the fixed-length password mask is independent of the real columns —
    // the canon per-column arithmetic would draw fragments of it; always
    // draw the whole mask instead (no selection: clipboard is disabled)
    if ( IsPassword() )
    {
        ibRenderer *rendMask = GetRenderer();

        wxRect rectMask = m_rectText;
        rectMask.width = GetTextWidth(m_value);

        rendMask->DrawTextLine(dc, GetTextToShow(m_value), rectMask,
                               -1, -1, GetStateFlags());
        return;
    }
    ...
    else // multiline
    {
        ...
        (void)HitTest(pt, nullptr, &lineStart);

        pt.y += rectUpdate.height;
        (void)HitTest(pt, nullptr, &lineEnd);
    }
    ...
    {
        CalcUnscrolledPosition(0, yClient, nullptr, &rectText.y);
    }

    ibRenderer *renderer = GetRenderer();
...
void ibTextCtrl::DoDrawLineWrapMarks(wxDC& dc, const wxRect& rectUpdate)
{
    ...
    CalcUnscrolledPosition(0, rectUpdate.y - yTop, nullptr, &rectMark.y);
    wxCoord hLine = GetLineHeight();
    ...
    wxCoord yBottom;
    CalcUnscrolledPosition(0, rectUpdate.GetBottom() - yTop, nullptr, &yBottom);
...
void ibTextCtrl::DoDraw(ibControlRenderer *renderer)
{
...
bool ibTextCtrl::SetFont(const wxFont& font)
{
...
bool ibTextCtrl::Enable(bool enable)
{
    if ( !ibControl::Enable(enable) )
        return false;
...
void ibTextCtrl::CreateCaret()
{
    // SEAM vs univ: the canon gave read-only controls no caret at all; the
    // native edit shows the caret in a read-only field too (you can browse
    // and select), so we always create one
    // FIXME use renderer
    wxCaret *caret = new wxCaret(this, 1, GetLineHeight());

    // SetCaret() will delete the old caret if any
    SetCaret(caret);
}

void ibTextCtrl::ShowCaret(bool show)
{
    wxCaret *caret = GetCaret();
    if ( caret )
    {
...
size_t ibTextCtrl::GetLinesPerPage() const
{
    if ( IsSingleLine() )
        return 1;
...
wxTextPos ibTextCtrl::GetPositionAbove()
{
...
wxTextPos ibTextCtrl::GetPositionBelow()
{
    wxCHECK_MSG( !IsSingleLine(), INVALID_POS_VALUE,
                 wxT("can't move cursor vertically in a single line control") );

    // see comments for ibACTION_TEXT_UP
    wxPoint pt = GetCaretPosition() - m_rectText.GetPosition();
    if ( MData().m_xCaret == -1 )
    {
        // remember the initial cursor abscissa
...
bool ibTextCtrl::PerformAction(const ibControlAction& actionOrig,
                               long numArg,
                               const wxString& strArg)
{
    ...
    // the command this action corresponds to or nullptr if this action doesn't
    // change text at all or can't be undone
    ibTextCtrlCommand *command = nullptr;

    wxString action;
    bool del = false,
         sel = false;
    if ( actionOrig.StartsWith(ibACTION_TEXT_PREFIX_DEL, &action) )
    {
        if ( IsEditable() )
            del = true;
    }
    else if ( actionOrig.StartsWith(ibACTION_TEXT_PREFIX_SEL, &action) )
    {
        sel = true;
    }
    ...
    if ( action == ibACTION_TEXT_HOME )
    {
        newPos = m_curPos - m_curCol;
    }
    else if ( action == ibACTION_TEXT_END )
    {
        newPos = m_curPos + GetLineLength(m_curRow) - m_curCol;
    }
    else if ( (action == ibACTION_TEXT_GOTO) ||
              (action == ibACTION_TEXT_FIRST) ||
              (action == ibACTION_TEXT_LAST) )
    {
        if ( action == ibACTION_TEXT_FIRST )
            numArg = 0;
        else if ( action == ibACTION_TEXT_LAST )
            numArg = GetLastPosition();
        //else: numArg already contains the position

        newPos = numArg;
    }
    else if ( action == ibACTION_TEXT_UP )
    {
        if ( !IsSingleLine() )
        {
    ...
    else if ( action == ibACTION_TEXT_DOWN )
    {
        if ( !IsSingleLine() )
        {
    ...
    else if ( action == ibACTION_TEXT_LEFT )
    {
        newPos = m_curPos - 1;
    }
    else if ( action == ibACTION_TEXT_WORD_LEFT )
    {
        newPos = GetWordStart();
    }
    else if ( action == ibACTION_TEXT_RIGHT )
    {
        newPos = m_curPos + 1;
    }
    else if ( action == ibACTION_TEXT_WORD_RIGHT )
    {
        newPos = GetWordEnd();
    }
    else if ( action == ibACTION_TEXT_INSERT )
    {
        if ( IsEditable() && !strArg.empty() )
        {
            ...
            if ( !acceptedString.empty() )
                ...
                command = new ibTextCtrlInsertCommand(acceptedString);
        }

    }
    else if ( (action == ibACTION_TEXT_PAGE_UP) ||
              (action == ibACTION_TEXT_PAGE_DOWN) )
    {
        if ( !IsSingleLine() )
        {
            ...
            bool goUp = action == ibACTION_TEXT_PAGE_UP;
            ...
            if ( goUp )
            {
                ...
                PositionToLogicalXY(pos, nullptr, &y);
            }
            else // scrolled down
            {
                PositionToLogicalXY(newPos, nullptr, &y);
            }
    ...
    else if ( action == ibACTION_TEXT_SEL_WORD )
    {
        SetSelection(GetWordStart(), GetWordEnd());
    }
    else if ( action == ibACTION_TEXT_ANCHOR_SEL )
    {
        newPos = numArg;
    }
    else if ( action == ibACTION_TEXT_EXTEND_SEL )
    {
        SetSelection(m_selAnchor, numArg);
    }
    else if ( action == ibACTION_TEXT_COPY )
    {
        Copy();
    }
    else if ( action == ibACTION_TEXT_CUT )
    {
        if ( IsEditable() )
            Cut();
    }
    else if ( action == ibACTION_TEXT_PASTE )
    {
        if ( IsEditable() )
            Paste();
    }
    else if ( action == ibACTION_TEXT_UNDO )
    {
        if ( CanUndo() )
            Undo();
    }
    else if ( action == ibACTION_TEXT_REDO )
    {
        if ( CanRedo() )
            Redo();
    }
    else if ( action == ibACTION_TEXT_RETURN )
    {
    ...
    if ( newPos != INVALID_POS_VALUE )
    {
        ...
        if ( del )
        {
            ...
            {
                command = new ibTextCtrlRemoveCommand(from, to);
            }
        }
...
// ----------------------------------------------------------------------------
// context menu — the themed univ ibMenu (stock ids keep their stock labels
// via the wxMenuItemBase ctor)
// ----------------------------------------------------------------------------

void ibTextCtrl::OnContextMenu(wxContextMenuEvent& event)
{
    if ( FindFocus() != this )
        SetFocus();
    ...
    // cut/copy are dead for password fields — nothing leaves the mask
    // through the clipboard, that is the whole point
    const bool password = IsPassword();
    menu.Enable(wxID_UNDO, CanUndo());
    menu.Enable(wxID_REDO, CanRedo());
    menu.Enable(wxID_CUT, IsEditable() && HasSelection() && !password);
    menu.Enable(wxID_COPY, HasSelection() && !password);
    menu.Enable(wxID_PASTE, IsEditable());
    menu.Enable(wxID_DELETE, IsEditable() && HasSelection());
    menu.Enable(wxID_SELECTALL, GetLastPosition() > 0);

    wxPoint pos = event.GetPosition();
    pos = (pos == wxDefaultPosition) ? GetCaretPosition()
                                     : ScreenToClient(pos);
    PopupMenu(&menu, pos);
}

void ibTextCtrl::OnContextMenuCommand(wxCommandEvent& event)
{
    switch ( event.GetId() )
    {
        ...
        case wxID_DELETE:    RemoveSelection(); break;
        case wxID_SELECTALL: SelectAll(); break;
        default:             event.Skip();
    }
}
void ibTextCtrl::OnChar(wxKeyEvent& event)
{
    // SEAM vs univ: wxWANTS_CHARS means the dialog navigation never sees
    // Tab — hand it over ourselves unless the control wants real tabs
    ...
    if ( !event.HasModifiers() )
    {
        ...
        if ( keycode == WXK_RETURN )
        {
            ...
            {
                PerformAction(ibACTION_TEXT_INSERT, -1, wxT('\n'));
            }
        }
        else if ( unicode >= 32 )
        {
            // SEAM vs univ: Unicode-first. The univ original trusted
            // GetKeyCode() for codes < 255, but on MSW that is an ANSI
            // code for non-ASCII chars — Cyrillic input became mojibake
            PerformAction(ibACTION_TEXT_INSERT, -1, wxString(unicode));
...
bool ibTextCtrl::ClickDefaultButtonIfPossible()
{
    // SEAM vs univ: with wxWANTS_CHARS the dialog never sees the Enter key,
    // so trigger the default button ourselves (ib or native)
    wxTopLevelWindow* tlw =
        wxDynamicCast(wxGetTopLevelParent(this), wxTopLevelWindow);
    if ( !tlw )
        return false;

    wxWindow* item = tlw->GetDefaultItem();
    if ( !item || !item->IsEnabled() || !item->IsShown() )
        return false;

    ibButton* ibBtn = wxDynamicCast(item, ibButton);
    if ( ibBtn )
    {
        ibBtn->Click();
        return true;
    ...
    wxButton* nativeBtn = wxDynamicCast(item, wxButton);
    if ( nativeBtn )
    {
        wxCommandEvent event(wxEVT_BUTTON, nativeBtn->GetId());
        event.SetEventObject(nativeBtn);
        nativeBtn->ProcessWindowEvent(event);
        return true;
    }
...
/* static */
ibInputHandler *ibTextCtrl::GetStdInputHandler(ibInputHandler *handlerDef)
{
    static ibStdTextCtrlInputHandler s_handler(handlerDef);

    return &s_handler;
}

// ----------------------------------------------------------------------------
// ibStdTextCtrlInputHandler
// ----------------------------------------------------------------------------

ibStdTextCtrlInputHandler::ibStdTextCtrlInputHandler(ibInputHandler *inphand)
...
/* static */
wxTextPos ibStdTextCtrlInputHandler::HitTest(const ibTextCtrl *text,
                                             const wxPoint& pt)
{
    wxTextCoord col, row;
    ...
    // SEAM vs univ: the canon did pos++ here for wxTE_HT_BEYOND "to adjust
    // to the last position in the line" — but HitTestLine already returns
    // col = line.length() for BEYOND, i.e. the position PAST the last char,
    // and the increment jumped the caret onto the NEXT line (confirmed by
    // telemetry: col=10/len=10 -> pos 44 -> ++ -> 45 = next line start)
    wxUnusedVar(ht);

    return pos;
}

bool ibStdTextCtrlInputHandler::HandleKey(ibInputConsumer *consumer,
                                          const wxKeyEvent& event,
                                          bool pressed)
{
    ...
    if ( shiftDown )
    {
        action = ibACTION_TEXT_PREFIX_SEL;
    }
    ...
    if ( event.AltDown() )
    {
        if ( keycode == WXK_BACK && !ctrlDown && !shiftDown )
            action = ibACTION_TEXT_UNDO;
    }
    else switch ( keycode )
    {
        // cursor movement
        case WXK_HOME:
            action << (ctrlDown ? ibACTION_TEXT_FIRST
                                : ibACTION_TEXT_HOME);
            break;

        case WXK_END:
            action << (ctrlDown ? ibACTION_TEXT_LAST
                                : ibACTION_TEXT_END);
            break;

        case WXK_UP:
            if ( !ctrlDown )
                action << ibACTION_TEXT_UP;
            break;

        case WXK_DOWN:
            if ( !ctrlDown )
                action << ibACTION_TEXT_DOWN;
            break;

        case WXK_LEFT:
            action << (ctrlDown ? ibACTION_TEXT_WORD_LEFT
                                : ibACTION_TEXT_LEFT);
            break;

        case WXK_RIGHT:
            action << (ctrlDown ? ibACTION_TEXT_WORD_RIGHT
                                : ibACTION_TEXT_RIGHT);
            break;
        ...
        case WXK_PAGEUP:
            action << ibACTION_TEXT_PAGE_UP;
            break;

        // delete
        case WXK_DELETE:
            if ( !ctrlDown )
                action << ibACTION_TEXT_PREFIX_DEL << ibACTION_TEXT_RIGHT;
            break;

        case WXK_BACK:
            if ( !ctrlDown )
                action << ibACTION_TEXT_PREFIX_DEL << ibACTION_TEXT_LEFT;
            break;

        case WXK_RETURN:
            action << ibACTION_TEXT_RETURN;
            break;
        // something else
        default:
            // reset the action as it could be already set to one of the
            // prefixes
            action = ibACTION_NONE;
            ...
            if ( ctrlDown )
            {
                ...
                {
                    case 'A':
                        // univ mapped Ctrl+A to redo; select-all is the convention
                        wxStaticCast(consumer->GetInputWindow(), ibTextCtrl)->SelectAll();
                        return true;

                    case 'Y':
                        action = ibACTION_TEXT_REDO;
                        break;

                    case 'C':
                        action = ibACTION_TEXT_COPY;
                        break;

                    case 'V':
                        action = ibACTION_TEXT_PASTE;
                        break;

                    case 'X':
                        action = ibACTION_TEXT_CUT;
                        break;
    ...
    if ( (action != ibACTION_NONE) && (action != ibACTION_TEXT_PREFIX_SEL) )
    {
        bool result = consumer->PerformAction(action, -1, str);
        if ( !result && action == ibACTION_TEXT_RETURN )
        {
        ...
        // that use ibTextCtrl, such as wxSpinCtrl
        if ( (action != ibACTION_TEXT_UP) &
             (action != ibACTION_TEXT_DOWN) &
             (action != ibACTION_TEXT_PAGE_DOWN) &
             (action != ibACTION_TEXT_PAGE_UP) )
            return true;
...
bool ibStdTextCtrlInputHandler::HandleMouse(ibInputConsumer *consumer,
                                            const wxMouseEvent& event)
{
    if ( event.LeftDown() )
    {
        ...
        ibTextCtrl *text = wxStaticCast(consumer->GetInputWindow(), ibTextCtrl);
        ...
        if ( pos != -1 )
        {
            text->PerformAction(ibACTION_TEXT_ANCHOR_SEL, pos);
        }
    }
    else if ( event.LeftDClick() )
    {
        // select the word the cursor is on
        consumer->PerformAction(ibACTION_TEXT_SEL_WORD);
    }
...
bool ibStdTextCtrlInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                                const wxMouseEvent& event)
{
    if ( m_winCapture )
    {
        // track it
        ibTextCtrl *text = wxStaticCast(m_winCapture, ibTextCtrl);
        wxTextPos pos = HitTest(text, event.GetPosition());
        if ( pos != -1 )
        {
            text->PerformAction(ibACTION_TEXT_EXTEND_SEL, pos);
        }
    }
...
bool
ibStdTextCtrlInputHandler::HandleFocus(ibInputConsumer *consumer,
                                       const wxFocusEvent& event)
{
    ...
    // ...and so does the border colour (focused = accent highlight)
    text->RefreshBorder();
...
wxIMPLEMENT_DYNAMIC_CLASS(ibTextCtrl, ibControl);
