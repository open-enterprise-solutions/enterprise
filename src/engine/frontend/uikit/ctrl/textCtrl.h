...
class wxCaret;
class ibTextCtrlCommandProcessor;
...
#define ibACTION_TEXT_GOTO          wxT("goto")  // to pos in numArg
#define ibACTION_TEXT_FIRST         wxT("first") // go to pos 0
#define ibACTION_TEXT_LAST          wxT("last")  // go to last pos
#define ibACTION_TEXT_HOME          wxT("home")
#define ibACTION_TEXT_END           wxT("end")
#define ibACTION_TEXT_LEFT          wxT("left")
#define ibACTION_TEXT_RIGHT         wxT("right")
#define ibACTION_TEXT_UP            wxT("up")
#define ibACTION_TEXT_DOWN          wxT("down")
#define ibACTION_TEXT_WORD_LEFT     wxT("wordleft")
#define ibACTION_TEXT_WORD_RIGHT    wxT("wordright")
#define ibACTION_TEXT_PAGE_UP       wxT("pageup")
#define ibACTION_TEXT_PAGE_DOWN     wxT("pagedown")
#define ibACTION_TEXT_RETURN        wxT("return")

// clipboard operations
#define ibACTION_TEXT_COPY          wxT("copy")
#define ibACTION_TEXT_CUT           wxT("cut")
#define ibACTION_TEXT_PASTE         wxT("paste")

// insert text at the cursor position: the text is in strArg of PerformAction
#define ibACTION_TEXT_INSERT        wxT("insert")
...
#define ibACTION_TEXT_PREFIX_SEL    wxT("sel")
#define ibACTION_TEXT_PREFIX_DEL    wxT("del")

// mouse selection
#define ibACTION_TEXT_ANCHOR_SEL    wxT("anchorsel")
#define ibACTION_TEXT_EXTEND_SEL    wxT("extendsel")
#define ibACTION_TEXT_SEL_WORD      wxT("wordsel")
#define ibACTION_TEXT_SEL_LINE      wxT("linesel")

// undo or redo
#define ibACTION_TEXT_UNDO          wxT("undo")
#define ibACTION_TEXT_REDO          wxT("redo")

// ----------------------------------------------------------------------------
// ibTextCtrl
// ----------------------------------------------------------------------------

#include <wx/textctrl.h>      // wxTextCtrlIface, wxTE_* styles
#include "frontend/uikit/ctrl/control.h"

// SEAM vs univ: wxTextCtrlBase sits on the native wxControl in our build;
// wxTextCtrlIface carries the windowless text API (wxTextAreaBase +
// wxTextEntryBase), so the control derives from ibControl + the iface
class FRONTEND_API ibTextCtrl : public ibControl,
...
public:
    ...
    ibTextCtrl() : wxScrollHelper(this) { Init(); }

    ibTextCtrl(wxWindow *parent,
               wxWindowID id,
    ...
    // wxTextCtrlBase shims (see the SEAM note above)
    bool IsMultiLine() const { return (GetWindowStyle() & wxTE_MULTILINE) != 0; }
    bool IsSingleLine() const { return !IsMultiLine(); }

    // context menu (native bridge until the univ menu revives)
    void OnContextMenu(wxContextMenuEvent& event);
    void OnContextMenuCommand(wxCommandEvent& event);

    // wxTextAreaBase pure virtuals: univ has no rich-style support, same
    // honest no-ops wxTextCtrlBase used to provide
    virtual bool SetStyle(long WXUNUSED(start), long WXUNUSED(end),
        const wxTextAttr& WXUNUSED(style)) override { return false; }
    virtual bool GetStyle(long WXUNUSED(position),
        wxTextAttr& WXUNUSED(style)) override { return false; }
    virtual bool SetDefaultStyle(const wxTextAttr& WXUNUSED(style)) override { return false; }

    // wxTextEntryBase plumbing
    ...
    // only for ibStdTextCtrlInputHandler
...
public:
    // SEAM vs univ: the univ partial-blit scroll (ScrollNoRefresh on the
    // target rect) leaves stale rows with the buffered paint pipeline —
    // scroll by full repaint instead (the prepared-DC origin does the rest)
...
private:
    ...
    // password mask: while typing the mask is "live" (one asterisk per
    // character), and collapses to the fixed length after a short delay
    bool m_passwordMaskLive;
    wxTimer *m_passwordMaskTimer;
    ...
    ibTextCtrlCommandProcessor *m_cmdProcessor;

    wxDECLARE_EVENT_TABLE();
    wxDECLARE_DYNAMIC_CLASS(ibTextCtrl);
