...
// Purpose:     ibMenuItem class for wxUniversal
...
// ----------------------------------------------------------------------------
// ibMenuItem implements wxMenuItemBase
// ----------------------------------------------------------------------------

// SEAM vs univ: there wxMenuItem itself was the univ class, so the generic
// wxMenuBase machinery (wxMenuItemList, DoAppend(wxMenuItem*), GetSubMenu())
// trafficked in the same type. Here the native wxMenuItem still exists, so we
// derive from it to stay castable along a single chain; its native behaviour
// is dead weight — every univ-relevant virtual is overridden below and the
// items are only ever shown by the univ popup machinery, never by an HMENU.
class FRONTEND_API ibMenuItem : public wxMenuItem
{
public:
    // ctor & dtor
    ibMenuItem(ibMenu *parentMenu = nullptr,
               int id = wxID_SEPARATOR,
               const wxString& name = wxEmptyString,
               const wxString& help = wxEmptyString,
               wxItemKind kind = wxITEM_NORMAL,
               ibMenu *subMenu = nullptr);
    virtual ~ibMenuItem();
...
private:
    wxDECLARE_DYNAMIC_CLASS(ibMenuItem);
};
