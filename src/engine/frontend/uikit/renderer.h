...
// Purpose:     ibRenderer class declaration
...
/*
   ibRenderer class is used to draw all wxWidgets controls. This is an ABC and
   the look of the application is determined by the concrete derivation of
   ibRenderer used in the program.
   ...
   Finally note that ibRenderer supersedes ibRendererNative in wxUniv build and
...
// helper class used by ibMenu-related functions
class FRONTEND_API ibMenuGeometryInfo
{
public:
    // get the total size of the menu
    virtual wxSize GetSize() const = 0;

    // UIKIT-REVIVE: defined in univ menu.cpp originally; inline until menu revives
    virtual ~ibMenuGeometryInfo() = default;
};

// ----------------------------------------------------------------------------
// ibRenderer: abstract renderers interface
// ----------------------------------------------------------------------------

class FRONTEND_API ibRenderer : public wxDelegateRendererNative
{
public:
    // drawing functions
    // -----------------
    ...
    // draw an item of a ibListBox
    virtual void DrawItem(wxDC& dc,
                          const wxString& label,
                          const wxRect& rect,
                          int flags = 0) = 0;

    // draw an item of a ibCheckListBox
    ...
    virtual void DrawMenuSeparator(wxDC& dc,
                                   wxCoord y,
                                   const ibMenuGeometryInfo& geomInfo) = 0;
#endif // wxUSE_MENUS
    ...
    virtual void AdjustSize(wxSize *size, const ibWindow *window) = 0;
...
// ----------------------------------------------------------------------------
// ibDelegateRenderer: it is impossible to inherit from any of standard
...
class FRONTEND_API ibDelegateRenderer : public ibRenderer
{
public:
    ibDelegateRenderer(ibRenderer *renderer) : m_renderer(renderer) { }

    virtual void DrawBackground(wxDC& dc,
                                const wxColour& col,
                                const wxRect& rect,
                                int flags,
                                ibWindow *window = nullptr ) override
    ...
    virtual void AdjustSize(wxSize *size, const ibWindow *window) override
...
// ----------------------------------------------------------------------------
// ibControlRenderer: wraps the ibRenderer functions in a form easy to use from
// OnPaint()
// ----------------------------------------------------------------------------

class FRONTEND_API ibControlRenderer
{
public:
    // create a renderer for this dc with this "fundamental" renderer
    ibControlRenderer(ibWindow *control, wxDC& dc, ibRenderer *renderer);
...
#if wxUSE_LISTBOX
    void DrawItems(const ibListBox *listbox,
                   size_t itemFirst, size_t itemLast);
#endif // wxUSE_LISTBOX
#if wxUSE_CHECKLISTBOX
    void DrawCheckItems(const ibCheckListBox *listbox,
                        size_t itemFirst, size_t itemLast);
    ...
    void DrawScrollbar(const ibScrollBar *scrollbar, int thumbPosOld);
#if wxUSE_GAUGE
    void DrawProgressBar(const ibGauge *gauge);
#endif // wxUSE_GAUGE

    // accessors
    ibWindow *GetWindow() const { return m_window; }
...
private:
    ...
    void DoDrawItems(const ibListBox *listbox,
                     size_t itemFirst, size_t itemLast,
