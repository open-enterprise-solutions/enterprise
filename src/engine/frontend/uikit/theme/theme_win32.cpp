...
#if 1 // UIKIT-REVIVE: wxUSE_THEME_WIN32 is a univ-build define, always on here
...
class wxWin32Renderer : public ibThemeRenderer
{
public:
    // ctor
    wxWin32Renderer(const ibColourScheme *scheme);
    ...
    virtual void DrawMenuItem(wxDC& dc,
                              wxCoord y,
                              const ibMenuGeometryInfo& geometryInfo,
                              const wxString& label,
    ...
    virtual void DrawMenuSeparator(wxDC& dc,
                                   wxCoord y,
                                   const ibMenuGeometryInfo& geomInfo);
#endif // wxUSE_MENUS
    ...
    virtual void AdjustSize(wxSize *size, const ibWindow *window);
    ...
    virtual wxRect GetTextClientArea(const ibTextCtrl *text,
                                     const wxRect& rect,
    ...
    virtual ibMenuGeometryInfo *GetMenuGeometry(ibWindow *win,
                                                const ibMenu& menu) const;
#endif // wxUSE_MENUS

protected:
    // overridden ibThemeRenderer methods
                              ...
                              bool *left, bool *right)
    {
        ...
        {
            wxFAIL_MSG( "inconsistent ibSlider flags" );
...
class wxWin32InputHandler : public ibInputHandler
{
public:
    wxWin32InputHandler() { }

    virtual bool HandleKey(ibInputConsumer *control,
                           const wxKeyEvent& event,
                           bool pressed);
    virtual bool HandleMouse(ibInputConsumer *control,
                             const wxMouseEvent& event);
};

#if wxUSE_SCROLLBAR
class wxWin32ScrollBarInputHandler : public ibStdScrollBarInputHandler
{
public:
    wxWin32ScrollBarInputHandler(ibRenderer *renderer,
                                 ibInputHandler *handler);

    virtual bool HandleMouse(ibInputConsumer *control,
                             const wxMouseEvent& event);
    virtual bool HandleMouseMove(ibInputConsumer *control,
                                 const wxMouseEvent& event);
...
protected:
    virtual void Highlight(ibScrollBar * WXUNUSED(scrollbar),
                           bool WXUNUSED(doIt))
    {
...
#if wxUSE_CHECKBOX
class wxWin32CheckboxInputHandler : public ibStdInputHandler
{
public:
    wxWin32CheckboxInputHandler(ibInputHandler *handler)
...
#if wxUSE_TEXTCTRL
class wxWin32TextCtrlInputHandler : public ibStdInputHandler
{
public:
    wxWin32TextCtrlInputHandler(ibInputHandler *handler)
        : ibStdInputHandler(handler) { }
...
class wxWin32StatusBarInputHandler : public ibStdInputHandler
{
public:
    wxWin32StatusBarInputHandler(ibInputHandler *handler);
...
protected:
    ...
    bool IsOnGrip(ibWindow *statbar, const wxPoint& pt) const;
...
class wxWin32FrameInputHandler : public ibStdInputHandler
{
public:
    wxWin32FrameInputHandler(ibInputHandler *handler);
...
#if wxUSE_MENUS
    void PopupSystemMenu(ibTopLevelWindow *window) const;
#endif // wxUSE_MENUS
...
class wxWin32ColourScheme : public ibColourScheme
{
public:
    virtual wxColour Get(StdColour col) const;
    virtual wxColour GetBackground(ibWindow *win) const;
};
...
WX_DEFINE_ARRAY_PTR(ibInputHandler *, wxArrayHandlers);

class wxWin32Theme : public ibThemeEngine
{
public:
    wxWin32Theme();
...
ibRenderer *wxWin32Theme::GetRenderer()
{
    if ( !m_renderer )
    {
...
#if wxUSE_CHECKBOX
        else if ( control == ibINP_HANDLER_CHECKBOX )
        {
...
#endif // wxUSE_CHECKBOX
#if wxUSE_SCROLLBAR
        else if ( control == ibINP_HANDLER_SCROLLBAR )
        {
...
#endif // wxUSE_SCROLLBAR
#if wxUSE_STATUSBAR
        else if ( control == ibINP_HANDLER_STATUSBAR )
        {
...
#endif // wxUSE_STATUSBAR
#if wxUSE_TEXTCTRL
        else if ( control == ibINP_HANDLER_TEXTCTRL )
        {
...
ibColourScheme *wxWin32Theme::GetColourScheme()
{
    if ( !m_scheme )
    {
...
wxColour wxWin32ColourScheme::GetBackground(ibWindow *win) const
{
    wxColour col;
    if ( win->UseBgCol() )
    {
    ...
    {
#if wxUSE_TEXTCTRL
        ibTextCtrl *text = wxDynamicCast(win, ibTextCtrl);
#endif // wxUSE_TEXTCTRL
#if wxUSE_LISTBOX
        ibListBox* listBox = wxDynamicCast(win, ibListBox);
#endif // wxUSE_LISTBOX
    ...
    else
    {
        ...
        {
#if wxUSE_SCROLLBAR
            if ( wxDynamicCast(win, ibScrollBar) )
...
wxWin32Renderer::wxWin32Renderer(const ibColourScheme *scheme)
               : ibThemeRenderer(scheme)
{
    // init data
                                ...
                                wxRect *rectBounds)
{
    ...
    ibThemeRenderer::DrawLabel(dc, label, rect, flags, alignment,
                                         ...
                                         int indexAccel)
{
    ...
    ibThemeRenderer::DrawFrameWithLabel(dc, label2, rectFrame, rectText,
                                      ...
                                      wxRect *rectBounds)
{
    ...
    ibThemeRenderer::DrawButtonLabel(dc, label, image, rect, flags, alignment,
                                   indexAccel, rectBounds);
}
...
// wxWin32MenuGeometryInfo: the ibMenuGeometryInfo used by wxWin32Renderer
class FRONTEND_API wxWin32MenuGeometryInfo : public ibMenuGeometryInfo
{
public:
...
private:
    ...
    friend ibMenuGeometryInfo *
        wxWin32Renderer::GetMenuGeometry(ibWindow *, const ibMenu&) const;
};
...
ibMenuGeometryInfo *wxWin32Renderer::GetMenuGeometry(ibWindow *win,
                                                     const ibMenu& menu) const
{
    ...
    for ( ibMenuItemList::compatibility_iterator node = menu.GetMenuItems().GetFirst();
          node;
          node = node->GetNext() )
    {
        // height of this item
                                      ...
                                      int style)
{
    ...
    ibThemeRenderer::DrawStatusField(dc, rect, label, flags, style);
}
...
void wxWin32Renderer::DrawScrollbarShaft(wxDC& dc,
                                         ...
                                         int flags)
{
    ibColourScheme::StdColour col = flags & wxCONTROL_PRESSED
                                    ? ibColourScheme::SCROLLBAR_PRESSED
                                    : ibColourScheme::SCROLLBAR;
...
wxRect
wxWin32Renderer::GetTextTotalArea(const ibTextCtrl *text,
                                  const wxRect& rect) const
{
    wxRect rectTotal = ibThemeRenderer::GetTextTotalArea(text, rect);
...
wxRect
wxWin32Renderer::GetTextClientArea(const ibTextCtrl *text,
                                   const wxRect& rect,
                                   wxCoord *extraSpaceBeyond) const
{
    ...
    return ibThemeRenderer::GetTextClientArea(text, rect, extraSpaceBeyond);
}
...
void wxWin32Renderer::AdjustSize(wxSize *size, const ibWindow *window)
{
#if wxUSE_SCROLLBAR
    if ( wxDynamicCast(window, ibScrollBar) )
    {
        /*
...
#if wxUSE_BMPBUTTON
    if ( wxDynamicCast(window, ibBitmapButton) )
    {
        // do nothing
    } else
    ...
    if ( 0
#  if wxUSE_BUTTON
         || wxDynamicCast(window, ibButton)
#  endif // wxUSE_BUTTON
#  if wxUSE_TOGGLEBTN
         || wxDynamicCast(window, ibToggleButton)
#  endif // wxUSE_TOGGLEBTN
        )
    {
        ...
        if (wxSystemSettings::GetScreenType() > wxSYS_SCREEN_PDA)
        {
            if ( !(window->GetWindowStyle() & wxBU_EXACTFIT) )
            {
                wxSize szDef = ibButton::GetDefaultSize();
                if ( size->x < szDef.x )
                    size->x = szDef.x;
            }
        }
    ...
    ibThemeRenderer::AdjustSize(size, window);
}
...
bool wxWin32InputHandler::HandleKey(ibInputConsumer * WXUNUSED(control),
...
bool wxWin32InputHandler::HandleMouse(ibInputConsumer *control,
                                      const wxMouseEvent& event)
{
    // clicking on the control gives it focus
    if ( event.ButtonDown() )
    {
        ibWindow * const win = control->GetInputWindow();

        if ( win->CanAcceptFocus() && ibWindow::FindFocus() != win )
        {
            win->SetFocus();
...
wxWin32ScrollBarInputHandler::
wxWin32ScrollBarInputHandler(ibRenderer *renderer, ibInputHandler *handler)
        : ibStdScrollBarInputHandler(renderer, handler)
{
    m_scrollPaused = false;
    m_interval = 0;
}

bool wxWin32ScrollBarInputHandler::OnScrollTimer(ibScrollBar *scrollbar,
                                                 const ibControlAction& action)
{
    // stop if went beyond the position of the original click (this can only
    // happen when we scroll by pages)
    bool stop = false;
    if ( action == ibACTION_SCROLL_PAGE_DOWN )
    {
        stop = scrollbar->HitTestBar(m_ptStartScrolling) != wxHT_SCROLLBAR_BAR_2;
    }
    else if ( action == ibACTION_SCROLL_PAGE_UP )
    {
    ...
    return ibStdScrollBarInputHandler::OnScrollTimer(scrollbar, action);
}

bool wxWin32ScrollBarInputHandler::HandleMouse(ibInputConsumer *control,
                                               const wxMouseEvent& event)
{
    ...
    bool rc = ibStdScrollBarInputHandler::HandleMouse(control, event);
...
bool wxWin32ScrollBarInputHandler::HandleMouseMove(ibInputConsumer *control,
                                                   const wxMouseEvent& event)
{
    ...
    ibScrollBar *scrollbar = wxStaticCast(control->GetInputWindow(), ibScrollBar);
    wxHitTest ht;
    if ( m_scrollPaused )
    {
    ...
    return ibStdInputHandler::HandleMouseMove(control, event);
}
...
bool wxWin32CheckboxInputHandler::HandleKey(ibInputConsumer *control,
                                            const wxKeyEvent& event,
                                            bool pressed)
{
    if ( pressed )
    {
        ...
        {
            case WXK_SPACE:
                action = ibACTION_CHECKBOX_TOGGLE;
                break;

            case WXK_SUBTRACT:
            case WXK_NUMPAD_SUBTRACT:
                action = ibACTION_CHECKBOX_CHECK;
                break;

            case WXK_ADD:
            case WXK_NUMPAD_ADD:
            case WXK_NUMPAD_EQUAL:
                action = ibACTION_CHECKBOX_CLEAR;
                break;
        }
...
bool wxWin32TextCtrlInputHandler::HandleKey(ibInputConsumer *control,
                                            const wxKeyEvent& event,
                                            bool pressed)
{
    ...
    return ibStdInputHandler::HandleKey(control, event, pressed);
}
...
bool wxWin32StatusBarInputHandler::IsOnGrip(ibWindow *statbar,
                                            const wxPoint& pt) const
{
    ...
    {
        ibTopLevelWindow *
            parentTLW = wxDynamicCast(statbar->GetParent(), ibTopLevelWindow);
...
bool wxWin32StatusBarInputHandler::HandleMouse(ibInputConsumer *consumer,
                                               const wxMouseEvent& event)
{
    if ( event.Button(1) )
    {
        ...
        {
            ibWindow *statbar = consumer->GetInputWindow();

            if ( IsOnGrip(statbar, event.GetPosition()) )
            {
                ibTopLevelWindow *tlw = wxDynamicCast(statbar->GetParent(),
                                                      ibTopLevelWindow);
                if ( tlw )
                {
                    tlw->PerformAction(ibACTION_TOPLEVEL_RESIZE,
...
bool wxWin32StatusBarInputHandler::HandleMouseMove(ibInputConsumer *consumer,
                                                   const wxMouseEvent& event)
{
    ibWindow *statbar = consumer->GetInputWindow();
...
public:
    ...
    void Attach(ibInputConsumer *consumer);
    void Detach();
...
void wxWin32SystemMenuEvtHandler::Attach(ibInputConsumer *consumer)
{
    wxASSERT_MSG( m_wnd == nullptr, wxT("can't attach the handler twice!") );

    m_wnd = wxStaticCast(consumer->GetInputWindow(), ibTopLevelWindow);
...
{
    m_wnd->PerformAction(ibACTION_TOPLEVEL_BUTTON_CLICK,
...
wxWin32FrameInputHandler::wxWin32FrameInputHandler(ibInputHandler *handler)
                        : ibStdInputHandler(handler)
{
...
bool wxWin32FrameInputHandler::HandleMouse(ibInputConsumer *consumer,
                                           const wxMouseEvent& event)
{
    if ( event.LeftDClick() || event.LeftDown() || event.RightDown() )
    {
        ibTopLevelWindow *tlw =
            wxStaticCast(consumer->GetInputWindow(), ibTopLevelWindow);

        long hit = tlw->HitTest(event.GetPosition());

        if ( event.LeftDClick() && hit == wxHT_TOPLEVEL_TITLEBAR )
        {
            tlw->PerformAction(ibACTION_TOPLEVEL_BUTTON_CLICK,
...
void wxWin32FrameInputHandler::PopupSystemMenu(ibTopLevelWindow *window) const
{
    ibMenu menu;
...
bool wxWin32FrameInputHandler::HandleActivation(ibInputConsumer *consumer,
                                                bool activated)
{
    ...
    return ibStdInputHandler::HandleActivation(consumer, activated);
}
