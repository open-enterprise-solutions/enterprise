////////////////////////////////////////////////////////////////////////////
//	Author		: Maxim Kornienko
//	Description : chart of calculation types action
////////////////////////////////////////////////////////////////////////////

#include "chartOfCalculationTypes.h"

enum
{
	eDefActionAndClose = 1,
	eSave,
	eCopy,
	eGenerate,
	eMarkAsDelete,
};

ibValueRecordDataObjectChartOfCalculationTypes::ibStandardCommandSet ibValueRecordDataObjectChartOfCalculationTypes::GetStandardCommands(const ibFormID& formType)
{
	ibStandardCommandSet actions(this);
	actions.AddAction(wxT("SaveAndClose"), _("Save and close"), g_picSaveCLSID, true, eDefActionAndClose);
	actions.AddAction(wxT("Save"), _("Save"), g_picSaveCLSID, true, eSave);
	actions.AddSeparator();
	actions.AddAction(wxT("Generate"), _("Generate"), g_picGenerateCLSID, true, eGenerate).SetModify(false);   // creates a NEW object — doesn't modify THIS one
	actions.AddSeparator();
	actions.AddAction(wxT("Clone"), _("Clone"), g_picCloneCLSID, true, eCopy);
	return actions;
}

void ibValueRecordDataObjectChartOfCalculationTypes::CallAsAction(const ibActionID& action, ibBackendValueForm* srcForm)
{
	switch (action)
	{
	case eDefActionAndClose:
		if (WriteObject())
			srcForm->CloseForm();
		break;
	case eSave: WriteObject(); break;
	case eGenerate: Generate(); break;
	case eCopy: CopyObject(true); break;
	}
}
