...
public:
	...
	inline	bool		r_chunk(u64 ID, void* dest) const {	// ������ Chunk'�� (4b-ID,4b-size,??b-m_data)
		m_last_pos = tell();
	...
	inline	bool		r_chunk(u64 ID, wxMemoryBuffer& dest) const {	// ������ Chunk'�� (4b-ID,4b-size,??b-m_data)
		m_last_pos = tell();
		u32	dwSize = find_chunk(ID);
		if (dwSize != 0) {
	...
	inline	bool		r_chunk_safe(u64 ID, void* dest, u32 dest_size) const { // ������ Chunk'�� (4b-ID,4b-size,??b-m_data)
		m_last_pos = tell();
	...
	inline	bool		r_chunk_safe(u64 ID, wxMemoryBuffer& dest, u32 dest_size) const { // ������ Chunk'�� (4b-ID,4b-size,??b-m_data)
		m_last_pos = tell();
		u64	dwSize = find_chunk(ID);
		if (dwSize != 0) {
			wxASSERT(dwSize == dest_size);
