...
public:
	...
	wxString GetDescription() const override
	{
		return ibMcpText("RUN CODE IN THE STOPPED RUNTIME AND UNDO IT. Whole statements, not an "
			...
			"document, post it, read a register back, work out why a total is wrong.\n"
			"NOTE - IT NEEDS A STOP, AND THE STOP IS WHAT IT BUYS: the person's own frame, their locals, "
			"their session - and their session WAITING on you. Wanting to run code without any of "
			"that is `code_run`: it takes a session of its own, keeps nobody waiting, can COMMIT, "
			"and therefore has no frame to look at. Choose by whether anybody may be made to wait; "
