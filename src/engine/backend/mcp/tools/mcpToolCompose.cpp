	...
	wxString GetDescription() const override
	{
		return ibMcpText("ASK THE DATA SOMETHING AND READ THE ANSWER - with a report, with a composition "
			...
			"whole day of measurements taken the long way round while this verb was one call away.\n"
			  "\n"
			"🛑 A CROSS TABLE ANSWERS IN TWO PARTS, AND READING ONLY ONE OF THEM LOOKS LIKE A MISSING "
			"FEATURE. An output whose `shape` is `cross` carries `rows` - the side headings, down - "
			"AND `axis`, which is the COLUMN dimension: one entry per column heading, each with the "
			"figures under it and a `path` back to its parent totals. The column grouping shows as "
			"null in `rows` BY CONSTRUCTION, because in a cross the columns are not a field of the "
			"row. Read `rows` alone and the report reads as though the `columns: true` level had "
			...
			"as a condition and is not in the text.)\n"
			  "\n"
			"🛑 NOT A STOPWATCH FOR THE FORM'S BUTTON. The FIGURES here are the ones a person sees, "
			"but after the fold the ROAD is different: this tool lays the answer out with a driver of "
			"its own (a structure to read), while the report form writes a SPREADSHEET DOCUMENT - so how "
			"long this takes is not how long the person waits. Measured 2026-09-12 on a payroll sheet of "
			"120 thousand rows: 17 s of the person's 52 went into writing the document and none of them "
			"showed here, and a fold of 166 thousand nodes did not come back within this call at all "
			"while the document took 30 s. To time what a person waits for, compose into a document "
			"from code_run - `r = Reports.X.Create(); d = new SpreadsheetDocument(); r.Main.Compose(d);` "
			"- and read the journal line `composer: output '...' done - pass N ms, driver end M ms` "
			"(journal_read).\n"
			  "\n"
