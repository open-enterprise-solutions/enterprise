...
public:
	...
	option_t(const wxString& name, const int& l) :
		m_name(name), m_label(name), m_intVal(l)
	{
	}

	option_t(const wxString& name, const wxString& label, const int& l) :
...
public:
	...
	bool HasValue(const int& l) const {
		auto foundedIt = std::find_if(m_options.begin(), m_options.end(),
			[l](const option_t& opt) {return l == opt; }
		);
	...
	const std::vector< option_t >& GetOptions() {
		return m_options;
	}
};
...
public:
	...
	*/
	...
	virtual void OnEventChanged(Event* property) {}
