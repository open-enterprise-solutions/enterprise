...
public:
	...
	Option(const wxString& name, const int& l) :
		m_name(name), m_label(name), m_intVal(l)
	{
	}

	Option(const wxString& name, const wxString& label, const int& l) :
...
public:
	...
	bool HasValue(const int& l) const {
		auto foundedIt = std::find_if(m_options.begin(), m_options.end(),
			[l](const Option& opt) {return l == opt; }
		);
		if (foundedIt != m_options.end())
			return true;
		return false;
	}
...
private:
	...
	PropertyCategory(const wxString& name, const wxString& label, const wxString& helpString, IPropertyObject* object, PropertyCategory* ownerCat = NULL) :
		m_name(name),
		m_label(label),
...
public:
	...
	virtual bool IsOk() const {
		...
		if (m_type == PropertyType::PT_WXNAME ||
			...
			m_type == PropertyType::PT_WXSTRING_I18N ||
			m_type == PropertyType::PT_TEXT) {
			wxString value = m_value.GetString();
			if (value.IsEmpty())
				return false;
		}
		return true;
...
class PropertyAction : public Property {
	class PropertyFunctor {
	public:
		virtual OptionList* Invoke(PropertyAction* property) = 0;
	};
	template <typename optClass>
	class ObjectPropertyFunctor : public PropertyFunctor {
		OptionList* (optClass::* m_funcHandler)(PropertyAction*);
		optClass* m_handler;
	public:
		ObjectPropertyFunctor(OptionList* (optClass::* funcHandler)(PropertyAction*), optClass* handler)
			: m_funcHandler(funcHandler), m_handler(handler)
		{
		}
		virtual OptionList* Invoke(PropertyAction* property) override {
			return (m_handler->*m_funcHandler)(property);
		}
	};
...
private:
	...
	template <typename optClass>
	PropertyAction(const wxString& name, const wxString& label, const wxString& helpString,
		OptionList* (optClass::* funcHandler)(PropertyAction*), PropertyCategory* cat, const wxVariant& value = wxNullVariant)
		: Property(name, label, helpString, PropertyType::PT_ACTION_DATA, cat, value)
	{
		m_functor = new ObjectPropertyFunctor<optClass>(funcHandler, (optClass*)cat->GetObject());
	}
...
protected:
	...
	struct propertyName_t {
		...
		propertyName_t(const wxString& propName) : m_propName(propName) {}
		propertyName_t(const wxString& propName, const wxString& propLabel) : m_propName(propName), m_propLabel(propLabel) {}
		propertyName_t(const wxString& propName, const wxString& propLabel, const wxString& propHelpStr) : m_propName(propName), m_propLabel(propLabel), m_propHelpStr(propHelpStr) {}
		propertyName_t(const char* propName) : m_propName(propName) {}
	};
	...
	Property* CreateProperty(PropertyCategory* cat, const propertyName_t& name, PropertyType type, const wxVariant& defValue = wxNullVariant) {
		if (type == PropertyType::PT_TYPE_DATA ||
			type == PropertyType::PT_SOURCE_DATA ||
			type == PropertyType::PT_RECORD_DATA ||
			type == PropertyType::PT_OWNER_DATA ||
			type == PropertyType::PT_GENERATION_DATA)
			return NULL;
	...
	Property* CreateTypeProperty(PropertyCategory* cat, const propertyName_t& name) {
		return new Property(name.m_propName, name.m_propLabel, name.m_propHelpStr, PropertyType::PT_TYPE_DATA, cat);
	}

	Property* CreateSourceProperty(PropertyCategory* cat, const propertyName_t& name) {
		return new Property(name.m_propName, name.m_propLabel, name.m_propHelpStr, PropertyType::PT_SOURCE_DATA, cat);
	}

	Property* CreateRecordProperty(PropertyCategory* cat, const propertyName_t& name) {
		return new Property(name.m_propName, name.m_propLabel, name.m_propHelpStr, PropertyType::PT_RECORD_DATA, cat);
	}

	Property* CreateOwnerProperty(PropertyCategory* cat, const propertyName_t& name) {
		return new Property(name.m_propName, name.m_propLabel, name.m_propHelpStr, PropertyType::PT_OWNER_DATA, cat);
	}

	Property* CreateGenerationProperty(PropertyCategory* cat, const propertyName_t& name) {
		return new Property(name.m_propName, name.m_propLabel, name.m_propHelpStr, PropertyType::PT_GENERATION_DATA, cat);
	}
	...
	template <typename optClass>
	Property* CreateProperty(PropertyCategory* cat, const propertyName_t& name, OptionList* (optClass::* funcHandler)(PropertyAction*), const wxVariant& defValue = wxNullVariant) {
		return new PropertyAction(name.m_propName, name.m_propLabel, name.m_propHelpStr, funcHandler, cat, defValue);
	}
	...
	PropertyCategory* CreatePropertyCategory(const propertyName_t& name, PropertyCategory* ownerCat = NULL) {
...
public:
	...
	*/
	...
	IPropertyObject* GetParent() const { return m_parent; }
	...
	*/
	...
	//get form owner 
	virtual class CValueForm* GetOwnerForm() const {
		return NULL;
	}
