...
#include "formDesignerCMD.h"

wxIMPLEMENT_CLASS(CFormDocument, CDocument);

CCommandProcessor* CFormDocument::CreateCommandProcessor()
{
	CVisualEditorContextForm* contextForm = GetFormDesigner()->GetDesignerContext();
	CVisualDesignerCommandProcessor* commandProcessor = new CVisualDesignerCommandProcessor(this, contextForm);
	contextForm->SetCommandProcessor(commandProcessor);
...
{
	CFormEditView* designerForm = GetFormDesigner();
	wxASSERT(designerForm);
	designerForm->ActivateEditor();

	CCodeEditorCtrl* autoComplete = designerForm->GetCodeCtrl();
