...
#include "metadata/metaObjects/objects/catalogVariant.h"
#include "metadata/metaObjects/objects/documentVariant.h"
...
bool Property::SaveData(CMemoryWriter& writer)
{
	...
	case PropertyType::PT_WXSTRING_I18N:
	case PropertyType::PT_TEXT:
	case PropertyType::PT_ACTION_DATA:
...
IPropertyObject::~IPropertyObject()
{
	...
	CObjectInspector* objInspector = CObjectInspector::Get(false);
	if (objInspector != NULL) {
		if (pobj == objInspector->GetSelectedObject()) {
			objInspector->ClearProperty();
		}
	}
