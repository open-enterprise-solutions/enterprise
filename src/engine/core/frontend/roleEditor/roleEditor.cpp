...
{
	IObjectValueAbstract* singleObject = CValue::GetAvailableObject(g_metaCommonMetadataCLSID);
	wxASSERT(singleObject);

	wxImageList* imageList = m_roleCtrl->GetImageList();
	int imageIndex = imageList->Add(singleObject->GetClassIcon());
	m_treeMETADATA = m_roleCtrl->AddRoot(_("configuration"), imageIndex, imageIndex, new wxTreeItemMetaData(metadata->GetCommonMetaObject()));
...
void CRoleEditor::FillData()
{
	...
	wxASSERT(metaData);
	IMetaObject *commonObject = metaData->GetCommonMetaObject();
