...
CGrid::CGrid(wxWindow* parent,
	...
	m_gridProperty(new CGridPropertyObject(this)),
...
{
	bool hasFrozenRowGridWin = m_frozenRowGridWin != NULL;
	bool hasFrozenColGridWin = m_frozenColGridWin != NULL;
	bool hasFrozenCornerGridWin = m_frozenCornerGridWin != NULL;

	if (wxGrid::FreezeTo(row, col)) {
		if (!hasFrozenRowGridWin && m_frozenRowGridWin != NULL) {
		...
		if (!hasFrozenColGridWin && m_frozenColGridWin != NULL) {
		...
		if (!hasFrozenCornerGridWin && m_frozenCornerGridWin != NULL) {
...
void CGrid::OnMouseRightDown(wxGridEvent& event)
{
	...
	wxMenuItem* item = NULL; wxMenu menuPopup;
...
void CGrid::OnDelete(wxCommandEvent& event)
{
	...
	for (auto cell : wxGrid::GetSelectedBlocks()) {
		...
		int rangeFrom = 0, rangeTo = 0; CSectionCtrl* sectionCtrl = NULL;
		if (GetWorkSection(rangeFrom, rangeTo, sectionCtrl)) {
...
#include "frontend/windows/rowHeightWnd.h"
#include "frontend/windows/colHeightWnd.h"
