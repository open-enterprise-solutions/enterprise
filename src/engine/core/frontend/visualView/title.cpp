...
CTitle::CTitle(wxWindow *parent, const wxString &title) : wxPanel(parent, wxID_ANY)
{
...
wxWindow * CTitle::CreateTitle(wxWindow *inner, const wxString &title)
{
	wxWindow *parent = inner->GetParent();

	wxPanel *container = new wxPanel(parent, wxID_ANY);
	CTitle *titleWin = new CTitle(container, title);
