	...
	if (control->GetComponentType() == COMPONENT_TYPE_WINDOW) {
		...
		if (controlSizer != NULL) {
			...
			if (controlParent->GetClassName() == wxT("notebookPage")) {
				CPageWindow* pageWnd = dynamic_cast<CPageWindow*>(GetWxObject(controlParent));
			...
			if (controlParentSizer != NULL) {
				controlParentSizer->Detach(controlSizer);
			}
			delete controlSizer;
			if (!force && controlParentSizer != NULL) {
