...
wxDateTime CSystemObjects::s_workDate = wxDateTime::Now();
...
void CSystemObjects::PrepareNames() const
{
	...
	m_methodHelper->AppendFunc("singleMode", "singleMode()");
...
bool CSystemObjects::CallAsFunc(const long lMethodNum, CValue& pvarRetValue, CValue** paParams, const long lSizeArray)
{
	if (!appData->DesignerMode()) {
		...
		case enRound: pvarRetValue = Round(*paParams[0],
			...
			lSizeArray > 2 ?
			paParams[2]->ConvertToEnumType<eRoundMode>() : eRoundMode::eRoundMode_Round15as20
		); return true;
		...
		case enMessage:
			Message(paParams[0]->GetString(),
				lSizeArray > 1 ? paParams[1]->ConvertToEnumType<eStatusMessage>() : eStatusMessage::eStatusMessage_Information);
			return true;
		case enAlert: Alert(paParams[0]->GetString()); return true;
		case enQuestion: pvarRetValue = Question(paParams[0]->GetString(), paParams[1]->ConvertToEnumType<eQuestionMode>());
		case enSetStatus: SetStatus(paParams[0]->GetString()); return true;
		case enClearMessages: ClearMessages(); return true;
		...
		case enSingleMode: pvarRetValue = SingleMode(); return true;
		...
		case enGetCommonForm: pvarRetValue = GetCommonForm(
			...
			lSizeArray > 2 ? paParams[2]->ConvertToType<CValueGuid>() : NULL);
			return true;
