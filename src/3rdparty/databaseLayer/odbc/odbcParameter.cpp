...
// ctor
OdbcParameter::OdbcParameter() : m_nParameterType(OdbcParameter::PARAM_NULL)
{
	m_nBufferLength = SQL_NULL_DATA;
}

OdbcParameter::OdbcParameter(const wxString& strValue) : m_nParameterType(OdbcParameter::PARAM_STRING), m_strValue(strValue)
{
	m_nBufferLength = GetEncodedStreamLength(m_strValue);
}

OdbcParameter::OdbcParameter(const number_t& dblValue) : m_nParameterType(OdbcParameter::PARAM_NUMBER), m_numValue(dblValue)
{
	m_nBufferLength = 0;
}

OdbcParameter::OdbcParameter(int nValue) : m_nParameterType(OdbcParameter::PARAM_INT), m_nValue(nValue)
{
	m_nBufferLength = 0;
}

OdbcParameter::OdbcParameter(double dblValue) : m_nParameterType(OdbcParameter::PARAM_DOUBLE), m_dblValue(dblValue)
{
	m_nBufferLength = 0;
}

OdbcParameter::OdbcParameter(bool bValue) : m_nParameterType(OdbcParameter::PARAM_BOOL), m_bValue(bValue)
{
	m_nBufferLength = 0;
}

OdbcParameter::OdbcParameter(const wxDateTime& dateValue) : m_nParameterType(OdbcParameter::PARAM_DATETIME)
{
...
{
	if (m_nParameterType == OdbcParameter::PARAM_NULL) return 0;
	else return m_nBufferLength;
}

#ifdef _WIN64
SQLLEN* OdbcParameter::GetDataLengthPointer()
{
	if (m_nParameterType == OdbcParameter::PARAM_NULL) return &m_nBufferLength;
	else return NULL;
}
#else
...
void* OdbcParameter::GetDataPtr()
{
	...
	case OdbcParameter::PARAM_DOUBLE:
		pReturn = &m_dblValue;
		break;
	case OdbcParameter::PARAM_NUMBER:
		pReturn = &m_numValue;
		break;
...
SQLUINTEGER OdbcParameter::GetColumnSize()
{
	...
	{
	case OdbcParameter::PARAM_STRING:
	case OdbcParameter::PARAM_BLOB: nReturn = m_nBufferLength; break;
	case OdbcParameter::PARAM_INT:
	case OdbcParameter::PARAM_DOUBLE:
	case OdbcParameter::PARAM_NUMBER:
	case OdbcParameter::PARAM_BOOL:
	case OdbcParameter::PARAM_NULL: nReturn = 0; break;
	case OdbcParameter::PARAM_DATETIME: nReturn = 19; break;
...
#ifdef _WIN64
SQLLEN* OdbcParameter::GetParameterLengthPtr()
{
