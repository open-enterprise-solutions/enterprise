...
PostgresParameter::PostgresParameter(const wxString& strValue) : m_nParameterType(PostgresParameter::PARAM_STRING), m_strValue(strValue), m_nBufferLength(strValue.Length())
{
}

PostgresParameter::PostgresParameter(int nValue) : m_nParameterType(PostgresParameter::PARAM_INT)
{
	m_strValue = wxString::Format(_("%d"), nValue);
}

PostgresParameter::PostgresParameter(double dblValue) : m_nParameterType(PostgresParameter::PARAM_DOUBLE)
{
	m_strValue = wxString::Format(_("%f"), dblValue);
}

PostgresParameter::PostgresParameter(const number_t &dblValue) : m_nParameterType(PostgresParameter::PARAM_NUMBER)
{
...
PostgresParameter::PostgresParameter(const void* pData, long nDataLength) : m_nParameterType(PostgresParameter::PARAM_BLOB)
{
...
const void* PostgresParameter::GetDataPtr()
{
	...
	case PostgresParameter::PARAM_DOUBLE:
		...
		pReturn = m_CharBufferValue;
		break;
	case PostgresParameter::PARAM_NUMBER:
		//pReturn = &m_dblValue;
		m_CharBufferValue = ConvertToUnicodeStream(m_strValue);
		pReturn = m_CharBufferValue;
		break;
