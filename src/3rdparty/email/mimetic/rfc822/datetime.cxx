...
namespace mimetic
{
	...
	DateTime::Zone::Zone(int iZone)
		: m_iZone((short)iZone), m_iZoneIdx(0)
	{
		...
		: m_iZone(0), m_iZoneIdx(0), m_sZone(txt)
	{
		...
		{ // check if txt is a numeric timezone (+0200)
			...
			if (tz[0] == '+' || tz[0] == '-' || (tz[0] >= '0' && tz[0] <= '9'))
			{
				int sign = (tz[0] == '-' ? -1 : 1);
				tz.erase(0, 1);
				m_iZone = (short)(utils::str2int(tz) * sign);
			}
		}
	}
	...
	*/
	...
	{
		return (short)m_iYear;
	}

	short DateTime::hour() const
	{
		return (short)m_iHour;
	}

	short DateTime::minute() const
	{
		return (short)m_iMinute;
	}

	short DateTime::second() const
	{
		return (short)m_iSecond;
	}
	...
	{
		streamsize width = os.width(), fill = os.fill();
