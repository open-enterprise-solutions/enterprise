...
| A save re-positioned the list onto the saved element | ✅ landed 2026-08-03 — `NotifyChange` stamped `m_changedValue` and the list moved the cursor onto that element, yanking the user off the row they were on (an object form stays open while the list is browsed). The whole channel is REMOVED, not gated: the current row survives a refresh as a refcounted node and re-locates itself by row-key, so nothing needs to be told where it went. A register — whose key floats over its dimensions, so editing one REPLACES the record — now reports the move through `NotifyCreate` (`WriteRegister` compares the key composite across `SaveData`), which for the list is the truth: the old row is gone, this one is new. See [dynamic-list.md](dynamic-list.md) § "Selection restore" |
...
**What a save is allowed to move (2026-08-03).** Only an identity the list cannot find earns a
re-position: one that did not exist (a create) or one that MOVED (a register record whose dimension edit
replaced it under a new key). A plain re-write moves nothing — the current row is a refcounted node and
re-locates itself here, by row-key, through `m_pagedRestoreFocus`. The `changedValue` channel that used to
carry every save is gone; see [dynamic-list.md](dynamic-list.md) § "Selection restore".

**No highlight dropped ahead of the frame that replaces it (2026-08-03).** The same rule the wipe
obeys applies to the SELECTION, and it did not. `ibValueModelTableBox::ApplyCurrentLine` cleared
the highlight (`UnselectAllRows`) and only then called `Select(item)` — but on a paged model the
row is usually a stub not yet in the buffer, so `Select` merely stamps the restore channel and the
new highlight lands a whole query later, when the Reset answers. The interval in between showed the
old rows with NO current row at all: reported as "the row disappears for a second and then shows up
in its new place" after saving a new record. The eager clear is gone; `Select` drops the old
highlight itself, in the same call that puts the new one up, and only when the row is actually in
the buffer. For the paged case the old row keeps its highlight until `OnPagedFetchResetComplete`
swaps both inside the frozen frame. `SetCurrentItem` went with it: under `wxDV_SINGLE` it IS
`Select` (`ibDataViewCtrlBase::SetCurrentItem`), so it repeated the work and could stamp the restore
channel a second time — which, per **Restore precedence** above, bumps the generation and costs an
extra round-trip. Only `EnsureVisible` remains under `focus`. One behaviour changed with it: under
`RowSelectionMode = multiple` a programmatic current-line no longer clears the user's other
selections, matching how a Ctrl-click behaves.
