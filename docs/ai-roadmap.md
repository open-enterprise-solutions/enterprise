...
- [ ] **in_progress** t1-004: BAS / 1С → OES migration tool — `import_bas_xml` + `import_bas_cf` MCP tools. Reference corpus: BAS Бухгалтерія 2.1 UA (3200+ метаобъектов) + OES_DEMO. Mapping: Справочник→Catalog, Документ→Document, типы реквизитов, табчасти, движения по регистрам, ChartOfAccounts/ChartOfCharacteristicTypes привязки, EnterpriseData XSD reverse-engineered from sample выгрузки. Output: same mutations[] shape as `oes_template_customize` — reuse template wizard infrastructure для preview/apply. Sigma RAG provides auto-mapping suggestions per object. ~3 weeks.
...
- [ ] **pending** t2-006: Transactional staging — `transaction_begin/commit/rollback` MCP tools wrapping metaBridge undo stack. ~1 week.
...
- [ ] **pending** t2-020: Tool name aliases — add aliases so agents trained on mcp-1c work on oes-mcp (e.g. `get_metadata_tree` → alias to `list_objects`, `get_object_structure` → `meta_query`, `get_configuration_info` → `config_info`). Cross-tool compatibility. ~1 day.
