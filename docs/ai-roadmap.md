...
- [x] **done** t1-002: Form layout authoring through MCP — `form_layout_set` validates JSON control trees and writes backend-safe XML sidecars under `.oes/form-layouts`, `form_layout_read` returns sidecar layouts as structured JSON, and the legacy Designer binary blob stays untouched/front-end-owned. Serializer keeps `OES_E_FORM_BLOB_GUI_DEPENDENCY` for native binary blobs, but MCP authoring now has a functional DSL path. 13 form-layout gtests + MCP smoke PASS.
...
- [x] **done** t3-010: Security audit MCP tool — new read-only `security_audit` scans all modules or one module with OES-SEC-11 rules (SQL execution, dynamic execution, plaintext HTTP, credential literals, broad catch, deserialize/import, process launch, temp paths, debug output, permission overrides, security TODOs) and returns structured findings + PASS/WARN/BLOCK verdict.
