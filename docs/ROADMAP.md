...
| **CI** | None | `.github/workflows/ci.yml` — **four** jobs: Linux suite, Windows build+suite, GUI under Xvfb, and macOS 14 arm64 (added 2026-08-03). Every job also **links the applications**, which no job did before — the first run that tried failed on both platforms at once (`codeRunner` had 21 unresolved externals, its CMake file having never listed `frontend`). State on 2026-08-03: Linux and Windows **919/919**, GUI **26/26**; macOS is the fastest job of the three and is working through the six defects its toolchain surfaced. See [portability.md § 3](portability.md) |
