...
| Query engine — window functions | [query-engine-layers.md](query-engine-layers.md) § L2-1 | landed 2026-08-20 — `ibQueryExpr::m_over` + one `ibRenderOverClause` (both L2 renderers spell the clause through it), frame an explicit choice, refused on an engine without windows. **Two consumers**: the information register's slice (one ranked pass, was three visits to the table) and the periodised `BalanceAndTurnovers` of BOTH registers (the running balance that used to fall to RAM). ⚠ Numbers not yet verified against a live engine — the RAM road is the oracle |
| Query language — result links + `WITH` | [query-language-arc.md](query-language-arc.md) § 24.4 / § 24.4a | 2026-08-21 — `ONTO <name>` names a result; a later statement reads it by naming it, and that IS the link (no data sets, no link editor). **Both roads exist and the LOWERING chooses**: where the engine reads a named query it is declared (`WITH … AS (…)`, `ibQueryCte` on the IR, `ibCteQueryable` as the source) and the server does the join; otherwise its rows are read and the join is ours — which is what the first cut did for every engine, silently. Constructor: a tab of its own for links between named results ([query-constructor.md](query-constructor.md) §8a). **The links belong to the PACKAGE** (2026-08-22, Max's own shaping after three rejected forms): a separate list (`ibQueryPackage::m_links`), spelled `JOIN A AND B ON …` — recognised by POSITION, no new keyword — and the package's final query is ASSEMBLED from them at execution time. Adding a link adds no source, no statement and no temp table. ⚠ The declared road is **not yet verified against a live engine** — no integration test runs a two-name link end to end |
| Reports — totals where a reader looks for them | [report-engine.md](report-engine.md) § 6bb | 2026-08-22 (settled) — every level's figures were always computed (`Sum/Count/Min/Max/Avg` at every node of the fold); the question was only the PLACE, and the answer is **a group is ONE row — its name and its figures**. A per-group `Total …` line was tried and reverted: our heading already carries the resources, so the line below repeated it word for word. Only the GRAND total stands alone, at the bottom of the section. It is the ROOT of the folded tree (`ApplyAggregates(tree.Root(), …)` — nothing is computed twice); `OVERALL` turned out to be a **walk** setting, so the reader asks for it in its own words: `ibSelector::WalkOverall()`, raised by `ibCompositionDriver::WantsGrandTotal()` (true for the report printer, false for a list's fetch). Pre-order hands the root over FIRST — the driver holds it and writes it LAST. ⚠ Fixed on the way: a `GROUP BY ROLLUP` tree never set `m_hasChildren` on any node, so a server-folded report printed its headings as ordinary rows |
| Reports — a resource's name is made unique | [query-language-arc.md](query-language-arc.md) § resource naming | 2026-08-22 — `SUM(Amount)` reads back as `Amount`, which put TWO columns under one name whenever the report also grouped by that field, and made `TOTALS SUM(x), COUNT(x)` print one figure twice (the fold rolls in place, into the INPUT column's slot). The later claimant is qualified by its function (`SUMAmount`); a repeated input is projected again under a synthetic alias, so it gets a column — and a slot — of its own |
| Reports — "available fields" actually narrow | [data-composer.md](data-composer.md) § available and selected | 2026-08-22 — the set was written, saved and consulted by NOTHING. It now narrows every field pane of the composer window through `ibSettingsFieldTree::SetVisibleTest`, per SELECTED node, with the same report → output → level inheritance (`AvailableFor`). A road (a reference) is never hidden — only leaves. Fixed with it: the REPORT-level set was neither loaded nor written back, so edits on the report row went nowhere |
| Reports — detail records | [data-composer.md](data-composer.md) § detail records | 2026-08-21 — "a detail record is an empty grouping": a level of the ladder with no fields (`ibCompositionLevelKind::Details`), one node per source row under the deepest heading, resources rolled over that one row. Asking for them REFUSES the server-side fold (ROLLUP leaves no rows to hang) and adds the SELECTed fields to the totals schema so a row prints more than its figures. Added through a FORM — one verb makes a level, an empty field list makes it the detail one. A level's own filter and sort now SAVE (the tree is kept, the expression derived). ⚠ Not yet run live |
| Reports — saving a report is accepting it | [report-engine.md](report-engine.md) § 4b | 2026-08-22 — the composer panel edits into a TRANSACTIONAL BUFFER and landed it in ONE place: the view's `OnClose` ("closing the tab is accepting it"). Saving is the second way of accepting and it asked nobody, so an external report written from its own tree carried the composition as it stood BEFORE the grouping was added. `ibComposerEditView::Commit()` is the verb, `ibCommitOpenComposers()` asks EVERY open tab (the diskette is pressed on the tree while the composer sits beside it), `ibComposerDocument::Save()` makes the same house call the module and form editors make. ⚠ Three VERDICTS on that path were wrong: `ibDataReportTree::Save()` answers false for "nothing to save" and both file documents read it as failure, so an UNCHANGED external report could not be written at all; and `ibMetaDocument::Save()` read `SaveDatabase()` inverted **both ways round** — a save that succeeded reported as failed, one that failed fell through to `SaveAs()` (true for a metaobject document) and reported as success. The write always happened; only the verdict lied |
| Reports — "selected fields" is writable again | [data-composer.md](data-composer.md) § available and selected | 2026-08-22 — `SelectedFor(output[, level])` composes the query's SELECT list and the window had no page to write it, so every report composed from the window projected **every column its source has**. The page had been taken off as "two nearly identical lists"; answered by stating what each one ASKS (Available narrows the pickers, Selected is what is read) rather than by hiding one. The plumbing was already generic — one `AddPage` was missing |
| Reports — a variant carries its parameters | [report-engine.md](report-engine.md) § 4b variants | 2026-08-22 — a snapshot held the flat settings and the structure, so switching a variant changed the groupings and left the period behind. It now carries what a person SET (value + expression), BY NAME and merged over the list the query declares — never replacing it, since that list is derived from the text. Fixed with it: `AddVariant(copyFrom)` copied only the flat settings, so a copy came back with no outputs, no levels and no parameters |
| Reports — detail records need no keyword, ever | [data-composer.md](data-composer.md) § detail records | 2026-08-22 (settled) — the word would name ONE setting of a dial that is continuous: descending from a group may go by another field, and going by *nothing* is what a detail record IS. The read carries the whole dial already (`ibSelectKind`, `withDetails`). The **empty grouping the user adds stays** — it is the declaration; what changes is its LOWERING. ⏭ Direction, not built: a PHANTOM LEVEL — the row's own uniqueness appended to `GROUP BY ROLLUP(…)` — takes detail records to the server instead of refusing the fold |
| ⚠ Server-side TOTALS fold — reaches the engine now, and STILL cannot fire from a report | [data-composer.md](data-composer.md) § the phantom level | 2026-08-22, one day, **four silent refusals stacked on each other** — and a queue of silent refusals is indistinguishable from one, so each was visible only once the one above it was lifted. **(1) The gate read the wrong lists** — the whole ROLLUP tier had never executed once. The door that folds totals fills `m_totals` / `m_totalAggregates` (`TotalByLevel` + `Totals()`) and never touches `m_groupBy` / `m_aggregates`; the gate read the second pair, so `CanRollupTotalsShape` refused on `m_groupBy->empty()` for every totals query the composer produces, and the other door (`SelectTotals()`) has no callers at all. The gate tests built the spec BY HAND (`buf.groupBy = {&aDim}`) — green about a path the builder never produces, which is also why the `MarkRollupFolders` symptom was written as "would be". Now: the spec carries the door's own two lists, `RollupLevelsOf` / `RollupAggregatesOf` answer in one place, and **the ROLLUP element is one per LEVEL** — a tuple level (`BY (a, b)`) is one composite element, and the parent key is the key minus the last LEVEL, not minus one value. ⚠ Three refusals that a flat column list could not even state are now made: a detail level, a HIERARCHY unfold, a dot-walked dimension — each a silently wrong answer if missed; plus a sort naming anything but a level field (invalid SQL over a grouped result). **(2) The POLICY decided the road** — `TryTotalsPushdown` answered false on the mere PRESENCE of an access policy, and the door's default ctor pulls the session's, so in enterprise mode the fold could not fire for ANY report, at any setting, on any engine. Fixed the way its neighbours already did it: ask `CheckSelect` ONCE — it both authorises and NARROWS, writing the policy's conditions into the query — then read plainly on the guarded copy. A policy divides DATA; it never chooses roads. **(3) An empty FROM** — the fold built `ibScan(name)` unconditionally and never asked `GetSourceRelation(alias)`, which is how a source that is not a plain table (a register's Balance / Turnover, whose date cannot live in a view) says what it is read from. Forced open, it rendered `FROM  GROUP BY ROLLUP(…)`, and Firebird answered `Token unknown - line 1, column 219: GROUP`. The engine was right; the statement was ours. **(4) And the one that still stands: A NESTED QUERY IS A RAM TABLE.** `ibSubqueryQueryable::IsComputedInRam()` is true, and a composition's source is ALWAYS query text (`FromText`), which becomes a nested source the moment it carries settings — so **no report can push its fold to the server at all**, structurally. That is a precondition for the compute-server stage, not an optimisation. 🔍 **Firebird has NEITHER ROLLUP NOR GROUPING**, and the road to that answer is the part worth keeping: the engine's keyword table carries PARTITION / OVER / WINDOW / MERGE / RETURNING / LATERAL and neither of these; a live run then answered `-804 Function unknown: GROUPING`, which was briefly read as "so ROLLUP was accepted" — and that inference is WRONG, because `ROLLUP(x)` parses as an ordinary FUNCTION CALL when the word is unknown, so the error code cannot tell a missing keyword from a missing function, and name resolution reports the FIRST unknown it meets. FirebirdSQL PR #9029 adds all of it (ROLLUP / CUBE / GROUPING SETS / GROUPING / GROUPING_ID) and is **open, not merged**. ⭐ Landed with it: `ibSqlFeatures::m_grouping` asked apart from `m_rollup` (they do not arrive together), and where GROUPING is missing the fold reads a row's level off its keys being SQL NULL — exact here, because OES attributes hold typed empties and never SQL NULL, so a NULL can only have come from the fold. Dot-walked DIMENSIONS landed with it too (`ibTotalField` carries its own path; the SQL names the joined LEAF while the tree keys on the dimension's own synthetic id — two columns, one field). ⏭ Next, in order: **a nested query as a server-side DERIVED TABLE** (without it the three fixes above cannot be reached from any report), then the phantom level |
| Query language — `OVER (…)` READ, not run | [query-language-arc.md](query-language-arc.md) § 23.3 | 2026-08-20 — lexer / AST / parser / renderer / round trip. The **lowering refuses** (`RefuseUnloweredWindow`): L4→L3 carries no window yet (`ibAggregateItem` has no field, five call sites in `dbTableProvider` build from it). Dropping the OVER would run and report a plain total under a running one's name |
...
| The GUI harness silenced wxASSERT and the CRT reports, but left the **log target** alone | **The GUI job hung on every run and died at 600 s (exit 137)** — a GUI `wxApp` installs `wxLogGui`, which BUFFERS warnings and pours them into a modal dialog when `wxApp::CleanUp` takes the target down, i.e. inside `wxEntryCleanup` AFTER the last test passed. Green suite, no exit. One warning is enough; ours was libpng's `iCCP` on six embedded icons, 21 times | `wxLogStderr` is installed right after `wxEntryStart` — headless, any deferred flush in a GUI build IS a modal window, so the rule now covers the log and not just asserts. The `iCCP` chunk was also dropped from the six PNGs (ancillary, own CRC → no re-encode; pixel-identical, 128 KB → 29 KB). Separately, the step ran under `bash -e`, where `wait "$pid"; status=$?` aborts at the `wait` — so the run that finally produced a stack printed no log and skipped its own control run. `\|\| status=$?` now |
...
| **Access policy / RLS** | [access-policy-rls.md](access-policy-rls.md) | read + write live; semi-join / temp-promote landed. **2026-08-08: a role now declares HOW IT COMBINES** (`ibRoleCompositionMode`, a runtime enumeration on the role — Union / Intersection, labelled *Permitting* / *Restricting*). Union roles OR as before; an intersection role grants nothing and SUBTRACTS, so its restriction holds whatever the others admitted and no further role can widen past it — which is what a data separator is, declared once instead of copied into every role (N+M roles instead of N×M). The verdict is `(union1 OR union2 …) AND intersection1 AND intersection2 …`, so the ORDER roles are assigned in cannot change the answer. The same mode folds the RIGHTS on the object (`ibAccessObject::AccessRight`, two passes over one list) and needed no new stored state there: a right's value is kept only where it was flipped, so "this role never said anything" was already expressible. Default is Union and every right defaults to allowed, so a tree with no restricting role behaves exactly as before. Open: **compiles and starts, but the fold itself is unverified** — no test over the two buckets, not yet run under a live role; the mode is stored in the membership (sys_user chunk), so changing a role's mode reaches an already-assigned user only when that user is saved again |
...
| **Metadata test coverage** | — | ⚠️ NEXT STEP, and the arc of 2026-08-06 is the argument for it. Every defect that day was a metatype missing from a list somebody kept by hand — the branch that could not create, the tree that did not fill, the runtime list the form is built from, a `ResolveChild` disagreeing with its own attribute lists. Shape: ONE TABLE of metatypes (11 business + `SessionParameter` / `CommonAttribute` / `PredefinedJob` / `ScheduledJob` / `AccountDimensionKindsTable` / `ConstantValueColumn`) × five questions asked of each — creates under the root and enumerates, survives a save/load round trip, `ResolveChild` agrees with the attribute lists, builds a schema snapshot, appears in the designer tree. A new metatype is then covered by adding a row, not a file. Started: `tests/test_commonAttribute.cpp` (10 cases over the composition mechanism, **not yet run** — the CMake test build needs the new sources). Harder and more valuable: a live-DB fixture for restructuring (as `test_totalsNumericParity` does on in-memory SQLite) — apply, fail mid-way, assert the schema came back |
...
### 4.1 ~~Accounting registers — DB execution disabled~~ RESOLVED 2026-08-13

`accountingRegisterManager_impl.cpp` used to be 1039 lines of hand-composed SQL with the execution
half of `Balance` / `Turnovers` / `DrCrTurnovers` / `BalanceAndTurnovers` under `#if 0` — SQL built,
never run, an empty table returned and nothing reported. That path is **deleted, not revived**: raw
L1 bypasses the access policy, the dialect layer and paging, and enabling a dead path opens
territory rather than closing it.

The register now reads through the same L3 door as its neighbours and declares its own totals
bundle. Current state and what is still open: **§2, row *Accounting register***, and
[accounting-register-arc.md](accounting-register-arc.md) § 5e / § 8. The decisions behind it were
inventoried on 2026-08-10 in the same arc (naming `AccountDimension`; the slot count declared by the
chart of accounts; correspondence as a register property; the kind stored beside the value;
addressing by kind; a per-call output schema).
...
2. ~~**Accounting execution**~~ — **done 2026-08-13** (§4.1, and the §2 row): the register reads,
   writes and maintains its own totals. What is left there is listed in
   [accounting-register-arc.md § 8](accounting-register-arc.md) and is narrower than the migration
   was — the accounting flags, the document half of a boundary, and the move of the readings onto
   the server.
