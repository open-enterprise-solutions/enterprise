...
| ⚠ Server-side TOTALS fold — looks UNREACHABLE from the composer | [data-composer.md](data-composer.md) § the phantom level | 2026-08-22, **read not run**: `queryLowering::ExecuteTotals`'s paged branch returns early, and the path after it (the `TotalByLevel` loop) never calls `GroupBy`; no other verb fills `m_groupBy`. `CanRollupTotalsShape` refuses on `spec.m_groupBy->empty()`, so `GROUP BY ROLLUP` cannot fire for a plain `SELECT … TOTALS SUM(x) BY Warehouse`. The other door, `SelectTotals()`, has NO callers. The gate tests build the spec by hand (`buf.groupBy = {&aDim}`), which is why the suite is green about a path the builder never produces — and why the `MarkRollupFolders` symptom was written as "would be". Step ONE of the phantom-level arc is to make the fold reachable by reading `m_totals` (which carries a tuple level) instead of `m_groupBy` (which cannot) |
