...
| ✅ Server-side TOTALS fold — was UNREACHABLE, now runs | [data-composer.md](data-composer.md) § the phantom level | 2026-08-22 — **the whole ROLLUP tier had never executed once.** The door that folds totals fills `m_totals` / `m_totalAggregates` (`TotalByLevel` + `Totals()`) and never touches `m_groupBy` / `m_aggregates`; the gate read the second pair, so `CanRollupTotalsShape` refused on `m_groupBy->empty()` for every totals query the composer produces, and the other door (`SelectTotals()`) has no callers at all. The gate tests built the spec BY HAND (`buf.groupBy = {&aDim}`) — green about a path the builder never produces, which is also why the `MarkRollupFolders` symptom was written as "would be". Now: the spec carries the door's own two lists, `RollupLevelsOf` / `RollupAggregatesOf` answer in one place, and **the ROLLUP element is one per LEVEL** — a tuple level (`BY (a, b)`) is one composite element, and the parent key is the key minus the last LEVEL, not minus one value. ⚠ Three refusals that a flat column list could not even state are now made: a detail level, a HIERARCHY unfold, a dot-walked dimension — each a silently wrong answer if missed; plus a sort naming anything but a level field (invalid SQL over a grouped result). **Verified LIVE 2026-08-22** on Firebird (the dialect advertises `m_rollup`): `Ref` → `(DeletionMark, Number)` → `Posted` over 62 documents, grand total and nesting identical to the RAM road, tuple level printed as ONE heading, expanders present — the first time `MarkRollupFolders` ever ran. ⏭ Next: dot-walked dimensions (the join chain exists for flat keys, not for level fields), then the phantom level |
