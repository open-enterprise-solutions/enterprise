...
- VES (Visual-Basic-style ES, same family as VBScript / VB6 — keyword-fenced `If … Then … EndIf`, `Foreach … Do … EndDo`, `End…`) is kept for migration; it is the same parser, just a different lexer mode.
...
> ⚠ **Removed after v1.3.0.** `metadataConfigurationXML.cpp` / `metadataConfigurationJSON.cpp` and
> the four `Save/LoadConfigTo/FromXML|JSON` methods no longer exist. The replacement is the
> `ibDataNode` + `ibFormatProvider` model, in which `ibJsonProvider` is **write-only** (export for
> diff / inspection; `Read` is a no-op). Accurate for the tag, not for current `develop`.
...
- Optional DB driver toggles: `OES_USE_FIREBIRD`, `OES_USE_POSTGRESQL`, `OES_USE_MYSQL`, `OES_USE_ODBC` (SQLite is always embedded — no flag).
...
- Self-contained: zero `#include` of ttmath; Add / Sub / Mul + base-2 long-division Div implemented as schoolbook routines in `backend/fnumber.cpp`. MSVC x86/x64 use `_addcarry_u32` / `_subborrow_u32` intrinsics; portable 64-bit fallback for Clang / GCC / ARM.
...
`Verify` additionally accepts legacy 32-hex MD5 hashes from pre-migration databases; callers use `NeedsRehash` + `Hash` to upgrade silently on successful login (`ibApplicationData::AuthenticateUser`). MD5 stays in-tree for metadata integrity (`ibMD5::ComputeMd5`) — **never** reuse it for passwords.
...
- Documentation pass: docs are English-only; references to peer enterprise platforms (Mendix, OutSystems, ERPNext, Salesforce Lightning, etc.) are framed as comparable products, not parents.
