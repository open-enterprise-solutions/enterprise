...
> 🛑 **A new `virtual` in a class the two DLLs share needs a FULL rebuild** (paid for
> 2026-08-26). A virtual added **in the middle** of a class shifts every slot after it in the
> vtable; whoever was not recompiled keeps calling by the old index and lands in a different
> method. What that looks like is **not** a link error and not a message — it is an access
> violation whose frame is nonsense: `ebp` holding a code address rather than a stack one, a
> `kb` of repeating frames four bytes apart. Adding `ibDocument::GetSaveFilter()` after
> `SaveAs()` and building incrementally crashed the client in `InitDocument`.
>
> Two rules follow. **Rebuild the solution** (`/t:Rebuild`) after touching a virtual in
> `backend`/`frontend` public headers — MSBuild tracks header dependencies, but nothing checks
> that a stale `.dll` from an earlier run is still in `bin\`. And when the addition is optional,
> **put it at the END of the class**, where it only grows the table instead of renumbering it.

Firebird embedded libraries (`fbclient.dll`,
...
Every configuration also gets `$(OesDriverDefines)` — `OES_USE_FIREBIRD;OES_USE_POSTGRESQL;OES_USE_ODBC`,
declared once at the top of the same file, so which drivers are compiled in is one statement rather
than six. ⚠ A project only receives it if its own `ItemDefinitionGroup` keeps the
`%(PreprocessorDefinitions)` token: without it MSBuild does not merge what the project inherits.
Three projects had dropped the token from their `Release` groups, and a Release build came out with
**not one** `OES_USE_*` macro defined — no Firebird, no PostgreSQL, no ODBC — which surfaced as a
file base that would not open and could not say why (`#ifndef OES_USE_FIREBIRD → return false` is a
bare `false`: no exception, no error chain, no code). Fixed 2026-08-15; keep the token.
...
The three general jobs (Linux, Windows, macOS) each also run a **Build the applications** step.
Until 2026-08-03 CI built only test
...
Five notes on why it is shaped this way — each one paid for by a wasted round:
...
- **A failing `bin/…tests.exe` step is often the LISTING, not a test.** `gtest_discover_tests`
  runs each freshly linked executable to enumerate its cases, and CMake allows it 5 seconds by
  default; overrun that and the *build* fails with empty output and `Process terminated due to
  timeout`. It happened on 2026-08-26 to `oes_string_tests`, which links nothing of ours beyond
  wxBase. The budget is for the machine rather than the program — the same listing takes 0.02 s
...
### Nightly release (added 2026-09-17)

`.github/workflows/nightly.yml` builds **Release on the three platforms every night at 23:00 UTC**
(02:00 in Kyiv in summer) — only when `develop` moved since the last nightly — and publishes them as
one pre-release under the tag `nightly`, replaced each time. Users take it from one stable address,
`https://github.com/open-enterprise-solutions/enterprise/releases/tag/nightly`, or the files directly
under `…/releases/download/nightly/`:

| File | Built on | What the user needs |
|---|---|---|
| `oes-nightly-windows-x64.zip` | windows-2022, `windows-x64-release` | Nothing: Firebird (`_fb/`), the PostgreSQL client, the MSVC runtime and the translations are inside. |
| `oes-nightly-linux-x64.tar.gz` | ubuntu-22.04, `linux-release` | Ubuntu 22.04 / Debian 12 or newer, GTK 3; `libpq` for a server base. Firebird 5 is inside. |
| `oes-nightly-macos-arm64.dmg` | macos-14, `macos-release` | Apple Silicon. Firebird 5 is inside. Signed ad hoc only, so every file arrives quarantined: once, after dragging the folder to Applications, `find /Applications/OES -exec xattr -d com.apple.quarantine {} \; 2>/dev/null`. Right-click → Open is not enough — it lets one app through while the launcher starts the others — and macOS 15 no longer offers it. |

**Firebird 5 on Linux and macOS comes from the official release, pinned in the workflow** (5.0.4, the
URLs hard-coded): no distribution ships version 5 (Ubuntu 22.04 and Debian 12 have 3, which cannot open
these bases), so the job downloads it before packing and lays it out in `_fb/` as Firebird's own tree —
the Linux tarball's `opt/firebird`, the macOS package's `Firebird.framework/Resources` (unpacked with
`pkgutil --expand-full`, nothing installed). Left out as in the Windows kit: the profiler, trace and UDR
plugins; `firebird.conf` is the Windows kit's own (SuperClassic). Linux adds the one library the kit
takes from the system and Ubuntu does not install by default, `libtommath.so.1`, from the runner's
...
the job fails instead of publishing it.

What had to change for a CMake tree to be the same as the Visual Studio one, found while doing it:

- **The database clients** — `backend.vcxproj` copies the Firebird kit into `_fb/` (renaming the `_x64`
  files, leaving out `default_profiler`, `fbtrace`, `udr_engine`) and libpq with OpenSSL beside the
  binaries; CMake copied nothing, so a CMake-built Windows tree opened no file base at all. The backend's
  post-build step now copies the same files under the same names. ⚠ OpenSSL keeps its `-x64` names on
  x64 — that is what the x64 libpq imports — and only x86 renames the pair.
- **The stack** — `Common.props` reserves 8 MB (`/STACK:8388608`, measured: 1 MB killed the process at a
  script depth of 75); the root `CMakeLists.txt` now says the same for MSVC.
- **`backend.conf`** is copied beside the binaries, as the vcxproj does.
- **`plugins/`** — the plugin's plain output directory lost to the root's per-configuration one and it
  landed beside the executables, where the plugin manager never looks.
- **Found and left for later:** the Windows `.rc` files (icon, version resource) are compiled by the
  vcxproj only; a CMake-built `.exe` has no icon.

**The packages are made to run from wherever they are unpacked.** A build-tree binary carries the
absolute build directory as its library path: the Linux script rewrites every ELF file to `$ORIGIN`
(RUNPATH is not inherited, so the wx libraries get it too), the macOS script rewrites every Mach-O to
`@executable_path/../../..` (bundles), `@executable_path` (flat programs), `@loader_path` (dylibs) and
re-signs it ad hoc, since arm64 refuses a binary whose signature an edit broke. The macOS image holds one
`OES` folder to be moved as a whole: the applications stand beside their libraries and find each other
as siblings.

⚠ **A schedule runs from the default branch.** GitHub reads `on: schedule` (and offers *Run workflow*)
only from the workflow file on `master`; what it builds is `develop`. The workflow also runs only in
`open-enterprise-solutions/enterprise`, not in a fork the tree is pushed to.
