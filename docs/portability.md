...
#### ⚠ A DEBT THIS RULE ALREADY NAMES — the Firebird status vector, walked as `long*`

`ibDatabaseLayerFirebird::TranslateErrorCodeToString` walks the status vector through a cast:
...
`ISC_STATUS` is **pointer-sized** — Firebird declares it as `intptr_t`. So the cast holds only where
`long` is pointer-sized too:

| target | `long` | `ISC_STATUS` | walk |
|---|---|---|---|
| Win32 (what ships today) | 32 | 32 | correct |
| LP64 — Linux, macOS | 64 | 64 | correct |
| **Win64 (LLP64)** | **32** | **64** | **strides half an element** |

Both branches of that function do it, and both have since long before anyone looked; it was copied
into the second branch on 2026-09-06 (PR #99) because the first one already read that way. Nothing
is broken on any target we build **now**, which is exactly why it is written down here instead of
being noticed the day an x64 desktop build reads a status vector and reports the wrong error — or
reads past the end of it. The fix is one word at each site: walk it as `ISC_STATUS*`, which is what
`GetFbInterpret` takes anyway, and let the cast disappear.
...
### 1.5c A cast to its own type is an LVALUE to MSVC — and a template that meets it only in a test

```cpp
EmplaceAt(pos, k, T(std::forward<M>(obj)));   // T&& parameter — refused when T = int
```

The standard makes `T(expr)` a prvalue, so it binds to `T&&`. **MSVC** keeps a cast of a scalar to its
OWN type an lvalue (an old extension), and refuses: *"cannot convert argument 3 from 'T' to 'T &&' —
you cannot bind an lvalue to an rvalue reference"*. The class template (`ibRowValues`, `rowValues.h`) was
instantiated in the engine only with `ibValue` — a class, where the cast is a real temporary — so the
Win32 `.sln` never saw it; `tests/test_rowValues.cpp` instantiates it with `int`, and the x64 CI build
failed there (2026-09-12; fixed `923da9ae`). Name it and move it — portable, and the same code for a class:
...
The wider lesson is § 1.10's: a template's instantiations in the TESTS are part of what must compile, and
only CMake builds them.
...
| a local called `small` (or `near`, `far`) | the OTHER way round — it compiles off Windows and fails ON it: the Windows headers `#define small char` (`rpcndr.h`), so `wchar_t small[256]` reads as `wchar_t char[256]` (2026-09-12, the database string converter) |
...
**And it may not be built in a console test either — the same fault, one scope wider (2026-09-02).**
A `wxFont` is a TOOLKIT object: constructing one asks GTK for a font map, and `oes_tests` has no
`wxApp` and no display. Four spreadsheet round-trip tests died on Linux with
`GLib-GObject-WARNING: invalid (NULL) pointer instance` and a SEGFAULT; **the same tests passed on
macOS**, because wxOSX will make a font without a running application. Two lessons in that
asymmetry: a green macOS job does not clear a GTK question, and "it works on two of the three
toolchains" is the shape this rule exists to catch.

The suite already knew: `test_spreadsheetDocument.cpp` stubs `SetCellFont` out entirely rather than
build one. So — **a backend test constructs no toolkit object.** What cannot be asserted without a
toolkit belongs to the GUI suite CI runs in a job of its own; say so in the test rather than leaving
the gap silent.
...
### 1.9a A `_()` msgid is ASCII, because the LOCALE decodes it (2026-09-02)

`_(s)` hands the literal to `wxGetTranslation` as a `const char*`, and the `wxString` is built with
the **runtime locale's** converter. CI runs under `C` / POSIX, where any byte above 0x7F cannot be
decoded — so the whole string comes back **EMPTY**. Not truncated, not garbled: empty, silently.

Ten MCP tools were published to callers with **no description at all**, and the set of tools whose
description literal contained an em dash matched the set CI reported, exactly and completely. It
passes on Windows and on macOS (UTF-8 by default), which is what kept it invisible.

- Write ASCII in a msgid: `-` for `—`, `...` for `…`, `"` for `«»`. A msgid is a **key**;
  typography belongs in the translation. The catalogue already agreed — `lang/ru/open_es.po` holds
  736 msgids and not one em dash.
- Sweep with `LC_ALL=C grep -rn '_("[^"]*[^ -~]' --include=*.cpp --include=*.h backend frontend`
  (the `LC_ALL` matters: without it `grep -P` refuses the byte class outright).
- ⚠ **Comments are not affected and must not be swept.** One file carried 119 em dashes in prose
  against 16 in literals, so a file-wide replace is churn with a defect hidden inside it.
- ⚠ The contract test covers tool DESCRIPTIONS only; the same sweep found ~19 more empty strings in
  refusals, activity lines and argument texts — 47 across the tree — that nothing was watching.
...
- **In the general jobs, only SQLite actually executes.** Firebird is compiled but loads
  `fbclient` at run time, and the runner has none; ODBC is compile-only. `FirebirdLeaseTest`
  exercises file-lock mechanics against a path that need not exist, not the driver. The other two
  took a job of their own each: `Dialect (PostgreSQL, Debug)` runs `oes_pg_dialect_test` against a
  `postgres:16` service container, with the connection handed in through the environment — so the
  same binary is a silent no-op wherever `OES_PG_USER` is unset, which is what keeps it out of a
  developer's way; and since 2026-09-22 `Firebird (Linux, Debug)` lays the pinned Firebird 5 kit
  out as `_fb/` beside `oes_tests` and runs the `Firebird*` tests through the embedded engine,
  failing on a skip.
