...
| ~~`BalanceAndTurnovers` with a periodicity, on the server~~ | § 8.3 | ✅ **DONE 2026-08-20.** The running sum it needed is a window (`ibQueryExpr::m_over` + `ibRenderOverClause`, gated by `m_features.m_window`); the read spec learned a GRAIN (`ibMaterializeGrain`) and the running forms, so the gate now closes only for a driver that cannot rank. ⚠ **`m_fillEmptyPeriods` still routes to RAM** — a window does not invent a row the surface has no data for, and carrying a balance across an empty month needs a calendar to LEFT JOIN against. Numbers unverified against a live engine; the RAM road is the oracle |
