...
| `src/engine/backend/compiler/byteCodeAOT.cpp` | `kAOTFormatVersion` was bumped to 10 at the `OPER_CALL_LINQ` insertion; **current in-tree value is 20** (CLSID encoding switch → 12, LINQ push-down lambda-AST → 14, kind-typed CLSIDs → 16, `restrict`-pushdown-AST fix → 17, shortLet peephole → 18, parameter default type name → 19, session-parameter context member → 20) — always read the constant from the source file rather than this table |
