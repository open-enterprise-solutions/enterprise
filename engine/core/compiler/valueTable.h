...
public:
	class CValueTableColumns : public IValueTable::IValueTableColumns {
		wxDECLARE_DYNAMIC_CLASS(CValueTableColumns);
	public:

		class CValueTableColumnInfo : public IValueTable::IValueTableColumns::IValueTableColumnsInfo {
	...
	public:
		...
		CValueTableColumns();
		CValueTableColumns(CValueTable *ownerTable);
		virtual ~CValueTableColumns();

		IValueTableColumnsInfo *AddColumn(const wxString &colName, CValueTypeDescription *types, const wxString &caption, int width = wxDVC_DEFAULT_WIDTH)
		{
...
public:
	...
	virtual void AddValue(unsigned int before = 0) { AddRow(before); }
	...
	//check is empty
	virtual inline bool IsEmpty() const override { return m_aObjectValues.empty(); }
	...
	CValueTable *Clone() { return new CValueTable(*this); }
	unsigned int Count() { return m_aObjectValues.size(); }
	void Clear();
